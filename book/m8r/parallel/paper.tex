...
POSIX threads (or pthreads) is a protocol for concurrent execution of
...
files that are not cleaned automatically and can be cleaned by running
\texttt{pscons -c}. However, this feature can be also advantageous for
checkpointing. If the parallel execution is interrupted, it can be
restarted  by running \texttt{pscons} again without the need to
regenerate the intermediate files that were generated during the
previous run.

pscons over multiple nodes...
