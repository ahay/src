...
sys.stderr.write('m8r.py in book directory\n')
...
class File(object):
    ...
    def size(self,dim=0):
        sys.stderr.write("File.size depricated.  Use File.leftsize\n")
...
if _swig_:
    ...
    class Output(_File):
        def __init__(self,tag='out',src=None):
            ...
            sys.stderr.write("swig Output.__init__ test src\n")
            if src: # clone source file
                sys.stderr.write("swig Output.__init__ clone source file\n")
                if hasattr(src,'file'):
                    sys.stderr.write("hasattr(src,'file')\n")
                    srcfile = src.file
                    srctype = src.type
                else:
                    sys.stderr.write("not hasattr(src,'file')\n")
                ...
                sys.stderr.write('at point1 write self.header')
                sys.stderr.write(self.header)
                if not hasattr(src,'file'):
                    c_rsf.sf_fileclose(srcfile)
            _File.__init__(self,self.tag)
            #no self.header kls
            #sys.stderr.write('at point2 write self.header')
            #sys.stderr.write(self.header)

        def write(self,data):
            sys.stderr.write('at point3 write self.header')
            sys.stderr.write(self.header)
            if self.type == 'float':
...
#    this is original Par.  non of this works with _swig_=True
#    par = Par(["prog","a=5","b=as","a=100","par=%s" % sys.argv[0]])
    ...
    n2 = 10
    output.put('n2',n2)
#    assert 10 == output.int('n2') No Ouput.int. Why get from output? karl
