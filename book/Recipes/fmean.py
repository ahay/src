...
def Freqmean(file,r=10,std='n'):
   ...
   Flow(file+'_s1', file, 
        'window min2=0 | math output="abs(input)*x2" | real | stack')
   Flow(file+'_s0',  file, 
        'window min2=0 | math output="abs(input)" | real | stack')
   Flow(file+'_fmn', '%s_s1 %s_s0' %(file,file), 
        'divn rect1=%d den=${SOURCES[1]} verb=n' %r)
   if std=='y':
      Flow(file+'_s2', file,
           'window min2=0 | math output="abs(input)*x2*x2" | real | stack')
...
def Freqplots(out,inp,axes=' ',scalebar='n',plotfat=10,plotcol=1,dash=1,transp='y',yreverse='y'):
   Plot(out,inp,
