...
dd = 2.007;
dd = 2.0;
Do[xss = 0.5*{VnmTTI[N[1*i*Pi/180], delta, 0.2, 2.0, vs, -angle, .2, 
       0.2, 2.0, vs, .0, .2, 0.2, 2.0, vs, 0.0, dd, 0.0, 0.0][[1]] - 
     VnmTTI[-N[1*i*Pi/180], delta, 0.2, 2.0, vs, -angle, .2, 0.2, 2.0,
        vs, .0, .2, 0.2, 2.0, vs, 0.0, dd, 0.0, 0.0][[1]], 
    VnmTTI[N[1*i*Pi/180], delta, 0.2, 2.0, vs, -angle, .2, 0.2, 2.0, 
       vs, .0, .2, 0.2, 2.0, vs, 0.0, dd, 0.0, 0.0][[2]] + 
     VnmTTI[-N[1*i*Pi/180], delta, 0.2, 2.0, vs, -angle, .2, 0.2, 2.0,
        vs, .0, .2, 0.2, 2.0, vs, 0.0, dd, 0.0, 0.0][[2]]};
 dt = xss[[2]] - 
   Vnmapt[xss[[1]], delta, 0.2, 2.0, vs, .2, 0.2, 2.0, vs, .2, 0.2, 
     2.0, vs, 1., 0.5, 0.5][[1]];
 fxst1 = Append[fxst1, {xss[[1]]/2, 100. dt/xss[[2]]}];
 dt = xss[[2]] - 
   Vnmapt[xss[[1]], delta, 0.2, 2.0, vs, .2, 0.2, 2.0, vs, .2, 0.2, 
     2.0, vs, 1., 0.5, 0.5][[2]];
 fxst2 = Append[fxst2, {xss[[1]]/2, 100. dt/xss[[2]]}];
 dt = xss[[2]] - 
   Vnmapt[xss[[1]], delta, 0.2, 2.0, vs, .2, 0.2, 2.0, vs, .2, 0.2, 
     2.0, vs, 1., 0.5, 0.5][[3]];
 fxst3 = Append[fxst3, {xss[[1]], 100. dt/xss[[2]]}];
 dt = xss[[2]] - 
   0.5*LinVeikdsTTI[xss[[1]], delta, 0.2, 2, angle, 2][[2]] - 
   0.5*LinVeikdsTTI[-xss[[1]], delta, 0.2, 2, angle, 2][[2]];
 fxst4 = Append[fxst4, {xss[[1]]/2, 100. dt/xss[[2]]}];
 dt = xss[[2]] - 
   0.5*LinVeikdsTTI[xss[[1]], delta, 0.2, 2, angle, 2][[7]] - 
   0.5*LinVeikdsTTI[-xss[[1]], delta, 0.2, 2, angle, 2][[7]];
 fxst5 = Append[fxst5, {xss[[1]]/2, 100. dt/xss[[2]]}];
 dt = xss[[2]] - 
   0.5*LinVeikdsTTI[xss[[1]], delta, 0.2, 2, angle, 2][[10]] - 
   0.5*LinVeikdsTTI[-xss[[1]], delta, 0.2, 2, angle, 2][[10]];
 ...
 dt = xss[[2]] - 
   LinVeikdsTTI[xss[[1]], delta, 0.2, 2, angle, 2][[8]];
 fxst7 = Append[fxst7, {xss[[1]]/2, 100. dt/xss[[2]]}];
 dt = xss[[2]] - 
   LinVeikdsTTI[xss[[1]], delta, 0.2, 2, angle, 2][[9]];
 fxst9 = Append[fxst7, {xss[[1]]/2, 100. dt/xss[[2]]}], {i, 0, 60}];
...
bb4 = ListPlot[fxst9, Joined -> True, 
  PlotStyle -> {Thickness[0.006`], GrayLevel[0.5`], 
    Dashing[{0.004`, 0.02`, 0.03`, 0.02`}]}];
ll = Show[bb1, bb2, bb3, bb4, Frame -> True, GridLines -> Automatic, 
  LabelStyle -> Directive[Bold, FontFamily -> "Times", 14], 
  FrameLabel -> {{Error["%"],}, {Offset[km],}}, 
  PlotRange -> {{0, 4}, {-2.5, 10}}];
Export["junk_ma.eps", ll, "EPS"];
