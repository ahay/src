...
A transversely isotropic model with a \geosout{tilt in the axis of} \geoulin2{tilted} symmetry \geouline{axis}
...
makes with the vertical, in a generally {\emph{inhomogeneous}} TI \geoulin2{background} with
a vertical axis of symmetry (VTI)\geosou2{background}. I do an additional expansion 
...
are more accurate than other recently derived approximations.
 ...
 gradients (Jacobian) for nonlinear tomographic-based inversion for \geosout{such parameters} the tilt in the symmetry axis\geosou2{ direction}.
...
The vertical symmetry axis (VTI) \geoulin2{medium} is a special case of
...
the eikonal equation. Among the most known methods for solving this equation are
 ray tracing and the finite-difference \geosou2{approximation} \geoulin2{approximations}. Finite-difference solutions 
...
TI medium,
$P$-wave traveltimes in 3D\geoulin2{, under the acoustic assumption,} become dependent on only three \geosout{velocity} \geouline{parameter} fields and two angles.
\geouline{These parameters include the tilt-direction velocity, $v_t$, the normal-moveout \geosou2{like} \geoulin2{equivalent} velocity, $v=v_{t} \sqrt{1+2 \delta}$}
 [where $\delta$ corresponds to the symmetry direction \cite[]{tsvankin:479}], and the anellipticity parameter 
 $\eta=\frac{\epsilon-\delta}{1+2 \delta}$ (\geoulin2{with $\epsilon$
 also defined with respect to} \geosou2{again in} the symmetry direction).
...
3D, the angle $\theta$ that the \geosout{tilt} \geoulin2{symmetry} \geouline{axis} makes
with the vertical \geosout{axis along a vertical plane} and
\geouline{the azimuth}
$\phi$\geosout{, the azimuth} \geosou2{that} \geoulin2{of} the vertical \geouline{symmetry-axis} plane
 \geoulin2{with respect} the $x$-axis [\cite{tsvankin:479}].
...
helps the inversion process. In this paper, \geoulin2{I} \geosou2{we} develop simplified formulations for traveltime calculation that can help in resolving
\geosou2{anisotropic} \geoulin2{anisotropy} parameters, specifically the tilt angle.
...
velocity and NMO\geosou2{-like} \geoulin2{velocity} (=$v_{t} \sqrt{1+2 \delta}$), respectively, \geosou2{in the symmetry direction} 
...
equation is \geoulin2{stated} in Appendix D.
...
\label{eqn:allorder}
\end{equation}
with\geosou2{, now,} $i=\eta,\theta,\eta_2,\eta \theta,\theta_2$, and $\tau_{0}$ satisfies the eikonal equation for an
...
TI traveltime and traveltime moveout in the homogeneous case. This is convenient since most parameter scan-type applications \geoulin2{(i.e. semblance
velocity analysis)}
are \geouline{performed considering} \geosout{applied for} an effective homogeneous medium.

As shown in Appendix C, \geosou2{we} \geoulin2{I} use the simple traveltime relation for an elliptically anisotropic homogeneous background 
...
$\eta=0$,  to allow for a simplified presentation,
 \geosou2{we} \geoulin2{I} obtain an analytical representation \geouline{of traveltime} for tilted elliptical anisotropy given by
\beqa
...
This \geosou2{type} formula \geoulin2{type} (equation~\ref{eqn:eikeqn2}) basically represents a moveout equation for traveltime in TI media as a function of offset (or $x$)
...
\label{eqn:eikeqnnT}
\eeqa
which \geoulin2{reveals} \geosou2{demonstrates} the \geoulin2{additional} \geosou2{level of} approximation
 involved in this equation as even the second-order term is \geosou2{a} linearized \geoulin2{with respect to} \geosou2{approximation
...
t(X)=\frac{\sqrt{X^2+4 z^2} \sqrt{2 (\delta -\epsilon ) \sin^4(\theta -\psi
   ...
   \label{eqn:eiksena}
\eeqa
where \geoulin2{the ray angle,} $\psi$, \geosou2{the ray angle,} satisfies
\beq
...
The analytical equations developed in this section \geosou2{was} \geoulin2{were}
 meant to test the accuracy of the perturbation theory approximations applied to the eikonal
equation. Though they show 
high accuracy in representing the moveout, the \geoulin2{general perturbation formulations} are not meant to be
only used as  an alternative to other available equations derived for
the homogeneous case. \geoulin2{Their perturbation from a background inhomogeneous model allows us to predict traveltime in more
complex media.}
...
be used to estimate the velocity along the \geosou2{tilt} \geoulin2{symmetry axis}
 as well as $\delta$ ($=\frac{v^{2}-v_{t}^{2}}{2 v_{t}^{2}}$). Specifically, the above formulations provide a
mechanism to evaluate the symmetry-direction \geoulin2{(anisotropy)} parameters from \geoulin2{measurements obtained using
...
solid and dashed black curves). This is also \geosou2{observe} \geoulin2{observed} for
anelliptic TI as we will see next.}
\plot{Verticaleta0}{width=7in}{The relative traveltime error as a function of
...
For anelliptic TI media with $\delta=0.1$ and $\epsilon=0.2$, \geosou2{we} \geoulin2{I} obtain similar results. Figure~\ref{fig:Vertical}
...
\plot{Vertical}{width=7in}{The relative traveltime error as a function of
...
\plot{Verticaleta2}{width=7in}{The traveltime error as a function of
...
reasonable to measure $\phi$ from the acquisition \geosout{line} direction. In this case, \geosou2{we} \geoulin2{I} can consider $\phi$ to be small, and thus, 
...
\plot{EtaTheta}{width=7in}{The traveltime difference between the TTI model computed using equation~\protect\ref{eqn:eikeqn2} and the elliptically
...
For a homogeneous background, \geosou2{we} \geoulin2{I} obtain analytic nonhyperbolic moveout equations
...
provide $\eta$ and tilt-angles estimation capabilities \geosou2{valid} for a general inhomogeneous background medium. 
...
\geosou2{we} \geoulin2{I} obtain the zeroth-order term given by
\beqa
...
For an expansion in $\theta$ and $\eta$, simultaneously, \geosou2{we} \geoulin2{I} use the following trial solution:
 \begin{equation}
 \tau(x,z) \approx \tau_{0}(x,z) +\tau_{\eta}(x,z) \eta+\tau_{\theta}(x,z) \sin\theta+ \tau_{\eta_2}(x,z)  \eta^{2}+ \tau_{\eta \theta}(x,z)  \eta \sin\theta+ \tau_{\theta_2}(x,z)  \sin^{2}\theta,
\label{eqn:n0ee2}
\end{equation}
...
To develop analytical traveltime representation for TI media, \geosou2{we} \geoulin2{I} start with a background velocity model that is homogeneous. 
...
\label{eqn:homo1}
\eeq
which satisfies the eikonal equation~\ref{eqn:ellip}. Using equation~\ref{eqn:homo1}, \geosou2{we} \geoulin2{I} evaluate $\frac{\partial \tau _{0}}{\partial x}$ and
...
\geosou2{We} \geoulin2{I} now evaluate $\frac{\partial \tau _{\theta}}{\partial x}$ and $\frac{\partial \tau _{\theta}}{\partial z}$ and use them to solve equation~\ref{eqn:sordert}.
...
For 3-D media, \geosou2{we} \geoulin2{I} include the azimuth angle as we will see next.
...
a vertical symmetry axis \geosou2{we} \geoulin2{I} use vector notations ($n_{x}$ and $n_{y}$) to describe the tilt angles, where the components of this 2D
...
3D. Using equation~\ref{eqn:homo13d}, \geosou2{we} \geoulin2{I} evaluate $\frac{\partial \tau _{0}}{\partial x}$, $\frac{\partial \tau _{0}}{\partial y}$ and
...
\geosou2{We} \geoulin2{I} now evaluate $\frac{\partial \tau _{\eta}}{\partial x}$, $\frac{\partial \tau _{\eta}}{\partial y}$, and $\frac{\partial \tau _{\eta}}{\partial z}$ 
