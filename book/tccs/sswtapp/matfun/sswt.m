...
%------------------------------Documentation-------------------------------
%
% [SWT,freq,Optional:wopt,WT,wfreq,IFR]=sswt(sig,fs,Optional:'PropertyName',PropertyValue)
%
% INPUT:
% sig - signal for which to calculate SWT
% fs  - sampling frequency of the signal
% 
% Properties: ({{...}} denotes default)
% /same as for wt:/
% 'fmin':value (default is the minimal frequency for which at least one SWT
%               coefficient is determined up to a specified relative
%               accuracy [RelTol] with respect to boundary errors)
%            minimal frequency for which to calculate SWT
% 'fmax':value (default = fs/2, i.e. the Nyquist frequency)
%            maximal frequency for which to calculate SWT
...
%            wavelet used in SWT calculation, for a list of all supported
...
%            the closer in frequency components can be resolved in (S)WT,
...
%            the signal in the band of interest [fmin,fmax], for which SWT
%            is calculated
...
%            of influence for (S)WT (i.e. range of (S)WT coefficients which are
...
%            WT so that SWT coefficients in the [fmin,fmax] are
%            determined with a specified accuracy in relation to boundary
%            effects in frequency (i.e. at least (1-RelTol) of the full
%            area under the WT peaks is acocunted for, see [1]).
% 'Plot':{{'off'}} - do not plot anything
%          'amp'   - plots SWT amplitude (i.e. its absolute value) together
%                    with line denoting the cone of influence
%          'amp+'  - additionally shows time-averaged SWT amplitude
%          'amp++' - additionally shows 95% range of SWT amplitude
%          'pow','pow+','pow++' - the same but for the SWT power (i.e. its 
%                                 squared modulus)
%           IMPORTANT: to avoid plotting huge data (in which case it might
%           be very slow to render and modify the figure, and MatLab can
%           even crash), the plotted SWT is resampled to have no more than
%           few data points displayed per pixel (for the current screen
%           resolution). Unless 'Display' is 'off', it will always notify if
%           the SWT size exceeds the number of pixels on the plot and the
...
%           structure of the original SWT. If one wants to investigate the
%           resultant plot in fine details (and not only see how it looks),
%           then the original, full SWT might be displayed without resampling
...
% 'CutEdges':{{'off'}}|'on'
%           determine should (S)WT coefficients be set to NaNs out of the cone
%           of influence (determined for the current 'RelTol'); if you wish
%           to analyze only (S)WT within the cone of influence, which is
%           recommended if you want to estimate e.g. only the time-averaged
%           quantities, set it to 'on'.
% /additional to the properties of wt:/
% 'CutFreq':{{'on'}}|'off'
%           as discussed in [1], WT used for synchrosqueezing should
%           be calculated in a broader frequency range to assure the
%           specified relative error 'RelTol' in SWT in a given range
%           [fmin,fmax]; if WT is requested as output, 'CutFreq' specifies
%           should it be truncated to the frequency band of SWT
% 'Threshold':value|'MAD' (default = 0, i.e. no denoising)
%           before performing synchrosqueezing, one can additionally
%           denoise the calculated WT by hard thresholding, i.e. set
%           WT coefficients lower than the specified 'Threshold' to zero;
%           when set to 'MAD', the value of the threshold is determined as
%           the median between median deviations of the WT amplitude
%           (around its median value) at frequencies from
%           max([fmax/2,sqrt(fmin*fmax)]) to [fmax], multiplied on
%           1.4826*sqrt(2*log(L)) (L is the signal length in samples), as
%           proposed in arXiv:1105.0010
%           (for 'MAD' it is better to calculate WT for a full range,
...
%           frequencies between max([fmax/2,sqrt(fmin*fmax)]) and [fmax])
%
%
% OUTPUT:
% SWT   - synchrosqueezed wavelet transform of the signal [sig],
...
% freq  - frequencies corresponding to rows of SWT
% wopt  - structure with all parameters of the wavelet and simulation
% WT    - wavelet transform, from which SWT was constructed,
%         with rows corresponding to frequencies and columns - to time;
...
%         [freq] for SWT, unless 'CutFreq' is set to 'on')
% wfreq - frequencies corresponding to rows of WT
% IFR   - the Instantaneous Frequency Representation corresponding to the
%         current WT ([IFR] is \nu_G/2\pi in notations of [1]), i.e.
%         the rate of phase growth of WT coefficients at each time and 
%         frequency, i.e. in fact WFT instantaneous frequency;
%         obviously, has the same size as returned WT.
%
%-------------------------------Examples-----------------------------------
%
% [SWT,freq]=sswt(sig,100,'fmin',0.1,'fmax',10,'f0',2)
% given a signal [sig] sampled at 100 Hz, returns its SWT [SWT] based on a
% Lognormal wavelet (default) with [f0=2] and calculated for frequencies from
...
% [SWT,freq]=sswt(sig,fs,opts);
% You can also add further parameters in the usual way, e.g.
% [SWT,freq]=sswt(sig,fs,opts,'Display','off');
% If you add a parameter contained in [opts], it will be ovewritten, e.g.
% [SWT,freq]=sswt(sig,fs,opts,'fmax',10);
% will change 'fmax' property from 12 specified in [opts.fmax] to 10.
%
% When one needs to calculate SWT using the same wavelet and simulation
% parameters, but for different signals, it is a good idea to do it like
% [SWT1,freq1,wopt]=sswt(sig1,fs1,...(parameters));
% [SWT2,freq2]=sswt(sig2,fs2,wopt); [SWT3,freq3]=sswt(sig3,fs3,wopt); ...
% This will also avoid recalculating wavelet parameters each time, thus
% slightly speeding up the computations.
%
% The same can be done if you calculated SWT of a signal but want to obtain
% it using some different parameter (e.g. Padding), for example
% [SWT1,freq1,wopt]=sswt(sig,fs,'Padding',0,...(other parameters));
% [SWT2,freq2]=sswt(sig,fs,wopt,'Padding','predictive');
% Unless you overwrite 'f0', 'Window' or 'RelTol' properties, the wavelet
% parameters will not be recalculated for the second time.
%
%------------------------------Changelog-----------------------------------
%
% v1.01:
% - changed estimation of WT instantaneous frequencies [IFR] from direct
...
function [SWT,freq,varargout] = sswt(signal,fs,varargin)
...
r1=wp.ompeak/wp.xi2e; r2=wp.ompeak/wp.xi1e; %multipliers to determine appropriate WT frequency range
if isempty(fmin), fmin=(wp.ompeak/2/pi)*(wp.t2e-wp.t1e)*(fs/L)/r1; end %if not specified, determine minimum possible SWT frequency for accuracy RelTol
...
if (wp.t2e-wp.t1e)*wp.ompeak/(r1*2*pi*fmax)>L/fs
    if ~strcmpi(DispMode,'off')
        fprintf(2,'--------------------------------------------- Warning! ---------------------------------------------\n');
        wstr='';
        wstr=[wstr,sprintf('For used wavelet function parameters and signal time-length there are no (S)WT coefficients determined with specified accuracy %1.2e.\n',RelTol)];
...
elseif (wp.t2e-wp.t1e)*wp.ompeak/(r1*2*pi*fmin)/(L/fs)>1+2*eps
    if strcmpi(DispMode,'on') || strcmpi(DispMode,'notify')
        fprintf(2,'--------------------------------------------- Warning! ---------------------------------------------\n');
        wstr='';
        wstr=[wstr,sprintf('At lowest frequency %1.2e there are no SWT coefficients determined with specified accuracy %1.2e.\n',fmin,RelTol)];
        wstr=[wstr,sprintf('There is nothing crucial in this, but (S)WT computation might be slower due to using more padding.\n')];
        wstr=[wstr,sprintf('If you want to use only SWT from the cone of influence (i.e. determined with specified precision),\n')];
        wstr=[wstr,sprintf('then better increase the minimal frequency to at least %1.2e or change wavelet resolution parameter(s).\n',(wp.t2e-wp.t1e)*(wp.ompeak/(r1*2*pi))/(L/fs))];
        fprintf(wstr);
        fprintf(2,'----------------------------------------------------------------------------------------------------\n');
    end
end
...
%Define WT frequencies
wfmin=r1*fmin; wfmax=r2*fmax; %WT frequency range
nvsim=nv; wp.nv=nv;
...
wfreq=2.^((ceil(nv*log2(wfmin)):floor(nv*log2(wfmax)))'/nv); % frequencies (exact values depend only on [nv])
SN=length(wfreq); %number of frequencies in WT
coib1=ceil(abs(wp.t1e*fs*(wp.ompeak./(2*pi*wfreq)))); coib2=ceil(abs(wp.t2e*fs*(wp.ompeak./(2*pi*wfreq)))); %cone of influence edges
...
WT=zeros(SN,L)*NaN; IFR=zeros(SN,L)*NaN; ouflag=0; if (wp.t2e-wp.t1e)*wp.ompeak/(r1*2*pi*fmax)>L/fs, coib1(:)=0; coib2(:)=0; end
if strcmpi(DispMode,'on'), pos=0; fprintf('Calculating Wavelet Transform and its phase growth rate (%d frequencies from %0.3f to %0.3f): ',SN,wfreq(1),wfreq(end)); end
for sn=1:SN
    freqwf=ff*wp.ompeak/(2*pi*wfreq(sn)); %frequencies for the wavelet function
    ...
    if ~isempty(fwt)
        ...
        timewf=(2*pi*wfreq(sn)/wp.ompeak)*(1/fs)*[-(1:ceil((NL-1)/2))+1,NL+1-(ceil((NL-1)/2)+1:NL)]';
    ...
    out=((wp.ompeak/(2*pi*wfreq(sn)))^(1-p))*ifft(cc,NL); % calculate WT at each time
    WT(sn,1:L)=out(1+n1:NL-n2);
    
    dcc=zeros(NL,1); dcc(ii)=fx(ii).*(1i*2*pi*ff(ii)).*fw(:); %convolution in the frequency domain
    dout=((wp.ompeak/(2*pi*wfreq(sn)))^(1-p))*ifft(dcc,NL); % calculate WT time derivative at each time
...
if strcmpi(CutEdges,'on') % cut unphysical coefficients
    icoib=find(coib1+coib2-L>0); WT(icoib,:)=NaN; 
    ...
    lid=sub2ind([SN,L],frn,ttn); WT(lid)=NaN; IFR(lid)=NaN;
end
if nargout>5
...
freq=wfreq(wfreq>=fmin & wfreq<=fmax); %frequencies for SWT (same as for WT in the corresponding range)
FN=length(freq); % number of frequencies

%Filter out white noise (if needed) using hard-thresholding
if strcmpi(Threshold,'MAD')
    iwfreq=find(wfreq>max([fmax/2,sqrt(fmin*fmax)]));
    MAD=zeros(length(iwfreq),1); %median absolute deviation
    for sn=1:length(iwfreq)
        cwn=iwfreq(sn);
        MAD(sn)=median(abs(abs(WT(cwn,~isnan(WT(cwn,:))))-median(abs(WT(cwn,~isnan(WT(cwn,:)))))));
        MAD(sn)=MAD(sn)*((wp.ompeak/(2*pi*wfreq(cwn))).^(p-1/2));
    end
...
if cthr>0
    mfilt=cthr*((wp.ompeak./(2*pi*wfreq)).^(1/2-p))*ones(1,L); %filtering matrix accounting for biases due to different choices of p
    WT(abs(WT)<mfilt)=0; %set WT coefficients lower than threshold to zero (wavelet shrinkage denoising)
end

%Transform phase growth rate [IFR] to numbers of frequency bins
IFR(IFR<2^(-1/nv/2)*freq(1))=NaN; %to avoid complex numbers and cut off unneeded frequencies
IFR=1+floor((1/2)+nv*log2(IFR/freq(1))); % calculate corresponding frequency bins
...
allsv=(log(2)/(nv*wp.C))*WT(sub2ind([SN,L],allsc,alltt)).*((2*pi*wfreq(allsc)/wp.ompeak).^(1-p));
SWT=sparse(allfr,alltt,allsv,FN,L); clear allsc allfr alltt allsv;

%Determine the cone of influence (for specified RelTol) for SWT from that of WT
nnup=find(wfreq>=fmin,1,'first');
coib1(nnup:end)=coib1(1:end-nnup+1); coib2(nnup:end)=coib2(1:end-nnup+1);
coib1=coib1(wfreq>=fmin & wfreq<=fmax); coib2=coib2(wfreq>=fmin & wfreq<=fmax);

%Cut or delete WT to free more memory
if nargout<4, clear WT;
elseif strcmpi(CutFreq,'on')
    WT=WT(wfreq>=fmin & wfreq<=fmax,:);
    wfreq=freq;
end

SWT=full(SWT); %transform SWT to a full form

%Set to NaN all SWT coefficients outside the cone of influence if specified
if strcmpi(CutEdges,'on')
    icoib=find(L-coib1-coib2<=0); SWT(icoib,:)=NaN;
    ovL=ceil(sum(coib1+coib2)-L*length(icoib)); frn=zeros(ovL,1)*NaN; ttn=zeros(ovL,1)*NaN; qn=0;
    for fn=1:FN, cL=coib1(fn)+coib2(fn);
        if cL>0 && cL<L, frn(1+qn:qn+cL)=fn; ttn(1+qn:qn+cL)=[1:coib1(fn),(L-coib2(fn)+1):L]'; qn=qn+cL; end
    end
    frn=frn(1:qn); ttn=ttn(1:qn);
    lid=sub2ind([FN,L],frn,ttn); SWT(lid)=NaN;
end
...
if ~strcmpi(PlotMode,'off')
    ...
    YY=freq; XX=(0:(L-1))/fs; ZZ=abs(SWT); ZZname='SWT amplitude';
    if ~isempty(strfind(lower(PlotMode),'pow')), ZZ=ZZ.^2; ZZname='SWT power'; end
    ...
    if isempty(strfind(lower(PlotMode),'wr')) && (size(ZZ,1)>MYL || size(ZZ,2)>MXL)
        if ~strcmpi(DispMode,'off')
            fprintf('Warning: SWT contains more data points (%d x %d) than pixels in the plot, so for a\n',size(ZZ,1),size(ZZ,2));
    ...
    if ~isempty(strfind(lower(PlotMode),'+'))
        ...
        hold all;
        mx=zeros(FL,1)*NaN; for fn=1:FL, mx(fn)=mean(ZZ(fn,~isnan(ZZ(fn,:))),2); end
