...
#    Flow(dif,[pwds,vel],'window f4=1 | inmo velocity=${SOURCES[1]}')
    ...
    '''
#  pad end1=500 beg2=200 end2=200     
    velcon = '''
    pad beg2=%d end2=%d beg1=%d | cosft sign2=1 | put o3=0 | 
    stolt vel=%g | 
    vczo nv=%d dv=%g v0=%g |
    transp plane=23 |
    cosft sign2=-1
    ''' % (pad_af1,pad_af2,beg1,v0,nv,dv,v0)
...
#% (padx,beg1,v0,nv,dv,v0,nx,beg1,x0)
    ...
    Flow(vlfpad,pwd,taper+' | '+velcon)
    Plot(vlfpad,'grey title="VC padded"')
    
    Flow(vlf,vlfpad,wind)
    Plot(vlf,'transp plane=23 | grey title="VC windowed"')
...
#
    Flow('vlf2_pad',stk,taper+' | '+velcon)
    Flow('vlf2','vlf2_pad',wind)
    
    
    Flow(vlf+'2',stk,taper+' | '+velcon+' | '+wind)
#
