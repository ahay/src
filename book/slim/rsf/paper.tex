...
\newpage
\section{Transforms}
\subsection{sffdct - 2D fast discrete curvelet transform}

\begin{verbatim}
NAME
        sffdct
SYNOPSIS
        sffdct < in.rsf > out.rsf nbs= nba= ac=n adj=n
COMMENTS
        Fast Discrete Curvelet Transform

        Written by: Gilles Hennenfent, UBC
        Created: January, 2007
        
PARAMETERS
        int     nbs             number of scales
        int     nba             number of angles at the 2nd coarsest scale
        bool    ac=n [y/n]      curvelets at the finest scale, wavelets otherwise.
        bool    adj=n [y/n]     forward transform, adjoint/inverse otherwise
SOURCE
        user/slim/fdct.py
\end{verbatim}
