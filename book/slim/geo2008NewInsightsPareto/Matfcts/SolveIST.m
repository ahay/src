function [x, xnorms, rnorms] = SolveIST(A, y, Iters, lambda, x, fullPath)
...
% MAIN LOOP
%----------------------------------------------------------------------
h = waitbar(0,'SolveIST - Please wait...');
for i = 1:Iters
    xTmp = SoftThr(x + projres - Aprod(Aprod(x,1),2),lambda);
    if fullPath
        rnorms(i+1) = norm(y-Aprod(xTmp,1),2);
        xnorms(i+1) = norm(xTmp,1);
    end
    x = xTmp;
    iter = iter+1;
