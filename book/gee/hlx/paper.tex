...
\opdex{helicon}{helical convolution}{36}{54}{user/gee}
The code fragment
\verb#aa->lag[ia]#
corresponds to 
\texttt{b-1}
...
\opdex{polydiv}{helical deconvolution}{55}{74}{user/gee}
...
vertical-axis helix to see its two-dimensional shape $\mathcal R$:
\begin{equation}
\mathcal r
\quad
\xrightarrow{\rm helical \; boundaries}
...
to see its two-dimensional shape $\mathcal H$
\begin{equation}
\label{eqn:lapfac}
{\mathcal H}
\eq
...
\moddex{wilson}{Wilson-Burg spectral factorization}
%
\begin{exer}
\item
...
\activesideplot{mam}{width=3.8in,height=3.8in}{} {
...
\activeplot{helgal}{width=6in,height=3.7in}{} {
...
\activeplot{scratch}{width=6in,height=3.0in}{} {
	Not a scratch
	}
...
\activesideplot{3dpef}{width=2.3in}{NR}{
...
\texttt{createhelixmod} \vpageref{lst:createhelixmod}.
...
Module \texttt{cartesian} below contains
...
\texttt{cart2line} and
\texttt{line2cart}.
\moddex{cartesian}{helical-cartesian coordinate conversion}
...
The \texttt{cartesian} module fails for negative subscripts.
...
module \texttt{createhelixmod} %\vpageref{lst:createhelixmod}
first finds the
...
\moddex{createhelixmod}{constructing helix filter in N-D} 
Near
...
\texttt{cube(n1,n2,...)} is
module \texttt{box}: %\vpageref{lst:box}.
\moddex{box}{Convert helix filter to (n1,n2,...)}
The \texttt{box} module is normally used
to display or manipulate a filter
that was estimated in helical form
(usually estimated by the least-squares method).

\par
The inverse process to \texttt{box} is to
...
\texttt{createhelixmod} \vpageref{lst:createhelixmod} to create a filter.
I set all the helix filter coefficients to 2.
Then I used module \texttt{box} \vpageref{lst:box}
...
\moddex{regrid}{Convert filter to different data size}
