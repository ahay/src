...
\activeplot{antoine11}{width=6in,height=8.0in}{ER}{
...
The Fortran-90 programming language allows us
to achieve this design goal by means of generic function interfaces.
\par
A basic solver is \texttt{solver_tiny()}.


\moddex{solver_tiny}{tiny solver}

\par
The two most important arguments in \texttt{solver_tiny()}
...
\texttt{cgstep()} \vpageref{/prog:cgstep}
for iterative solving the operator
\texttt{matmult} \vpageref{/prog:matmult}
would write the call
\par
\texttt{call solver_tiny ( matmult\_lop, cgstep, ...}
\par\noindent
so while you are reading the \texttt{solver_tiny} module,
you should visualize the \texttt{Fop()} function
as being \texttt{matmult_lop}, and
you should visualize the \texttt{stepper()} function
as being \texttt{cgstep}.

\par
The other required parameters to \texttt{solver_tiny()} 
...
\subsection{Why Fortran 90 is much better than Fortran 77}
...
Then module
\texttt{solver_tiny} \vpageref{/prog:solver_tiny}
...
\par
Now we assemble a module \texttt{cgmeth} for solving simultaneous equations.
Starting with the conjugate-direction module {\tt cgstep\_mod} 
\vpageref{/prog:cgstep}
we insert the module \texttt{matmult} \vpageref{/prog:matmult} as the linear operator.
\moddex{cgmeth}{demonstrate CD}
...
\item
What linear operator subroutine from Chapter \ref{ajt/paper:ajt}
...
\activesideplot{invstack90}{width=3in}{ER}{
...
\par
The seismograms at the bottom of Figure~\ref{fig:invstack90}
...
which is just like
\texttt{cgmeth} \vpageref{/prog:cgmeth} except that the matrix-multiplication operator
\texttt{matmult} \vpageref{/prog:matmult} has been replaced by
\texttt{imospray} \vpageref{/prog:imospray}.
Studying the program,
you can deduce that,
except for a scale factor,
the output at {\tt niter=0} is identical to the stack $\bold M' \bold d$.
All the signals in Figure~\ref{fig:invstack90} are intrinsically the same scale.%
\moddex{invstack}{inversion stacking}
\par
...
\sx{phase unwrapping}
\sx{phase}
Figure \ref{fig:vesuvio90} shows
radar\footnote{
...
The product's amplitude and phase are shown in Figure \ref{fig:vesuvio90}.
Examining the data,
...
as shown in Figure \ref{fig:squeeze90}.
...
\activeplot{squeeze90}{width=6in,height=3in}{ER}{
        Phase based on decimated data (left)
        and smoothed data (right).
        }

\par
From Figures \ref{fig:vesuvio90} and \ref{fig:squeeze90}
...
module \texttt{unwrap} \vpageref{/prog:unwrap}.
...
Figure \ref{fig:screw90} shows the locations
...
the planes in Figure \ref{fig:screw90} would be one-third covered with dots
but as expected, we see considerably fewer.
\activeplot{screw90}{width=6in,height=3in}{ER}{ 
...
like the modules \texttt{cgmeth()} \vpageref{/prog:cgmeth} and 
\texttt{invstack()} \vpageref{/prog:invstack}.
...
Figure~\ref{fig:veshigh90}.
\activeplot{veshigh90}{width=6in,height=3in}{ER}{ 
        Estimated altitude.
        }
To summarize, the input is the phase map
Figure~\ref{fig:vesuvio90}
...
in Figure \ref{fig:vesuvio90} might be such
...
data in Figure \ref{fig:synmod90},
which is noise free and definitely has one.
\par
We notice the polarity of the synthetic data in \ref{fig:synmod90}
...
\item
 ...
 \texttt{klick()} \vpageref{/prog:klick}
 make snapshots 
...
Gradient operator \texttt{igrad2} \vpageref{/prog:igrad2}
...
%Hint: Compare with
%\texttt{icai2} \vpageref{/prog:icai2} and
%\texttt{tcai2} \vpageref{/prog:tcai2}.
\end{exer}

\activeplot{synmod90}{width=6in,height=1.5in}{ER}{ 
...
where inside \texttt{solver} \vpageref{/prog:solver_tiny},
...
\texttt{cgstep} module \vpageref{/prog:cgstep} is not the
...
%I include module \texttt{lsqr} \vpageref{/prog:lsqr} without explaining
%why it works.
%The interface is similar to \texttt{solver}
%\vpageref{/prog:smallsolver}. Note that the residual vector does not appear
...
%Module \texttt{conjgrad} \vpageref{/prog:conjgrad} provides an
%implementation of this method. The interface is exactly similar to
%that of \texttt{cgstep} \vpageref{/prog:cgstep}, therefore you can
