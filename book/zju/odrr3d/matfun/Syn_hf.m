...
for t=-0.055:0.002:0.055
    k=k+1;
    b1(k)=(1-2*(pi*60*t).^2).*exp(-(pi*60*t).^2);
    b2(k)=(1-2*(pi*60*t).^2).*exp(-(pi*60*t).^2);
    b3(k)=(1-2*(pi*60*t).^2).*exp(-(pi*60*t).^2);
    b4(k)=(1-2*(pi*60*t).^2).*exp(-(pi*60*t).^2);
end
for i=1:m
  t1(i)=round(140);
  t3(i)=round(-6*i+180);
...
d=yc_bp(d,0.002,58,60,100,102);
randn('state',201314);
n=0.1*randn(size(d));
n=yc_bp(n,0.002,58,60,100,102);
...
%% denoise
flow=60;fhigh=100;dt=0.002;N=6;verb=0;
