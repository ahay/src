function [ D1 ] = rr5d_lb_recon(D,MASK,flow,fhigh,dt,N,Niter,eps,verb,mode,iflb,a)
%  RR5D_LB_RECON: Rank-reduction method for 5D simultaneous denoising and reconstruction
...
function [dout]=P_R(din,N, iflb)
% Rank reduction on the block Hankel matrix
[n1,n2]=size(din);
switch iflb
    case 0
        %      [U,B,V]=svds(din,N); % a little bit slower for small matrix
        %      dout=U*D*V';
        % %
        [U,B,V]=svd(din);
    case 1
