function [ D1,H ] = fxydmssa_auto(D,flow,fhigh,dt,Nmax,NN,verb,mode,f,eps)
%  FXY_DMSSA: F-XY domain damped multichannel singular spectrum analysis (MSSA)
...
%       Nmax:      number of singular value to be preserved
...
function [dout,H]=P_R(din,Nmax,NN,mode,eps)
...
for j=1:N
    E(j,j)=E(j,j)*(1-E(N+1,N+1)^NN/E(j,j)^NN);
end
