...
Subroutine \texttt{simpleft()} \vpageref{/prog:simpleft}
...
Subroutine \texttt{ftu()} \vpageref{/prog:ftu}, however has a frequency range
...
% used by subroutine~\texttt{ftu()} \vpageref{/prog:ftu},
...
\opdex{halfint}{half derivative}{52}{68}{filt/imag}
%\newslide

\par
In practice, it is easiest to represent
and to apply the 2-D Huygen's wavelet in the frequency domain.
Subroutine \texttt{halfint()} \vpageref{/prog:halfint} is provided for that purpose.
...
(vanishes when $t<t_0$).
Fourier transform is done using subroutine \texttt{ftu()} \vpageref{/prog:ftu}.
...
found in subroutine \texttt{halfint()} \vpageref{/prog:halfint}.
...
\item	If we input a one layer model $\delta(t-t_0)\,{\rm const}(x)$
	to subroutine \texttt{kirchslow()} \vpageref{/prog:kirchslow},
