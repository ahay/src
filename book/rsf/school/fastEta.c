...
/*< Run fast marching eikonal solver >*/
{
  float xs[3], *p, ***tx, ***ty, ***tz, ***dD, ***dw, *pt, *ptt, *ttime, *dtime, *ddtime;
    float tmp;
    ...
    ttime[i] = time[i];
    warn("time=%f dtime=%f eta=%f",time[i],dtime[i],eta);
	...
	*(pm = mask+i) = FMM_IN;
	...
	/*time[i] = ttime[i];*/
	time[i] += dtime[i]*eta;
	warn("time=%f time2=%f dtime=%f eta=%f",time[i],time[i],dtime[i],eta);
      }
      return;
    }
  ...
  nm = n;
	...
	*(pm = mask+i) = FMM_IN;
	...
	warn("time2=%f ddtime=%f eta=%f",time[i],ddtime[i],eta*eta);
      }
    } else {
      for (i=0; i<n; i++) {
	/*time[i] = ttime[i];*/
	...
	time[i] += dtime[i]*dtime[i]*eta/tmp;
	warn("time2=%f dtime==%f ddtime==%f ddtimeS=%f eta=%f",time[i],dtime[i]*eta,dtime[i]*eta+ddtime[i]*eta*eta,dtime[i]*dtime[i]*eta/(-ddtime[i]*eta+dtime[i]),eta*eta);
      }
    }
