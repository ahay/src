...
\multiplot{4}{cmp,nmo,taup,nmo2}{width=.35\textwidth}{CMP gather from Canterbury data set before applying NMO (a). After applying NMO (b). After Forward paraolic Radon transfrom (c). The gather in (c) after applying inverse parabolic Radon transform (d). The forward and inverse parabolic Radon transforms are applied in sequence to examine the parameters of the process and that events are not lost during the process}
