...
\lefthead{Maurice}
\righthead{Tutorial}
\footer{Madagascar School at Delft}

\maketitle

\begin{abstract}
  In this tutorial, you will go through the steps required for writing a research paper with reproducible examples. In particular, you will
  \begin{enumerate}
...
\item What does the \texttt{sfsmooth} module do? Try to find it out by running
\begin{verbatim}
bash$ sfsmooth
\end{verbatim}
...
\end{verbatim}
and by inserting it as a new value for the \texttt{bias=} parameter in the \texttt{SConstruct} file. Does smoothing by \texttt{sfsmooth} change the mean value?
...
part affected by your change needs to be updated.
\end{enumerate}
...
\begin{equation}
\label{eq:gaussian}
W_G(x) = \exp{\left(-\alpha\,\frac{|x|}{x_0}\right)}
\end{equation}
Repeat the previous computation. Does the result change? What is a good value for $\alpha$? 
\item Thinking about this problem, you invent an idea\footnote{In fact, you reinvent the idea of \emph{bilateral} or \emph{non-local} filters
...
from the center point but also on the difference in function values between the two points. That is, instead of filtering by
\begin{equation}
\label{eq:local}
g(x) = \int f(y)\,W(x-y)\,dy\;,
\end{equation}
where $f(x)$ is input, $g(y)$ is output, and $W(x)$ is a linear weight, filter by
\begin{equation}
\label{eq:nonlocal}
...
\multiplot{2}{triangle,similarity}{width=0.45\textwidth}{(a) Linear and stationary triangle weights. (b) Non-linear weights reflecting both distance
...
\lstinputlisting[frame=single]{Mnonloc.c}
...
\item Edit the file \texttt{paper.tex} to include your additional results. If you have not used \LaTeX\ before, no worries. It is a descriptive language. Study the file, and it should become evident how to include figures.
\item Run
\begin{verbatim}
bash$ scons paper.pdf
\end{verbatim}
and open \texttt{paper.pdf} with an PDF viewing program such as \textbf{Acrobat Reader}. Enjoy your new paper?
\item Want to submit your paper to \emph{Geophysics}? Edit \texttt{SConstruct} in the paper directory to add \texttt{option=manuscript} to the \texttt{End} statement. Then run
\begin{verbatim}
bash$ scons paper.pdf
\end{verbatim}
