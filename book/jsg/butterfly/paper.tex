...
\plot{fftabs}{width=1\textwidth}{The Fourier transform (absolute value) on time axis of the synthetic data in Figure \ref{fig:S1_data}.}

\plot{fmod}{width=1\textwidth}{$N_{\tau}=N_p=1000$. Output of the fast butterfly algorithm applied to the synthetic data in Figure \ref{fig:S1_data}. $N=32$, $q_{k_1}=q_{k_2}=q_{x_1}=q_{x_2}=9$. CPU time: {\bf 1.75 s}. Purple curve overlaid is the true slowness.}

\plot{dimod}{width=1\textwidth}{$N_{\tau}=N_p=1000$. Output of the {\it velocity scan} applied to the synthetic data in Figure \ref{fig:S1_data}. CPU time: {\bf 37.23 s}. Purple curve overlaid is the true slowness.}

\plot{diimod}{width=1\textwidth}{Difference between the results of the fast algorithm and the direct evaluation of equation \ref{sum} plotted at the same scale as in Figure \ref{fig:S1_fmod}.}
...
We now make two synthetic datasets using rectangular sampling $N_t=4000$, $N_h=400$. The first one (Figure \ref{fig:S2_data}) has the same range as the previous example (Figure \ref{fig:S1_data}), while the second one (Figure \ref{fig:S3_data}) doubles the range of time and offset. Results of the fast algorithm are shown in Figures \ref{fig:S2_fmod} and \ref{fig:S3_fmod}. The purpose of showing these two examples is to demonstrate that the choice of $N$ does not depend on the problem size, but rather on the range of parameters --- for the data in Figure \ref{fig:S3_data}, one has to increase $N$ to preserve the same accuracy (the range of $\Phi=f\sqrt{\tau^2+p^2h^2}$ is about 125 for the first dataset, and 250 for the second one).

\inputdir{synth2D-2}
\plot{data}{width=1\textwidth}{2D synthetic CMP gather. $N_t=4000$, $N_h=400$. $\Delta t=0.001$ s, $\Delta h=0.0125$ km.}

\inputdir{synth2D-3}
\plot{data}{width=1\textwidth}{2D synthetic CMP gather. $N_t=4000$, $N_h=400$. $\Delta t=0.002$ s, $\Delta h=0.025$ km.}

\inputdir{synth2D-2}
\plot{fmod}{width=1\textwidth}{$N_{\tau}=4000$, $N_p=400$. Output of the fast butterfly algorithm applied to the synthetic data in Figure \ref{fig:S2_data}. $N=32$, $q_{k_1}=q_{k_2}=q_{x_1}=q_{x_2}=9$. CPU time: {\bf 2.46 s}. Ref: CPU time of {\it velocity scan}: {\bf 21.84 s}. Purple curve overlaid is the true slowness.}

\inputdir{synth2D-3}
\plot{fmod}{width=1\textwidth}{$N_{\tau}=4000$, $N_p=400$. Output of the fast butterfly algorithm applied to the synthetic data in Figure \ref{fig:S3_data}. $N=64$, $q_{k_1}=q_{k_2}=q_{x_1}=q_{x_2}=9$. CPU time: {\bf 4.35 s}. Ref: CPU time of {\it velocity scan}: {\bf 21.93 s}. Purple curve overlaid is the true slowness.}
...
\inputdir{synth2D-4}
\plot{data}{width=1\textwidth}{3D synthetic CMP gather. $N_t=1000$, $N_{h_1}=N_{h_2}=128$. $\Delta t=0.004$ s, $\Delta h_1=\Delta h_2=0.08$ km.}

\plot{fmod}{width=1\textwidth}{$N_{\tau}=1000$, $N_p=128$. Output of the fast butterfly algorithm applied to the synthetic data in Figure \ref{fig:S4_data}. $N=64$, $q_{k_1}=q_{k_2}=q_{x_1}=q_{x_2}=5$. CPU time: {\bf 1.67 s}. Purple curve overlaid is the true slowness.}

\plot{dimod}{width=1\textwidth}{$N_{\tau}=1000$, $N_p=128$. Output of the {\it velocity scan} applied to the synthetic data in Figure \ref{fig:S4_data}. CPU time: {\bf 125.54 s}. Purple curve overlaid is the true slowness.}
...
\plot{fftabs}{width=1\textwidth}{The Fourier transform (absolute value) on time axis of the field data in Figure \ref{fig:F1_samiss1}.}

\plot{fmod}{width=1\textwidth}{$N_{\tau}=1500$, $N_p=800$. Output of the fast butterfly algorithm applied to the field data in Figure \ref{fig:F1_samiss1}. $N=128$, $q_{k_1}=q_{x_1}=7$, $q_{k_2}=q_{x_2}=5$ (here the range of $\Phi=f\sqrt{\tau^2+p^2h^2}$ is about 306). CPU time: {\bf 6.62 s}.}

\plot{dimod}{width=1\textwidth}{$N_{\tau}=1500$, $N_p=800$. Output of the {\it velocity scan} applied to the field data in Figure \ref{fig:F1_samiss1}. CPU time: {\bf 9.91 s}.}
...
\inputdir{synth2D-1-timer}
\plot{fdat}{width=1\textwidth}{Output of the adjoint fast butterfly algorithm applied to the data in Figure \ref{fig:S1_fmod}. $N=32$, $q_{k_1}=q_{k_2}=q_{x_1}=q_{x_2}=9$.}

\plot{didat}{width=1\textwidth}{Output of the adjoint {\it velocity scan} applied to the data in Figure \ref{fig:S1_dimod}.}
