...
\begin{abstract}
\new{Reflection moveout approximations are commonly used for velocity analysis, stacking, and time migration.}
...
particular choice of \old{parameters} \new{coefficients}. By testing
the accuracy of the proposed approximation with analytical and
numerical examples, we show that it can bring
several-orders-of-magnitude improvement in accuracy \new{at large
offsets} compared to known \new{analytical} approximations, which
makes it as good as exact for \old{all} \new{many} practical purposes.
\end{abstract}

\section{INTRODUCTION}

\new{Reflection moveout approximations are commonly used for velocity analysis, stacking, and time migration \cite[]{yilmaz}.}
...
by expanding it into a sum of basis functions. \cite{douma} \new{and
...
several orders of magnitude. \new{Potential applications of the new
...
\end{eqnarray}
The \new{existence of the} nonhyperbolic part \old{of} \new{in} the traveltime approximation~\ref{eq:mo}
and~\ref{eq:mo2} is controlled by parameter \old{$\xi$ or} $A$.
\old{correspondingly.} When \new{$A$ is zero (which implies that} $\xi=0$ or $c=b^2$\new{)},
...
\item The choice of parameters $A = - 4\,\eta$; $B = 1 + 2\,\eta$;
  ...
  proposed by \cite{GEO60-05-15501566} \new{for VTI media}, which is the following
...
introduces more degrees of freedom for \old{the} optimal \old{selection of
parameters} \new{fitting}.
...
In the special case of an isotropic \old{$v(z)$} \new{$V(z)$} medium,
...
\new{\cite[]{lyakh,blias0,GEO62-02-06620675,ursin}}.
...
approximation~\ref{eq:mo2} into equations $t(X)=T$ and \old{$t'(X)=P$}
\new{$dt/dX=P$} and solving for $B$ and $C$ produces the explicit
analytical solution
\begin{eqnarray}
  \label{eq:bp}
...
several analytical and numerical models. Using \old{this set of}
\new{these} models, we test the proposed approximation against the
...
\plot{linvel}{width=\textwidth}{Relative \new{absolute} error of different traveltime
...
\plot{linsloth}{width=\textwidth}{Relative \new{absolute} error of different
...
\old{One} \new{Our} next analytical example is a curved reflector under a
constant-velocity overburden. The reflector curvature is one of the
possible causes of non-hyperbolic moveout \old{, as described by}
...
\plot{circle}{width=\textwidth}{Relative \new{absolute} error of different traveltime
...
superior accuracy of equation~\ref{eq:mo2} in \old{a} case of strongly
anisotropic \old{material} \new{media}.
...
approximation parameters, we used equations~\ref{eq:m-1}-\ref{eq:m3}
...
\new{The choice of a proper functional form is important for the
...
\new{A proper selection of the reference ray for equations~\ref{eq:bp}
...
\new{Possible extensions of this work may include nonhyperbolic
...
accuracy. Based on these experiments, we claim that, for \old{all}
\new{many} practical purposes, the proposed approximation \old{is as
good as} \new{can be used in place of} the exact moveout.

\old{Possibly extensions of this work may include nonhyperbolic
approximations for diffraction traveltimes (for use in prestack time
migration) and reflection surfaces (for use in multifocusing or
common-reflection-surface methods) as well as approximations for
anisotropic phase and group velocities in ray tracing and wave
extrapolation.}
...
\new{We would like to thank Tariq Alkhalifah, Emil Blias, Houb Douma, and
Bin Wang for many helpful suggestions that improved the paper.}

\new{This publication is authorized by the Director, Bureau 
of Economic Geology, The University of Texas at Austin.}

\append{LINEAR VELOCITY MODEL}
...
\begin{equation}
  ...
  \label{eq:tlinear}
\end{equation}
where \new{$H$ is the depth of the reflector, and} $r=V(H)/V_0$ \old{being} \new{is}
...
\end{equation}
where \new{$H$ is the depth of the reflector,} $r=V(H)/V_0$ is the
...
\append{REFLECTION FROM A HYPERBOLIC REFLECTOR IN A HOMOGENEOUS VELOCITY MODEL}
\inputdir{XFig}
...
\append{REFLECTION FROM A CIRCULAR REFLECTOR IN A HOMOGENEOUS VELOCITY MODEL}
\inputdir{XFig}
...
equations~\ref{eq:bi}-\ref{eq:ci}, we \old{define} \new{derive} the
