...
\footnotetext[3]{\emph{ChevronTexaco Exploration and Production Technology
    ...
    \ABS{ Spectral factorization is a computational procedure for
      constructing minimum-phase (stable inverse) filters required for
      recursive inverse filtering. We present a novel method of
      spectral factorization. The method iteratively constructs an
...
accelerate Wilson's iteration was suggested by \longcite{laurie}. We have
...
spectrum.  The algorithm, suggested by \longcite{mywilson}, approaches
...
\end{tabular}
\end{center}
\caption{Example convergence of the Wilson-Burg iteration}
\label{tbl:tablerate}
}
...
\cite{galilee}. A remedy, suggested by \longcite{schweikert},
...
in elastic plates \cite{timoshenko}. \longcite{GEO55.03.02930305}
...
\begin{equation}
  ...
  (\nabla^2)^2 f(x,y) = 0\;.
\end{equation}
\longcite{fung} and \longcite{GEO39.01.00390048} derive
...
equation~(\ref{eqn:bi}). \longcite{GEO55.03.02930305} suggest the
...
$\lambda$ is unknown\footnote{\longcite{mitas} derive an analytical Green's
...
\bibliography{MISC,EAEG,SEGCON,GEOTLE,SEGBKS,GEOPHYSICS,sergey,SEG}
