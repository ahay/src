...
\plot{velfig}{width=6.0in,height=3.0in}{.}
{The Marmousi velocity model.}

The Marmousi velocity model (Figure~\FIG{velfig}) results in
...
at an earlier time of 0.6 s (Figures~\FIG{snapdatum}a and \FIG{snapdatum}b)
...
Figures~\FIG{snapdatum}c and \FIG{snapdatum}d are generated by
...
in the shallow part of Figure~\FIG{snapdatum}c, but for the most part
the finite-difference traveltime contour fits the bulk of the acoustic 
energy pretty well. Overall, the contours in Figure~\FIG{snapdatum}
have not pulled away from the wavefield as they have in Figure~\FIG{snap0}.

\activeplot{snap0}{width=6.0in,height=3.0in}{.}
...
\activeplot{snapdatum}{width=6.0in,height=6.0in}{.}
{The result of acoustic wavefield modeling for the Marmousi model overlain
by contours of finite-difference traveltime. Snapshots (a) and (b) are
taken at a time of 0.6 s for the same two source locations depicted
in Figure \protect \FIG{snap0}. Snapshots (c) and (d) 
...
\mhead{LAYER-STRIPPING KIRCHHOFF MIGRATION}
...
This is illustrated by Figures~\FIG{s-g} through~\FIG{datum}.

In shot-profile migration, shots and geophones are
alternately downward continued through each depth level.
Figure~\FIG{s-g} illustrates this for one line of shots or geophones.  
...
illustrated in Figure~\FIG{kirch}. There is only one path linking
...
The layer-stripping method is illustrated in Figure~\FIG{datum}.
...
in Figure~\FIG{kirch}, first-arrival traveltimes are more likely to 
be valid.

\activesideplot{s-g}{width=3.0in}{.}
{Propagation paths from the surface to an image point for shot 
profile migration.}

\activesideplot{kirch}{width=3.0in}{.}
{Propagation paths from the surface to an image point for Kirchhoff downward
continuation.}

\activesideplot{datum}{width=3.0in}{.}
{Propagation paths from the surface to a depth point for layer-stripping
Kirchhoff downward continuation.}

\mhead{MARMOUSI EXAMPLE}

Figure~\FIG{eikmarmcggmig} is the result of
...
Figure~\FIG{mergecggall1500} is generated by downward continuing the data
...
in an even crisper
image of the anticline and the target (Figure~\FIG{mergecgg1500}).
...
\activeplot{eikmarmcggmig}{height=3.0in,width=6.0in}{.}
{Standard Kirchhoff migration using eikonal traveltimes.}

\activeplot{mergecggall1500}{height=3.0in,width=6.0in}{.}
...
\activeplot{mergecgg1500}{height=3.0in,width=6.0in}{.}
{Migrated image using traveltimes calculated from the surface, and 
traveltimes calculated from a depth of 1500 m.
The lower part of the image was obtained by migrating data which
was redatumed to a depth of 1500 m in three steps of 500 m each.}

In Figure~\FIG{targref}, I compare the images in the vicinity
...
\activeplot{targref}{height=6.0in,width=6.0in}{.}
...
\reference{Audebert, F., Biondi, B., Lumley, D., Nichols, D., Rekdal, T., and
	Urdaneta, H., 1994b,
	Marmousi traveltime computation and imaging comparisons,
	SEP--{\bf 80}, 47--66}

\reference{Berryhill, J. R., 1979,
	Wave equation datuming: Geophysics, {\bf 44}, 1329--1344.}

\reference{Berryhill, J. R., 1984,
	Wave equation datuming before stack (short note):
	Geophysics, {\bf 49}, 2064--2067.}

\reference{Bevc, D., 1993,
	Data parallel wave equation datuming: SEP--{\bf 77}, 131--140.}

\reference{Geoltrain, S. and Brac, J., 1993,
	Can we image complex structure with first-arrival traveltime?,
	Geophysics {\bf 58}, 564--575}

\reference{Gray, S. H., and May, W. P., 1993,
	Kirchhoff migration using eikonal equation traveltimes,
	Geophysics {\bf 59}, 810--817}

\reference{Nichols, D., 1994,
	Imaging complex structures using band limited Green's functions,
	Ph.D. Thesis, Stanford University.}

\reference{van Trier, J., and Symes, W. W., 1991,
	Upwind finite-difference calculation of traveltimes:
	Geophysics, {\bf 56}, 812--821.}

\reference{Versteeg, R., and Grau, G., 1991,
	The Marmousi Experience: Proceedings of the 1990 EAEG workshop on
	practical aspects of seismic data inversion,
	52nd EAEG Meeting, Eur. Assoc. Expl. Geophys.}
