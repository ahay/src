...
They include Julian and Gubbins'~\shortcite{Julian} ray bending, 
Dines and Lytle's~\shortcite{Dines}
shooting rays and \v{C}erven\'{y}'s~\shortcite{Cervenys} paraxial
extrapolation. More recently, several new methods have appeared
and are enjoying an increasing popularity.
They include Vidale~\shortcite{GEO55.05.05210526},
Podvin and Lecomte~\shortcite{Podvin},
van Trier and Symes~\shortcite{GEO56.06.08120821} finite differences
and Moser's~\shortcite{GEO56.01.00590067} shortest path rays.
...
In Lomax~\shortcite{Lomax}, the author approximates 
...
First, Lomax~\shortcite{Lomax} points out, ``it 
is the wavelength
...
\passivesideplot {lomax1}{width=3.in}{.}{Waveray wavepath and
wavelength-dependent velocity smoothing at point ${\bf \vec{x}}_{\nu}$
Adapted from Lomax (1994). % \shortcite{Lomax}.
}

\shead{Waveray implementation}
...
location (see Figure~\ref{fig:lomax1}). Equation~\EQN{velavg} expresses the
...
The discrete representation of equation~\EQN{velavg} is given
by equation~\EQN{dvelavg}:
%
\begin{equation}
 \overline{v}\, (\vec{\bf x}_{\nu},{\bf T\/}) = \frac{
...
wavefronts
are given by equations~\EQN{dpost0} and \EQN{dpost}.
%
\begin{equation}
 \vec{\bf x}_{\nu} = \vec{\bf x}_{\nu}^{\, 0}
\EQNLABEL{dpost0}
...
These two equations are the discrete version of equation~\EQN{post}, but,
...
\passivesideplot {lomax2}{width=3.in}{.}{Waveray wavepath calculation.
...
and $\gamma_{max} = 1.5$, based on Lomax's~\shortcite{Lomax}
...
\passiveplot {lomaxsbs}{}{.}{Left frame shows a fan of high frequency 
...
The main idea behind the ``NORSAR'' method \shortcite{Vinje}
...
and the ray's angle of direction (see Vinje et. al. \shortcite{Vinje}).
...
\passivesideplot {norsar1}{}{.}{New wavefronts (dashed lines)  are
...
Adapted from Vinje et. al. (1993). %  \shortcite(Vinje)
}
...
\passivesideplot {norsar2}{}{.}{The new wavefront crosses itself.
...
\passivesideplot {norsar3}{}{.}{Traveltimes and amplitudes are found
...
A final point on Norsar's method is, as said by Vinje et. al.
\shortcite{Vinje}:
...
\passivesideplot {norsar4}{}{.}{Amplitudes are computed from
...
\activeplot {gauss-wv}{width=6.in}{.}{The ``NORSAR'' or
...
\shead{Travel-times and amplitudes in the Marmousi model}
...
\activeplot {marm80-time}{height=3.5in}{.}{Traveltime contours
computed from the combined method, overlaid on a section of 
the Marmousi model.
A frequency of 80~Hz is used.
}

\activeplot {marm10-time}{height=3.5in}{.}{As in 
the previous Figure, but for a frequency of 10 Hz.
}

\activeplot {marm80-ampl}{height=3.5in}{.}{Amplitude maps from the
combined method at a frequency of 80~Hz.
}

\activeplot {marm10-ampl}{height=3.5in}{.}{As in 
the previous Figure, but for a frequency of 10 Hz.
}
%\newpage
