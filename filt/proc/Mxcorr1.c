...
{
    int n, nw, w, iw, i0, maxshift, i, order;
    float dt,h, eps, lam;
    bool verb;
    ...
    xcorr_init (w, maxshift);
    str = stretch2_init (n,1.,1.,nw,eps,lam);

    sf_read (dat,sizeof(float),n,in);
    sf_read (dat2,sizeof(float),n,other);
  
    for (iw=0; iw < nw; iw++) {
	i0 = window1_apply(iw,dat ,true,true,win);
	i0 = window1_apply(iw,dat2,true,true,win2);
	coord[iw] = i0 + 0.5*(w+1.);
	shift[iw] = xcorr (win, win2);
