...
static float **p, **tmp, *trace1, *trace2, *tt, amp;
...
/*< initialize >*/
{
    ...
    predict_init(n1,n2,eps);
}

void pwdsl_set(float** p1 /* slope [m2][m1] */)
/*< set slopes >*/
{
    p = p1;
}
...
static void forw(int i2, const float* t1, float* t2)
{
	...
	predict_step(true,true,tt,p[i2-rect]);
	for (i1=0; i1 < n1; i1++) {
	    t2[i1] += tt[i1];
	}
    }
...
static void back(int i2, float* t1, const float* t2)
{
	...
	predict_step(false,true,tt,p[i2-rect]);
	for (i1=0; i1 < n1; i1++) {
	    t1[i1] += tt[i1];
	}
    }
