...
#else
		xx[it-itau+lag-1] = sf_cadd(xx[it-itau+lag-1],sf_cmul(yy[it],conjf(filter[it+n1*itau])));
#endif
	    } else {
#ifdef SF_HAS_COMPLEX_H
		yy[it] += xx[it-itau+lag-1]*filter[it+n1*itau];
#else  
		yy[it] = sf_cadd(yy[it],sf_cmul(xx[it-itau+lag-1],filter[it+n1*itau]));
#endif
	    } 
	}
    }
...
#else
		xx[it-na2+itau] = sf_cadd(xx[it-na2+itau],sf_cmul(yy[it],conjf(filter[it+n1*itau])));
#endif
	    } else {
#ifdef SF_HAS_COMPLEX_H
		yy[it] += xx[it-na2+itau]*filter[it+n1*itau];
#else  
		yy[it] = sf_cadd(yy[it],sf_cmul(xx[it-na2+itau],filter[it+n1*itau]));
#endif
	    } 
	}
    }
...
#else
		xx[it+itau] = sf_cadd(xx[it+itau],sf_cmul(yy[it],conjf(filter[it+n1*itau])));
#endif
	    } else {
#ifdef SF_HAS_COMPLEX_H
		yy[it] += xx[it+itau]*filter[it+n1*itau];
#else  
		yy[it] = sf_cadd(yy[it],sf_cmul(xx[it+itau],filter[it+n1*itau]));
#endif
	    } 
	}
    }
