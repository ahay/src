...
int main(int argc, char* argv[])
{
    ...
    float **scan, **weight, *pick, *ampl, *pick2, o2, d2, an, asum, a, ct, vel0, eps;
    bool smooth;
    char key[6], *label;
    sf_file scn, pik;
    ...
    /* if apply smoothing */
    if ( !sf_getfloat ("eps", &eps) ) eps = 1e-6;
    /* verticality measure */
...
///////////////////////////////////////////////////
	...
	/* normalize amplitudes */
		...
		for(i=0; i < nm; i++) {
		    ampl[i] /= asum;
			const float a2 = ampl[i] * ampl[i];
			const float temp = sqrt (a2 + eps * eps);			
			d[i] = o2 + pick[i] * d2 - vel0;
