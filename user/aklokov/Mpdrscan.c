...
#pragma omp parallel for
#endif					
		for (int iz = 0; iz < zoSize; ++iz) {
			const float sampleSq = pow (zo [iz], 2);
			const float sqSample = zoSq[iz];
			const float curSemb = sqSample ? sampleSq / ( count[iz] * sqSample ) : 0.f;
			semb[iz] = curSemb;
		}
