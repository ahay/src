...
CurveDefinerDipOffset3D::CurveDefinerDipOffset3D () {
...
void CurveDefinerDipOffset3D::getEscapePoint (const float curY, const float curX, const float curZeroTime, 
...
// at the moments it is working for zero offset only
float CurveDefinerDipOffset3D::getEscapeTime (const float dy, const float dx, const float t0, const float migVel, float& p) {
	
	const float z2 = pow (t0 * migVel * 0.5, 2);
	const float dl = sqrt (dx * dx + dy * dy);

	const float distToRec = sqrt (z2 + dl * dl);	
	const float distToSrc = distToRec;

	float time = (distToRec + distToSrc) / migVel;

	p = distToRec ? fabs ( 2 * dl / (distToRec * migVel) ) : 0.f;
