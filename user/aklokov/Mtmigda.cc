...
void prepareVelocityTrace (int taskX, int taskY, float* velTrace) {
	...
	memset (velTrace, 0.f, velZNum * sizeof (float));   
...
// OUTPUT FILES
	...
	// data
	// time - in ms
	corUnit = "ms"; unit = sf_histstring (dataFile, "unit1");
	if ( strcmp (corUnit, unit) ) { dp.zStep *= 1000; dp.zStart *= 1000; }
	// inline - in m
	corUnit = "m"; unit = sf_histstring (dataFile, "unit2");
	if ( strcmp (corUnit, unit) ) { dp.xStep *= 1000; dp.xStart *= 1000; }
	// crossline - in m
	corUnit = "m"; unit = sf_histstring (dataFile, "unit3");
