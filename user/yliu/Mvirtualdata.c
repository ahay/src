/* Construction of virtual seismic data. */
/*
	...
	for (iw=0; iw < nw; iw++) { /* loop over frequency */
		...
		for (i2=0; i2 < n2; i2++) {
			...
			for (m=(-1*n1+1); m < n1; m++) {
				...
				if (s1 >= 0 && s1 < n2 && x1 >= 0 && x1 < n1 && 
				    ...
				    if(NULL != dif) {
					mm[i2*n1+i1] += conjf(ref[s2*n1+x2])*dd[s1*n1+x1];
				    } else {
					mm[i2*n1+i1] += conjf(dd[s2*n1+x2])*dd[s1*n1+x1];
				    }
#else
				    if(NULL != dif) {
					mm[i2*n1+i1] = sf_cadd(mm[i2*n1+i1],sf_cmul(sf_conjf(ref[s2*n1+x2]),dd[s1*n1+x1]));
				    } else {
				        mm[i2*n1+i1] = sf_cadd(mm[i2*n1+i1],sf_cmul(sf_conjf(dd[s2*n1+x2]),dd[s1*n1+x1]));
				    }
#endif
				} else {
				    mm[i2*n1+i1] = sf_cmplx(0.,0.);
		...
		for (i2=0; i2 < n2; i2++) {
			...
			for (m=0; m < n1; m++) {
			    ...
			    s2 = m + i2 - n1/2;
			    if (stack) {
				if (s1 >= 0 && s1 < n2 && x1 >= n1/2 && x1 < n1 && 
				    s2 >= x1 && s2 < n2 && x2 >= 0 && x2 < n1 ) {
#ifdef SF_HAS_COMPLEX_H
				    if(NULL != dif) {			
					mm[i2*n1+i1] += dd[s1*n1+x1]*conjf(ref[s2*n1+x2]);
				    } else {
					mm[i2*n1+i1] += dd[s1*n1+x1]*conjf(dd[s2*n1+x2]);
				    }
#else
				    if(NULL != dif) {	
					mm[i2*n1+i1] = sf_cadd(mm[i2*n1+i1],sf_cmul(dd[s1*n1+x1],sf_conjf(ref[s2*n1+x2])));
				    } else {
					mm[i2*n1+i1] = sf_cadd(mm[i2*n1+i1],sf_cmul(dd[s1*n1+x1],sf_conjf(dd[s2*n1+x2])));
				    }
#endif

                               } if (s1 >= 0 && s1 < n2 && x1 >= 0 && x1 < n1/2 && 
				    s2 >= 0 && s2 < x1 && x2 >= 0 && x2 < n1 ) {
#ifdef SF_HAS_COMPLEX_H
				    if(NULL != dif) {			
					mm[i2*n1+i1] += dd[s1*n1+x1]*conjf(ref[s2*n1+x2]);
				    } else {
