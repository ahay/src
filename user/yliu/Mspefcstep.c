/* Streaming prediction-error filter with constant step. */
/*
...
int main(int argc, char *argv[]) 
{
    ...
    if (!sf_getint("ngp", &ngp)) ngp = 0;
    /* constant step */

    nw  = na + ngp;
    if (verb) sf_warning("nw=%d ", nw);
    /* assign filter size and gap size */
    ...
    /* deconvolution using SPEF with constant gap */
...
void gspef_loop(float *inp, float *res) 
{
	...
	for (i1 = 0; i1 < n1; i1++) {
	    ...
	    gspef_pick(inp, tmp, i2, i1);
	    /* SPEF method for calculating data points */
