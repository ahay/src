...
int main(int argc, char* argv[])
{
    ...
    fcurr = sf_floatalloc3(nt,nx,nz);
    curr = sf_complexalloc3(nt,nx,nz);
   ...
   dh *= 2*SF_PI;
	...
	for (iz=0; iz < nz; iz++) {
	    for (ix=0; ix < nx; ix++) {
		fcurr[iz][ix][0] = img[ix][iz];
	    }
	}
	cosft3(false,nt,nx,nz,fcurr);

	for (iz=0; iz < nz; iz++) {
	    for (ix=0; ix < nx; ix++) {
		for (it=0; it < nt; it++) {
		    curr[iz][ix][it] = sf_cmplx(fcurr[iz][ix][it],0.);
		}
	    }
	}
	...
	if (NULL != snaps && 0 == ih%snap) {
		...
		for (ix=0; ix < nx; ix++) {
		    for (it=0; it < nt; it++) {
			fcurr[iz][ix][it] = crealf(curr[iz][ix][it]);
		    }
		}
	    }
