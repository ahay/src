...
int main(int argc, char* argv[])
{
   ...
   * ***************************************************************************/
   ...
   ElasticSepSV.put("o2",fx/1000);
    ...
    //     coeffecient for 1st order finite difference
    float* coeff_1d=new float[M];
    float* coeff_1dx=new float[M];
    float* coeff_1dz=new float[M];
    switch(M)
    {
        case 1:
            coeff_1d[0]=1;
        ...
        case 2:
            coeff_1d[0]=1.125;
            coeff_1d[1]=-0.04166667;
            break;
        case 3:
            coeff_1d[0]=1.171875;
            coeff_1d[1]=-0.06510416;
            coeff_1d[2]=0.0046875;
            break;
        case 4:
            coeff_1d[0]=1.196289;
            coeff_1d[1]=-0.0797526;
            coeff_1d[2]=0.009570313;
            coeff_1d[3]=-0.0006975447;
            break;
        case 5:
            coeff_1d[0]=1.211243;
            coeff_1d[1]=-0.08972168;
            coeff_1d[2]=0.01384277;
            coeff_1d[3]=-0.00176566;
            coeff_1d[4]=0.0001186795;
            break;
        default:
            cout<<"wrong order of staggered grid!";
    }
    for(ix=0;ix<M;ix++)
    {
        coeff_1dx[ix]=coeff_1d[ix]/dx;
        coeff_1dz[ix]=coeff_1d[ix]/dz;
    }
	...
	float **rxx=sf_floatalloc2(nzpad,nxpad);
    float **rxz=sf_floatalloc2(nzpad,nxpad);
    float **rzz=sf_floatalloc2(nzpad,nxpad);

    zero2float(rxx, nzpad, nxpad);
    zero2float(rxz, nzpad, nxpad);
    zero2float(rzz, nzpad, nxpad);
    ...
    float **vpp, **vss, **epp, **dee, **thee, **rho;
    ...
    float **mc11 =sf_floatalloc2(nz,nx);
    float **mc13 =sf_floatalloc2(nz,nx);
    float **mc33 =sf_floatalloc2(nz,nx);
    float **mc44 =sf_floatalloc2(nz,nx);
    ...
    for(ix=0;ix<nxv;ix++){
        for(iz=0;iz<nzv;iz++){
            mc11[ix][iz]=vpp[ix][iz]*vpp[ix][iz]*(1+2*epp[ix][iz])*rho[ix][iz];
            mc33[ix][iz]=vpp[ix][iz]*vpp[ix][iz]*rho[ix][iz];
            mc44[ix][iz]=vss[ix][iz]*vss[ix][iz]*rho[ix][iz];
            mc13[ix][iz]=rho[ix][iz]*(sqrt(2*vpp[ix][iz]*vpp[ix][iz]*(vpp[ix][iz]*vpp[ix][iz]-vss[ix][iz]*vss[ix][iz])*dee[ix][iz]+pow(vpp[ix][iz]*vpp[ix][iz]-vss[ix][iz]*vss[ix][iz],2))-vss[ix][iz]*vss[ix][iz]);
        }
    }
	...
	if(it%50==0)
        ...
        */ 
        ...
        fwpvtiDisSts(p1, p2, p3, q1, q2, q3, rxx, rzz, rxz, coeff_1dx, coeff_1dz, dx, dz, dt, nx, nz, mc11, mc33, mc44, mc13, mrho);
