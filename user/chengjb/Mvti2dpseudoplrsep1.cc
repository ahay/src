   ...
   * ***************************************************************************/
   ...
   if(ireconstruct==1)
   {
      ...
      for(int im=0;im<nxz;im++){
        ...
        for(j=0;j<nz;j++)
        {
           float err=w[im][k]-dev[i][j];
           //if(fabs(err/dev[i][j])>0.01) sf_warning("Relative Error > 1%:  im=%d i=%d j=%d ikx=%d ikz=%d w=%f a=%f",im,i,j,k/nkz,k%nkz,w[im][k], dev[i][j]);
           sum += err*err;
           //if(im==nxz/2) wx[k] = err*1000000;
           if(im==nxz/2) wx[k] = err;
           k++;
         }
   ...
   if(ireconstruct==1)
   {
      ...
      for(int im=0;im<nxz;im++){
        ...
        //calculate projection deviation operators directly for Z-component
        devzp2dvti(dev, nx, nz, im);
        ...
        for(j=0;j<nz;j++)
        {
           ...
           //if(fabs(err/dev[i][j])>0.01) sf_warning("Relative Error > 1%:  im=%d i=%d j=%d ikx=%d ikz=%d w=%f a=%f",im,i,j,k/nkz,k%nkz,w[im][k], dev[i][j]);
           sum += err*err;
           //if(im==nxz/2)  wz[k] = err*1000000;
           if(im==nxz/2)  wz[k] = err;
           k++;
         }
