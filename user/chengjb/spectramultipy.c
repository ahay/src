...
*/
       ...
       xf=fftw_plan_dft_3d(ny,nx,nz, (fftw_complex *) xin, (fftw_complex *) xout,
                            FFTW_FORWARD,FFTW_ESTIMATE);

       yr=fftw_plan_dft_3d(ny,nx,nz, (fftw_complex *) xin, (fftw_complex *) xout,
                            FFTW_BACKWARD,FFTW_ESTIMATE);
       ...
       for(ix=0;ix<nx;ix++)
       for(iz=0;iz<nz;iz++)
            d[iy][ix][iz] = xout[i][0]/m;

       fftw_destroy_plan(xf);
       fftw_destroy_plan(yr);
