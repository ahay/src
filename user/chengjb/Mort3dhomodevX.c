...
int main(int  argc,char **argv)
{
   ...
   if(nx%2!=0) /* guarent axis symmetry, both good for nkx is odd or even */
     dkx=2*PI/dx/nx;
   else
     dkx=2*PI/dx/(nx-1);

   if(nz%2!=0) /* guarent axis symmetry, both good for nkz is odd or even */
     dkz=2*PI/dz/nz;
   else
     dkz=2*PI/dz/(nz-1);

   if(nz%2!=0) /* guarent axis symmetry, both good for nky is odd or even */
     dky=2*PI/dy/ny;
   else
     dky=2*PI/dy/(ny-1);
    ...
    for( j=-hny; j<=hny; j++){
	  ...
	  for( i=-hnx; i<=hnx; i++ ){
		...
		for( k=-hnz; k<=hnz; k++)
		{
			...
			//apvy[jhny][ihnx][khnz]=siny;
			//apvx[jhny][ihnx][khnz]=sinx;
			//apvz[jhny][ihnx][khnz]=sinz;
			//apvy[jhny][ihnx][khnz]=upy;
			//apvx[jhny][ihnx][khnz]=upx;
			//apvz[jhny][ihnx][khnz]=upz;
			
			apvy[jhny][ihnx][khnz]=(float)(upy/siny);
			apvx[jhny][ihnx][khnz]=(float)(upx/sinx);
			apvz[jhny][ihnx][khnz]=(float)(upz/sinz);

                        //if(apvz[jhny][ihnx][khnz]>1.5) apvz[jhny][ihnx][khnz]=1.5;
                        //if(apvz[jhny][ihnx][khnz]<-1.5) apvz[jhny][ihnx][khnz]=-1.5;

                        /*
			if(j!=0)
				apvy[jhny][ihnx][khnz]=upy/siny;
                        else
				apvy[jhny][ihnx][khnz]=1.0;

                        if(i!=0)
				apvx[jhny][ihnx][khnz]=upx/sinx;
                        else
				apvx[jhny][ihnx][khnz]=1.0;

                        if(k!=0)
				apvz[jhny][ihnx][khnz]=upz/sinz;
                        else
				apvz[jhny][ihnx][khnz]=1.0;
                        */
		}// k loop
