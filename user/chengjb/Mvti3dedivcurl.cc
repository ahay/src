...
int main(int argc, char* argv[])
{
   ...
   float *rkx, *rky, *rkz;
   rkx=sf_floatalloc(nxyz);
   rky=sf_floatalloc(nxyz);
   rkz=sf_floatalloc(nxyz);
   ...
   ikxikyikz(ijkx, ijky, ijkz, nkx, nky, nkz);
    ...
    sepdiv3d(rkx,p,ijkx,ijky,ijkz,nx,ny,nz,nxyz,nk,iflag);
    for(k=0;k<nxyz;k++) pp[k] = p[k];

    for(k=0;k<nxyz;k++)
		p[k] = py[k];
    sepdiv3d(rky,p,ijkx,ijky,ijkz,nx,ny,nz,nxyz,nk,iflag);
    for(k=0;k<nxyz;k++) pp[k] += p[k];

    for(k=0;k<nxyz;k++)
		p[k] = pz[k];
    sepdiv3d(rkz,p,ijkx,ijky,ijkz,nx,ny,nz,nxyz,nk,iflag);
    for(k=0;k<nxyz;k++) pp[k] += p[k];
   ...
   float *rksx, *rksy, *rksz, ss;
   rksx=sf_floatalloc(nxyz);
   rksy=sf_floatalloc(nxyz);
   rksz=sf_floatalloc(nxyz);

    // separate SH wave  
    iflag=0;
    sf_warning("separate SH-wave based on div-curl-operation."); 
    for(k=0;k<nxyz;k++){
		float r = rky[k]*rky[k]+rkx[k]*rkx[k];
		if(r==0)
		   ss = 0.0;
		else
    ...
    sepdiv3d(rksy,p,ijkx,ijky,ijkz,nx,ny,nz,nxyz,nk,iflag);
    for(k=0;k<nxyz;k++) pp[k] = p[k];

    for(k=0;k<nxyz;k++) p[k] = py[k];
    sepdiv3d(rksx,p,ijkx,ijky,ijkz,nx,ny,nz,nxyz,nk,iflag);
    for(k=0;k<nxyz;k++) pp[k] += p[k];
    ...
    sepdiv3d(rksx,px,ijkx,ijky,ijkz,nx,ny,nz,nxyz,nk,iflag);
    for(k=0;k<nxyz;k++) pp[k] = px[k];

    sepdiv3d(rksy,py,ijkx,ijky,ijkz,nx,ny,nz,nxyz,nk,iflag);
    for(k=0;k<nxyz;k++) pp[k] += py[k];

    sepdiv3d(rksz,pz,ijkx,ijky,ijkz,nx,ny,nz,nxyz,nk,iflag);
