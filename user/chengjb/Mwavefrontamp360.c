...
int main(int argc, char* argv[])
{
		...
		amp1=sf_floatalloc(naa);
		amp2=sf_floatalloc(naa);
		...
		for(i=0;i<na;i++){
			...
			for(j=0;j<nb/2;j++){
				...
				if(ix>=0&&ix<nx&&iz>=0&&iz<nz/2+1){
				  sn1[i][nb/2-j]=snap1[ix][iz];
				  sn2[i][nb/2-j]=snap2[ix][iz];
				}
			}
		}
		for(i=0;i<na;i++){
...
*/
		...
		for(i=0;i<na;i++){
			...
			for(j=0;j<nb/2;j++){
				...
				ix=x/dx;
				iz=z/dz;
				if(ix>=0&&ix<nx&&iz>=nz/2-1&&iz<nz){
				  sn1[i][j+nb/2]=snap1[ix][iz];
				  sn2[i][j+nb/2]=snap2[ix][iz];
				}
			}
	...
	/*	
		...
		*/
        ...
        sf_floatwrite(amp1, naa, Fo5);
        sf_floatwrite(amp2, naa, Fo6);
        free(*snap1);
        free(*snap2);
        free(*sn1);
        free(*sn2);
