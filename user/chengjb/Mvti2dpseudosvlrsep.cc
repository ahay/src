/* 2-D two-components wavefield modeling based on pseudo-pure mode SV-wave equation
 * and P-SV separation using low-rank symbol approximation in 2D VTI media.

Authors: Jiubing Cheng & Wei Kang(Tongji University) and Sergey Fomel (The University of Texas at Austin) 
...
int main(int argc, char* argv[])
{
   ...
   * ***************************************************************************/
   ...
   PseudoPureSepSVz.put("o2",fx/1000);
    ...
    int iflag=1;    // iflag must be 1 for deviation correction of pseudo-pure-mode wavefield
	...
	q2[ixms][izms]+=Ricker(t, f0, t0, A);
        ...
        /* fwpvtipseudop: forward-propagating in VTI media with pseudo-pure-mode SV-wave equation */
        ...
        if(it==ns-1)
	{
	      ...
	      for(i=0;i<nx;i++)
		   ...
		   for(j=0;j<nz;j++)
		   {
              ...
              /* correct projection error for accurate separate qSV wave in spatial domain */
...
static int samplezs(vector<int>& rs, vector<int>& cs, DblNumMat& resz)
{
    ...
    for(int a=0; a<nr; a++) 
    {
        ...
        for(int b=0; b<nc; b++)
        {
            ...
            if(s!=0.0){
               resz(a,b) = -u2/s;
               //sf_warning("u2=%f s=%f",u2,s);
               if(u2*s>0.0) { sf_warning("ooooo"); exit(0);}
            }else // in fact, not happen after we move sample from zero
               resz(a,b) = 1.0;
            getchar();
         }// b loop
