/* Extended time-domain acoustic FD modeling */
/*
...
/*    sf_warning("vel=%f,ww=%f,rr=%f,nh=%d,oh=%d",vv[15][10][10],ww[100],rr[74][74],nh,oh); */
	...
	/* change velocity multiply to operator*/
        for (ih=0; ih < nh; ih++) {
            h = ih + oh;
	    for (ix=0; ix<nx; ix++) {
                if (ix-2*h>=0 && ix-2*h<nx) {
	           for (iz=0; iz<nz; iz++) {
     	 	       uc[ix][iz] += ud[ix-2*h][iz]*vv[ih][ix-2*h][iz]*vv[ih][ix-2*h][iz];
                   }
                }
	    }
	}
