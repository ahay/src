/* 2D acoustic time-domain FD modeling  for source perturbation - 2nd order approximation for complex v*/
/*
...
#define DX2(a,ix,iz,s) (CB*(a[ix+2][iz  ] + a[ix-2][iz  ]) +  \
                       CA*(a[ix+1][iz  ] + a[ix-1][iz  ])  + \
                        C0*a[ix][iz])*s*s
#define DX22(a,ix,iz,s) ((a[ix+1][iz  ] + a[ix-1][iz  ])  - \
                        2.0*a[ix][iz])*s*s
...
int main(int argc, char* argv[])
{
    ...
    float **dv2=NULL;           /* d2wdx2 term */

    float **um,**uo,**up,**ua,**ut; /* wavefield: um = U @ t-1; uo = U @ t; up = U @ t+1 */
    float **pm,**po,**pp,**pa,**pt,**ptemp; /* perturbation wavefield: pm = P @ t-1; po = P @ t; pp = P @ t+1 */
    float **mm,**mo,**mp,**ma,**mt; /* perturbation wavefield 2nd: mm = M @ t-1; mo = M @ t; mp = M @ t+1 */
	...
	mc=sf_floatalloc2(sf_n(acz),sf_n(acx));
    ...
    dv2  =sf_floatalloc2(fdm->nzpad,fdm->nxpad); 
    ...
    for(ix=NOP; ix<fdm->nxpad-NOP; ix++)
	for(iz=0; iz<fdm->nzpad; iz++) {
	  dv[ix][iz] = vp[ix][iz] *vp[ix][iz];
	  dv2[ix][iz] = 2*(DX2(vp,ix,iz,idx)*vp[ix][iz]+
			   DX(vp,ix,iz,idx)*DX(vp,ix,iz,idx))/(vp[ix][iz]*vp[ix][iz]);
	};
    ...
    ptemp=sf_floatalloc2(fdm->nzpad,fdm->nxpad);
    mm=sf_floatalloc2(fdm->nzpad,fdm->nxpad);
    mo=sf_floatalloc2(fdm->nzpad,fdm->nxpad);
    mp=sf_floatalloc2(fdm->nzpad,fdm->nxpad);
    ma=sf_floatalloc2(fdm->nzpad,fdm->nxpad);
...
#pragma omp parallel for				\
	...
	for    (ix=NOP; ix<fdm->nxpad-NOP; ix++) {
		...
		ptemp[ix][iz] = pa[ix][iz];
		pa[ix][iz] -= DX(up,ix,iz,idx)/dv[ix][iz]-DX(ua,ix,iz,idx);
	    }
	}
...
#pragma omp parallel for	    \
	...
	/*Perturbation part 2nd------------------------------------------------------------------------------------------------------------*/
	#ifdef _OPENMP
#pragma omp parallel for				\
    schedule(dynamic,fdm->ompchunk)			\
    private(ix,iz)					\
    shared(fdm,ma,mo,co,cax,caz,cbx,cbz,idx,idz)
#endif
	for    (ix=NOP; ix<fdm->nxpad-NOP; ix++) {
	    for(iz=NOP; iz<fdm->nzpad-NOP; iz++) {
		...
		ma[ix][iz] = 
		    co * mo[ix  ][iz  ] + 
		    cax*(mo[ix-1][iz  ] + mo[ix+1][iz  ]) +
		    cbx*(mo[ix-2][iz  ] + mo[ix+2][iz  ]) +
		    caz*(mo[ix  ][iz-1] + mo[ix  ][iz+1]) +
		    cbz*(mo[ix  ][iz-2] + mo[ix  ][iz+2]);
		
		/* density term */
		/*		ma[ix][iz] -= (
		    DZ(mo,ix,iz,idz) * roz[ix][iz] +
		    DX(mo,ix,iz,idx) * rox[ix][iz] );*/

		/*adding the perturbation source function*/
		ma[ix][iz] -= 2*DX(pp,ix,iz,idx)/dv[ix][iz]-2*DX(ptemp,ix,iz,idx)+DX2(up,ix,iz,idx)/dv[ix][iz]-DX2(ua,ix,iz,idx);
	    }
	}
...
#pragma omp parallel for	    \
	...
	for    (ix=0; ix<fdm->nxpad; ix++) {
		...
		mp[ix][iz] = 2*mo[ix][iz] 
		    -          mm[ix][iz] 
		    +          ma[ix][iz] * vt[ix][iz];
		ua[ix][iz] = uo[ix][iz]-po[ix][iz]*dl+0.5*mp[ix][iz]*dl*dl;
		/*tmp = mp[ix][iz]*dl+2*po[ix][iz];
		tmp = SGN(tmp)*MAX(ABS(tmp),0.00000000001);
		ua[ix][iz] = uo[ix][iz]-2*po[ix][iz]*po[ix][iz]/tmp;*/
	    }
	}
	/* circulate wavefield arrays */
	mt=mm;
	...
	if(dabc) {
	    /* one-way abc apply */
	    abcone2d_apply(mo,mm,NOP,abc,fdm);
	    sponge2d_apply(mm,spo,fdm);
	    sponge2d_apply(mo,spo,fdm);
	    sponge2d_apply(mp,spo,fdm);
	}
	/*Perturbation part 2nd end ------------------------------------------------------------------------------------------------------------*/
