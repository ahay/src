/* 2D acoustic time-domain FD modeling  for source perturbation- 2nd order test*/
/*
...
#pragma omp parallel for				\
	...
	for    (ix=NOP; ix<fdm->nxpad-NOP; ix++) {
		...
		/*adding the perturbation source function*/
		ma[ix][iz] -= 2*dv[ixx][iz]*pa[ix][iz]-dv2[ixx][iz]*ua[ix][iz];
	    }
	}
