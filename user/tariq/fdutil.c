...
{ 
    fdm2ds fdm;
    fdm = (fdm2ds) sf_alloc(1,sizeof(*fdm));
...
#pragma omp parallel for schedule(dynamic,1) private(ia) shared(ca,dd,uu)
    ...
    for (ia=0;ia<ca->n;ia++) {
       for (ib=0;ib<ca->n;ib++) {
	dd[ia][ib] =
	    uu[ ca->jx[ia]  ][ ca->jx[ib]  ] * ca->w00[ia] +
	    uu[ ca->jx[ia]  ][ ca->jx[ib]+1] * ca->w01[ia] +
	    uu[ ca->jx[ia]+1][ ca->jx[ib]  ] * ca->w10[ia] +
	    uu[ ca->jx[ia]+1][ ca->jx[ib]+1] * ca->w11[ia];
       }
    }
}  
...
/*< init 3D ABC >*/
{
    ...
    abc = (abcone2ds) sf_alloc(1,sizeof(*abc));
    ...
    for (ix=0;ix<fdm->nxpad;ix++) {
      d = vv[ix][           nop  ] *dt/fdm->dz; abc->bzl[0][ix] = (1-d)/(1+d);
      d = vv[ix][fdm->nzpad-nop-1] *dt/fdm->dz; abc->bzh[0][ix] = (1-d)/(1+d);
    }


    for (iz=0;iz<fdm->nzpad;iz++) {
      d = vv[           nop  ][iz] *dt/fdm->dx; abc->bxl[0][iz] = (1-d)/(1+d);
      d = vv[fdm->nxpad-nop-1][iz] *dt/fdm->dx; abc->bxh[0][iz] = (1-d)/(1+d);
    }
