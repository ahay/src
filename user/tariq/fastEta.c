...
/*< Run fast marching eikonal solver >*/
{
    float xs[3], ***tx, ***ty, ***tz, ***dD, ***dw, *pt, *ptt, *ttime, *dtime, *ddtime;
    float tmp;
	...
	for(i2 = 0; i2 < n2; i2++){
	    for(i1 = 0; i1 < n1; i1++){
		i = AT(i1,i2,i3);
		dw[i3][i2][i1] = -(1-v[i]*tz[i3][i2][i1]*tz[i3][i2][i1])*(1/v[i]-tz[i3][i2][i1]*tz[i3][i2][i1]);
	    }
	}
    }
	...
	/(sqrt(((xs[0]-i1)*(xs[0]-i1)*d1*d1+(xs[1]-i2)*(xs[1]-i2)*d2*d2+(xs[2]-i3)*(xs[2]-i3)*d3*d3)*v[i])*((xs[0]-i1)*(xs[0]-i1)*d1*d1+(xs[1]-i2)*
													    (xs[1]-i2)*d2*d2+(xs[2]-i3)*(xs[2]-i3)*d3*d3)+0.00001);
    ttime[i] = time[i];
    sf_warning("time=%f dtime=%f eta=%f",time[i],dtime[i],eta);
	...
	for (i=0; i<n; i++) {
	    /*time[i] = ttime[i];*/
	    time[i] += dtime[i]*eta;
	    sf_warning("time=%f time2=%f dtime=%f eta=%f",time[i],time[i],dtime[i],eta);
	}
	return;
    }
	...
	for(i2 = 0; i2 < n2; i2++) {
	    for(i1 = 0; i1 < n1; i1++){
		i = AT(i1,i2,i3);
		dw[i3][i2][i1] = -0.5*dD[i3][i2][i1]*dD[i3][i2][i1]-2*dD[i3][i2][i1]*ty[i3][i2][i1]*(1-v[i]*tz[i3][i2][i1]*tz[i3][i2][i1]);
	    }
	}
    }
	...
	for(i2 = 0; i2 < n2; i2++) {
	    for(i1 = 0; i1 < n1; i1++){
		i = AT(i1,i2,i3);
		dw[i3][i2][i1] += -0.5*dD[i3][i2][i1]*dD[i3][i2][i1]+2*dD[i3][i2][i1]*tz[i3][i2][i1]*(1-v[i]*tz[i3][i2][i1]*tz[i3][i2][i1]);
	    }
	}
    }
	...
	for(i3 = 0; i3 < n3; i3++){
		...
		for(i1 = 0; i1 < n1; i1++){
		    i = AT(i1,i2,i3);
		    dw[i3][i2][i1] += -0.5*dD[i3][i2][i1]*dD[i3][i2][i1]-2*dD[i3][i2][i1]*tx[i3][i2][i1]*(1-v[i]*tz[i3][i2][i1]*tz[i3][i2][i1]);
		}
	    }
	}
    }
    ...
    ddtime[i] =  3*(((xs[0]-i1)*(xs[0]-i1)*d1*d1+(xs[1]-i2)*(xs[1]-i2)*d2*d2)*((xs[0]-i1)*(xs[0]-i1)*d1*d1+(xs[1]-i2)*(xs[1]-i2)*d2*d2)*
		    ...
		    4*((xs[0]-i1)*(xs[0]-i1)*d1*d1+(xs[1]-i2)*(xs[1]-i2)*d2*d2)*((xs[0]-i1)*d1*d1+(xs[1]-i2)*(xs[1]-i2)*d2*d2)
		    *((xs[0]-i1)*d1*d1+(xs[1]-i2)*(xs[1]-i2)*d2*d2)*
		    (xs[2]-i3)*(xs[2]-i3)*d3*d3*(xs[2]-i3)*(xs[2]-i3)*d3*d3)
	/(2*sqrt(((xs[0]-i1)*(xs[0]-i1)*d1*d1+(xs[1]-i2)*(xs[1]-i2)*d2*d2+(xs[2]-i3)*(xs[2]-i3)*d3*d3)*v[i])
	  *((xs[0]-i1)*(xs[0]-i1)*d1*d1+(xs[1]-i2)*(xs[1]-i2)*d2*d2+(xs[2]-i3)*(xs[2]-i3)*d3*d3)
	  *((xs[0]-i1)*(xs[0]-i1)*d1*d1+(xs[1]-i2)*(xs[1]-i2)*d2*d2+(xs[2]-i3)*(xs[2]-i3)*d3*d3)
	  *((xs[0]-i1)*(xs[0]-i1)*d1*d1+(xs[1]-i2)*(xs[1]-i2)*d2*d2+(xs[2]-i3)*(xs[2]-i3)*d3*d3)+0.00001);
    ttime[i] = time[i];
	...
	for (i=0; i<n; i++) {
	    /*time[i] = ttime[i];*/
	    time[i] += dtime[i]*eta+ddtime[i]*eta*eta;
	    sf_warning("time2=%f ddtime=%f eta=%f",time[i],ddtime[i],eta*eta);
	}
    } else {
	for (i=0; i<n; i++) {
	    /*time[i] = ttime[i];*/
	    tmp = dtime[i]-ddtime[i]*eta;
	    tmp = (SF_ABS(tmp)<0.00000001 ? SF_SIG(tmp)*0.00000001 : tmp);
	    /*if(SF_ABS(tmp)<0.0001)
	      time[i] += dtime[i]*eta+ddtime[i]*eta*eta;
	      else
	      time[i] += dtime[i]*dtime[i]*eta/tmp;*/
	    time[i] += dtime[i]*dtime[i]*eta/tmp;
	    sf_warning("time2=%f dtime==%f ddtime==%f ddtimeS=%f eta=%f",time[i],dtime[i]*eta,dtime[i]*eta+ddtime[i]*eta*eta,dtime[i]*dtime[i]*eta/(-ddtime[i]*eta+dtime[i]),eta*eta);
	}
    }
...
void updateds (int p1, int p2, int p3, float* tj, float* dtj, unsigned char* mj, float t, float tx, float ty, float tz, 
	       float s, float dy)
{
    float b, c, t1=0.0, t2, u, den,tp1,dt1=0.0;
    unsigned int k, i;
