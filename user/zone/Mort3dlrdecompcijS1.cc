/* 3-D three-components wavefield modeling based on original elastic anisotropic displacement 
  wave equation and vector decomposition using low-rank symbol approximation in 3D ORT media (qS1). */
/*
...
int main(int argc, char* argv[])
{
   ...
   par.get("point",tol,0.1); // tolerance for point singularity
   par.get("power",power,1); // exponent for smoothing


   sf_warning("npk=%d ",npk);
   sf_warning("eps=%f",eps);
   sf_warning("point=%f",tol);
   sf_warning("power=%d",power);
   ...
   at = sf_iaxa(Fx,4); nt = sf_n(at); dt = sf_d(at);
   fy = sf_o(ay)*1000.0;
   fx = sf_o(ax)*1000.0;
   fz = sf_o(az)*1000.0;
   ft = sf_o(at);
   ...
   * ***************************************************************************/
   ...
   ikxikyikz(ijkx, ijky, ijkz, nkx, nky, nkz);
    ...
    sf_file Fpx, Fpy, Fpz;
    Fpx= sf_output("out");
    Fpy= sf_output("ElasticS1y");
    Fpz= sf_output("ElasticS1z");

    puthead4x(Fpx, nz, nx, ny, nt, dz/1000.0, dx/1000.0, dy/1000.0, dt, fz/1000, fx/1000, fy/1000, 0.0);
    puthead4x(Fpy, nz, nx, ny, nt, dz/1000.0, dx/1000.0, dy/1000.0, dt, fz/1000, fx/1000, fy/1000, 0.0);
    puthead4x(Fpz, nz, nx, ny, nt, dz/1000.0, dx/1000.0, dy/1000.0, dt, fz/1000, fx/1000, fy/1000, 0.0);


    // Loop over snapshots
    int snap;
    for (snap=0;snap<nt;snap++) {
		sf_floatread(px, nxyz, Fx);
		sf_floatread(py, nxyz, Fy);
		sf_floatread(pz, nxyz, Fz);

		// separate qP wave 
		sf_warning("vector decomposition of S1-wave based on lowrank decomp.");
		...
		/*    double b11, b22, b33, b12, b13, b23, b44, b55, b66, a11, a12, a22, a33, a13, a23, kxt, kyt, kzt;
		nkx=nx;
		nky=ny;
		nkz=nz;
		dkx=2*SF_PI/dx/nx;
		...
		for(iy=0; iy < nky; iy++)
		   {
			 ky = ky0+iy*dky;
			 if(ky==0.0) ky=0.0000001*dky;

			 for(ix=0; ix < nkx; ix++)
			 {
			   kx = kx0+ix*dkx;
			   if(kx==0.0) kx=0.0000001*dkx;

			 for (iz=0; iz < nkz; iz++)
			 {
				kz = kz0+iz*dkz;
				if(kz==0.0) kz=0.0000001*dkz;

				rk = sqrt(ky*ky+kx*kx+kz*kz);
				ky /= rk; kx /= rk; kz /= rk;
				...
				kxt = cosphiy*cosphiz*kx + (cosphix*sinphiz + sinphix*sinphiy*cosphiz)*ky + (sinphix*sinphiz - cosphix*sinphiy*cosphiz)*kz;
				kyt = -cosphiy*sinphiz*kx + (cosphix*cosphiz - sinphix*sinphiy*sinphiz)*ky + (sinphix*cosphiz + cosphix*sinphiy*sinphiz)*kz;
				kzt = sinphiy*kx - sinphix*cosphiy*ky + cosphix*cosphiy*kz;
				kx=kxt; ky=kyt; kz=kzt;
				...
				// Locate point singularity/work/02621/yanadets/RSFSRC/user/chengjb
				double A = -(a11+a22+a33);
				double B = a11*a22+a11*a33+a22*a33-a12*a12-a13*a13-a23*a23;
				double C = a11*a23*a23+a22*a13*a13+a33*a12*a12-a11*a22*a33-2*a12*a13*a23;
				double D = -A*A/3 + B;
				double Q = 2*pow(A/3,3)-A*B/3+C;
				double nu = acos(-Q/(2*sqrt(pow(-D/3,3))));
				double coss = cos(nu/3);
				double sine = sqrt(1-coss*coss);

				double scale = abs(sine)/tol;
				if (abs(sine) < tol && abs(sine) > 1e-10) {
				px[i] *= 1/pow(scale,power);
				py[i] *= 1/pow(scale,power);
				pz[i] *= 1/pow(scale,power);
				}
...
static int samplebpyy(vector<int>& rs, vector<int>& cs, DblNumMat& res)
{
	...
	b66 = c66[i];
		...
		res(a,b) = usy*usy*pow(scale,power);
	    }
...
static int samplebpxx(vector<int>& rs, vector<int>& cs, DblNumMat& res)
{
	...
	b66 = c66[i];
		...
		res(a,b) = usx*usx*pow(scale,power);
	    }
...
static int samplebpzz(vector<int>& rs, vector<int>& cs, DblNumMat& res)
{
	...
	b66 = c66[i];
		...
		res(a,b) = usz*usz*pow(scale,power);
	    }
...
static int samplebpxy(vector<int>& rs, vector<int>& cs, DblNumMat& res)
{
	...
	b66 = c66[i];
		...
		res(a,b) = usx*usy*pow(scale,power);
	    }
...
static int samplebpxz(vector<int>& rs, vector<int>& cs, DblNumMat& res)
{
	...
	b66 = c66[i];
		...
		res(a,b) = usx*usz*pow(scale,power);
	    }
...
static int samplebpyz(vector<int>& rs, vector<int>& cs, DblNumMat& res)
{
	...
	b66 = c66[i];
		...
		res(a,b) = usy*usz*pow(scale,power);
	    }
