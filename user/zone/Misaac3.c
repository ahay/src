...
{
	int nr1, nr2, nr3, N, ir1, ir2, ir3, nt, nt2, order, niter, vstatus, count, debug;
	float x, y, dt, t0, bmin, bmax, xbmin, xbmax, ybmin, ybmax, tt;
	float ***rr, ***temp_rr, ***rd1, ***rd2, **ans, **xx, **xxnew, *xinitial, *yinitial, *updown, *v_inp, *gx_inp, *gy_inp, *gz_inp, *xref_inp, *yref_inp, *zref_inp ,*v, *gx, *gy, *gz, *xref, *yref, *zref, **F, **dk, **dk_old, **xxtem, **zk,***ck_inv; 
	...
	if (!sf_histint(refl,"n2",&nr2)) sf_error("No n2= in input");
	...
	for (q=0; q<w; q++) {
		...
		for (t=0; t<nr3+2;t++) {
			for (t1=0; t1<2; t1++) {
				...
				dk_old[t][t1] = dk[t][t1]; /*store the working dk*/
			}
