...
void fcoh1_tr(void *h, float *io1, float *in2, float *out1, float *out2)
...
>*/
{
	...
	xcorr(io1, in2, p->c, p->n1, p->min, p->max, p->ntw);
	...
	for(i1=0; i1<p->n1; i1++)
	{
		for(j1=p->min; j1<=p->max; j1++)
		{
			...
			var = out1[i1];
			if(it<0) var *= out2[0];
			else if(it>=p->n1) var *= out2[p->n1-1];
			else var *= out2[it];
			p->c[i1][j1] /= sqrt(var);
		}
		io1[i1] = para_intp_max(p->c[i1], m2);
		out1[i1] = p->c[i1][0];
	}
}
