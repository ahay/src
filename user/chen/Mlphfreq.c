...
int main(int argc, char*argv[])
{
	...
	/* linear phase approximator: maxflat; lagrange; bspline */
	if(!sf_getint("nf1", &nf1)) nf1=1;
	/* b[-nf1, ... ,nf2] */
	if(!sf_getint("nf2", &nf2)) nf2=1;
	/* b[-nf1, ... ,nf2] */
	...
	b1 = sf_floatalloc(nf1+nf2+1);

	c = lphase(nf1, nf2, interp);
	if(c==NULL) sf_error("interp=%s not correct", interp);
	for(i2=0; i2<n2; i2++)
	{
		delay = o2+d2*i2;
		if(iir) delay /= 2.0;
		lphase_filt(nf1+nf2, c, delay, b1, nf1+nf2, false);
		for(i1=0; i1<n1; i1++)
		{
			f = (o1+d1*i1);
			z1 = fir_freq(-nf1, nf2, b1+nf1, f);
