...
#pragma omp parallel for	    \
	...
	for    (ix=NOP; ix<fdm->nxpad-NOP; ix++) {
		...
		tzz[ix][iz] = Fz(uoz,ix,iz,idz);
	        ...
	        txz[ix][iz] = Bx(uoz,ix,iz,idx) + Bz(uox,ix,iz,idz);
	    }
	}		
...
#pragma omp parallel for			\
    schedule(dynamic,fdm->ompchunk)		\
    private(iz,ix)				\
    shared(fdm,tzz,txz,txx,uaz,uax,idz,idx)
#endif
	for    (ix=NOP; ix<fdm->nxpad-NOP; ix++) {
	    for(iz=NOP; iz<fdm->nzpad-NOP; iz++) {
		uax[ix][iz] = Bx( txx,ix,iz,idx ) + Fz( txz,ix,iz,idz );
		uaz[ix][iz] = Fx( txz,ix,iz,idx ) + Bz( tzz,ix,iz,idz );
	    }
	}
