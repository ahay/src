...
#pragma omp parallel
    ...
    c15in=sf_floatalloc2(n1   ,n2   ); 
    c33in=sf_floatalloc2(n1   ,n2   ); 
    c35in=sf_floatalloc2(n1   ,n2   ); 
    c55in=sf_floatalloc2(n1   ,n2   ); 

    c11  =sf_floatalloc2(fdm->n1pad,fdm->n2pad); 
    c13  =sf_floatalloc2(fdm->n1pad,fdm->n2pad); 
    c15  =sf_floatalloc2(fdm->n1pad,fdm->n2pad); 
    c33  =sf_floatalloc2(fdm->n1pad,fdm->n2pad); 
    c35  =sf_floatalloc2(fdm->n1pad,fdm->n2pad); 
    c55  =sf_floatalloc2(fdm->n1pad,fdm->n2pad); 

    sf_floatread(c11in[0],n1*n2,Fccc );
    sf_floatread(c13in[0],n1*n2,Fccc );
    sf_floatread(c15in[0],n1*n2,Fccc );
    sf_floatread(c33in[0],n1*n2,Fccc );
    sf_floatread(c35in[0],n1*n2,Fccc );
    sf_floatread(c55in[0],n1*n2,Fccc );

    expand(c11in,c11,fdm);
    expand(c13in,c13,fdm);
    expand(c15in,c15,fdm);
    expand(c33in,c33,fdm);
    expand(c35in,c35,fdm);
    expand(c55in,c55,fdm);

    free(*c11in); free(c11in);
    free(*c13in); free(c13in);
    free(*c15in); free(c15in);
    free(*c33in); free(c33in);
    free(*c35in); free(c35in);
    free(*c55in); free(c55in);
...
#pragma omp parallel for \
    ...
    private(iz,ix) \
    shared(fdm,tzz,txz,txx,uoz,uox,idz,idx)
#endif
	...
	/*                      c11(x     ,z     ) exx(x     ,z     )   */
	/* sxz(x-dx/2,z-dz/2) = c55(x-dx/2,z-dz/2) exz(x-dx/2,z-dz/2)   */
#ifdef _OPENMP
#pragma omp parallel for \
    schedule(dynamic,fdm->ompchunk) \
    private(iz,ix,szz,sxz,sxx) \
    shared(fdm,tzz,txz,txx,c11,c13,c15,c33,c35,c55)
#endif
	for    (ix=0; ix<fdm->n2pad; ix++) {
	    for(iz=0; iz<fdm->n1pad; iz++) {
...
#pragma omp parallel for \
    schedule(dynamic,fdm->ompchunk) \
    private(iz,ix) \
    shared(fdm,tzz,txz,txx,ua1,uax,idz,idx)
#endif
	for    (ix=NOP; ix<fdm->n2pad-NOP; ix++) {
	    for(iz=NOP; iz<fdm->n1pad-NOP; iz++) {
...
#pragma omp parallel for \
    schedule(dynamic,fdm->ompchunk) \
    private(iz,ix) \
    shared(fdm,uoz,uox,umz,umx,upz,upx,ua1,uax,ro,dt2)
#endif
	for    (ix=0; ix<fdm->n2pad; ix++) {
	    for(iz=0; iz<fdm->n1pad; iz++) {
