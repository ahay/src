...
#pragma omp parallel for					\
	...
	for        (iy=NOP; iy<fdm->nypad-NOP; iy++) {
		...
		for(iz=NOP; iz<fdm->nzpad-NOP; iz++) {
		    ...
		    txx[iy][ix][iz] = Fx(uox,ix,iy,iz,idx);
		    tyy[iy][ix][iz] = Fy(uoy,ix,iy,iz,idy);
		    tzz[iy][ix][iz] = Fz(uoz,ix,iy,iz,idz);
		    
		    txy[iy][ix][iz] = By(uox,ix,iy,iz,idy) + Bx(uoy,ix,iy,iz,idx);
		    tyz[iy][ix][iz] = Bz(uoy,ix,iy,iz,idz) + By(uoz,ix,iy,iz,idy);
		    tzx[iy][ix][iz] = Bx(uoz,ix,iy,iz,idx) + Bz(uox,ix,iy,iz,idz);
		}
	    }
	}
...
#pragma omp parallel for					\
	...
	for        (iy=NOP; iy<fdm->nypad-NOP; iy++) {
	    for    (ix=NOP; ix<fdm->nxpad-NOP; ix++) {
		for(iz=NOP; iz<fdm->nzpad-NOP; iz++) {		    
		    uax[iy][ix][iz] = Bx( txx,ix,iy,iz,idx ) + Fy( txy,ix,iy,iz,idy ) + Fz( tzx,ix,iy,iz,idz ) ;
		    uay[iy][ix][iz] = Fx( txy,ix,iy,iz,idx ) + By( tyy,ix,iy,iz,idy ) + Fz( tyz,ix,iy,iz,idz ) ;
		    uaz[iy][ix][iz] = Fx( tzx,ix,iy,iz,idx ) + Fy( tyz,ix,iy,iz,idy ) + Bz( tzz,ix,iy,iz,idz ) ;		    
		}
	    }
	}
