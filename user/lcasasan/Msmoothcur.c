...
int main(int argc, char* argv[])
{
    ...
    float **data, **dipt,**model, *model0; // **wt;
    char key[6];
    sf_file DATA, DIPT, MODEL, DATA_OUT;
    ...
    dipt = sf_floatalloc2(n1,n2);
    model = sf_floatalloc2(n1,n2);
    //model0 = sf_floatalloc2(n1,n2);
    model0 = sf_floatalloc(n2);
    //wt = sf_floatalloc2(n1,n2);

    sf_floatread(data[0],nd,DATA); /* data is slope R*/
    sf_floatread(dipt[0],nd,DIPT); /* dipt is slope derivative Dt[R] */
    ...
    for (i2=0; i2 < n2; i2++) {
    	for (i1=0; i1 < n1; i1++) {
    		dipt[i2][i1] *= data[i2][i1];	/* R*Dt[R] */
    	}
    }

    sf_repeat_lop(false,true,nd,nd,dipt[0],data[0]); /* d = R+Lp[ R*Dt[R] ]*/
