/* Slope-based tau-p velocity transform for VTI media by DIX equations. */
/*
...
int main (int argc, char* argv[])
{
	...
	float tau0, Vn2_eff, eta_eff,S_eff,dtau0_dtau,dVn2_dtau,deta_dtau; /* required for DIX oriented inversion*/
    float N, D, Nt, Dt;
	    ...
	    //p=p0+ip*dp;
            ...
            for (it=0; it < nt; it++) { /* time tau loop*/
	            ...
	            coord3[it][0]=t0-10.*dt;    // zero offset traveltime...
                    ...
                    if (inter) {			/* INTERVAL VELOCITIES with DIX EQUATIONS*/
			...
			//tempvec[it]= coord1[it][1];		
                        ...
                        coord3[it][1] = (1/p) * ( N * (4*t*R[it]*dt-D) ) / (32*t*R[it]*R[it]*R[it]*dt*dt*dt );
                        

                    }
			coord2[it][1] = sqrtf(fabsf( coord1[it][1]*coord1[it][1] * (1+2*coord3[it][1]) 	) );	
                        //sf_warning("\ntau0 %f Vn %f Vh %f eta %f",coord1[it][0],coord1[it][1],coord2[it][1],coord3[it][1]);
                }
            } /* END tau t loop */
