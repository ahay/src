...
int main(int argc, char* argv[])
{
	...
	case 's': /* slowness squared */
	    upgrad_set2(upg,t);
	    upgrad_forw2(upg,t,s); /* s = grad(t)*grad(t) */
	...
	case 'l': /* linear operator */
	    ...
	    if (inv) {
		if (adj) {
		    upgrad_inverse2(upg,s,t,NULL);
		} else {
		    upgrad_solve2(upg,t,s,NULL);
		}
	    } else {
		if (adj) {
		    upgrad_adj2(upg,s,t);
		} else {
		    upgrad_forw2(upg,t,s);
		}
	    }
	...
	case 'd': /* differentiation */
	    ...
	    upgrad_set2(upg,t);
	    upgrad_forw2(upg,t0,s); /* s = grad(t0)*grad(t) */

	    if (!squared) {
		ds = sf_floatalloc(nt);
		upgrad_forw2(upg,t,ds);
		for (it=0; it < nt; it++) {
		    s[it] /= sqrtf(ds[it]);
		}
		free(ds);
	...
	case 'i': /* integration */
	    ...
	    upgrad_solve2(upg,s,t,t0); /* solve for t */
