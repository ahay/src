...
int main (int argc, char *argv[])
{
    ...
    float dht = 0., oht, dhx = 0., dhy = 0., dhz = 0.;
    ...
    sf_file Fdx=NULL;   /*  dip field                       */
    sf_file Fdy=NULL;   /*  dip field                       */
	...
	if(! sf_getfloat("dhx",&dhx)) dhx = cub->amx.d*2;
	if(! sf_getfloat("dhy",&dhy)) dhy = cub->amy.d*2;
	if(! sf_getfloat("dhz",&dhz)) dhz = cub->az.d*2; 
	oht = -nht*dht;
    ...
    cip = wexncip_init(cub,nhx,nhy,nhz,nht,nhx2,nhy2,nhz2,nht2,nc,dhx,dhy,dhz,dht,oht,Fc,Fdx,Fdy,eic);

    /* construct image */
    wexncip_for(cub,cip,Fts,Ftr,Fm,0,0,0);

    /* construct CIP gathers */
    wexncip_for(cub,cip,Fts,Ftr,Fe,eic,0,0);
    ...
    if (Fdx!=NULL) sf_fileclose(Fdx);
    if(cub->amy.n>1)
      if (Fdy!=NULL) sf_fileclose(Fdy);
    /*------------------------------------------------------------*/
