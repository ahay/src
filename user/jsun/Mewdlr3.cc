...
/* x-to-x component 3d elastic wave extrapolation operator */
{
    ...
    for(int a=0; a<nr; a++) 
    {
	    ...
	    double sine = sqrt(1-coss*coss);
            ...
            /* slow S wave */
            double u1a = Chr[0+compa];
            double u1b = Chr[0+compb];
            /* fast S wave */
            double u2a = Chr[3+compa];
            double u2b = Chr[3+compb];
            /* P wave */
            double u3a = Chr[6+compa];
            double u3b = Chr[6+compb];
		...
		res(a,b) = u3a*u3b; 
            ...
            if (abs(sine) <= tol) res(a,b) *= abs(sine)/tol;
