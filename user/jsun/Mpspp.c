...
int main(int argc, char* argv[])
{
    ...
    float *vel,**dat,**dat_v,**wvfld,**wvfld0; /*velocity profile*/
    sf_file Fi,Fo,Fw,Fd_v,snaps; /* I/O files */
    ...
    if (NULL!=sf_getstring("wave")) {
        Fw = sf_input("wave");
        sf_histint(Fw,"n1",&tmp);
        if (tmp != nz1) sf_error("Error in first dimension!");
	sf_histint(Fw,"n2",&tmp);
        if (tmp != nx1) sf_error("Error in second dimension!");
	sf_histint(Fw,"n3",&tmp);
        if (snap <=0 && tmp != nt/snap) sf_error("Error in third dimension!");
    } else Fw = NULL;
    ...
    if (NULL!=Fw) wvfld0 = sf_floatalloc2(nx1*nz1,nt/snap);
    else wvfld0 = NULL;
    ...
    if (NULL!=wvfld0) sf_floatread(wvfld0[0],nz1*nx1*nt/snap,Fw);
    ...
    /*do the work*/
    psp4(wvfld, wvfld0, dat, dat_v, vel, par);
