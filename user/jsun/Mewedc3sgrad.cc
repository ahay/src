...
/* component-wise 3d elastic wave extrapolation operator */
{
    ...
    for(int a=0; a<nr; a++) 
    {
        ...
        for(int b=0; b<nc; b++)
        {
            ...
            switch (component) {
                case 0: // xx
                    res(a,b) = exp(v2t*zpx(0.,1.))*u2x*iu2x + exp(v1t*zpx(0.,1.))*u1x*iu1x;
                    break;
                case 1: // xy
                    res(a,b) = exp(v2t*zpx(0.,1.))*u2x*iu2y + exp(v1t*zpx(0.,1.))*u1x*iu1y;
                    break;
                case 2: // xz
                    res(a,b) = exp(v2t*zpx(0.,1.))*u2x*iu2z + exp(v1t*zpx(0.,1.))*u1x*iu1z;
                    break;
                case 3: // yx
                    res(a,b) = exp(v2t*zpx(0.,1.))*u2y*iu2x + exp(v1t*zpx(0.,1.))*u1y*iu1x;
                    break;
                case 4: // yy
                    res(a,b) = exp(v2t*zpx(0.,1.))*u2y*iu2y + exp(v1t*zpx(0.,1.))*u1y*iu1y;
                    break;
                case 5: // yz
                    res(a,b) = exp(v2t*zpx(0.,1.))*u2y*iu2z + exp(v1t*zpx(0.,1.))*u1y*iu1z;
                    break;
                case 6: // zx
                    res(a,b) = exp(v2t*zpx(0.,1.))*u2z*iu2x + exp(v1t*zpx(0.,1.))*u1z*iu1x;
                    break;
                case 7: // zy
                    res(a,b) = exp(v2t*zpx(0.,1.))*u2z*iu2y + exp(v1t*zpx(0.,1.))*u1z*iu1y;
                    break;
                case 8: // zz
                    res(a,b) = exp(v2t*zpx(0.,1.))*u2z*iu2z + exp(v1t*zpx(0.,1.))*u1z*iu1z;
                    break;
            }
