...
{
    int nt, n3, nx, np, i3, ntx, ntp,niterin,niterout,iter;
    bool adj,verb,rho,inv,cauchy;
    float o1,d1, x0,dx, anti, p0,dp,p1,eps;
    float *cmp=NULL, *vscan=NULL, *ww=NULL;
    ...
    if (!sf_getbool ( "inv",&inv )) inv=adj;  /* inversion flag */
    if (!sf_getbool ( "cauchy",&cauchy )) cauchy=false;  /* Norm for inversion */
	...
	if (inv) {
	    if (!sf_getint("niterin",&niterin)) niterin=100;
	    if (!sf_getint("niterout",&niterout)) niterout=1;
	...
	if(inv) {
	    for (iter=0;iter<=niterout;iter++) {
		if (cauchy) sf_cauchy (ntp,vscan,ww);
		else sf_l1 (ntp,vscan,ww);
		sf_weight_init(ww);
		sf_solver_reg (slant_lop,sf_cgstep,sf_weight_lop,
			       ntp,ntp,ntx,vscan,cmp,niterin,eps,"end");
	    }
	}
