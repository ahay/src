...
void gradient_pas_init(float **dd1, float ***ww1, float ***gwt1, sf_mpi *mpipar, sf_acqui acpar, sf_vec array, sf_fwi fwipar, bool verb1)
/*< initialize >*/
{
	verb=verb1;
	first=true; // only at the first iteration, need to calculate the gradient scaling parameter
	...
	numprocs=mpipar->numprocs;
        ...
        dd = (float***) sf_alloc (1,sizeof(float**));
        dd[0] = dd1;
        ww3 = ww1;
        gwt = gwt1;
...
#pragma omp parallel for \
            ...
            /*
            //JS
            if(counter==3 && it%50==0) sf_floatwrite(p1[0],padnzx,Fwfl1);
            */
...
#pragma omp parallel for \
        ...
        /*
        // JS
        if(counter==3) sf_floatwrite(pp[0], nr*nt, Fres);
        */
...
#pragma omp parallel for \
            ...
            /*
            // JS
            if(counter==3 && it%50==0) sf_floatwrite(p1[0],padnzx,Fwfl2);
            */
...
#pragma omp parallel for \
                    ...
                    for(ix=0; ix<nx; ix++){
                        for(iz=0; iz<nz; iz++){
                            ...
                            temp=temp*temp*temp;
                            temp=-2./temp;
                            grad[ix*nz+iz] += gwt[it][ix][iz]*(wave[wit+1][ix][iz]-2.*wave[wit][ix][iz]+wave[wit-1][ix][iz])/wdt2*p1[ix+nb][iz+nb]*temp;
                        }
                    }
                }
                wit--;
