...
/*
	    if (iz < nbt)
		phf = exp(-powf(ct*(nbt-iz)*ksz[ikz]/hypot(ksz[ikz],ksx[ikx]),2));
	    else if (iz > nz-1-nbb)
		phf = exp(-powf(cb*(iz-nz+1+nbb)*ksz[ikz]/hypot(ksz[ikz],ksx[ikx]),2));
	    if (ix < nbl)
		phf = exp(-powf(cl*(nbl-ix)*ksx[ikx]/hypot(ksz[ikz],ksx[ikx]),2));
	    else if (ix > nx-1-nbr)
		phf = exp(-powf(cr*(ix-nx+1+nbr)*ksx[ikx]/hypot(ksz[ikz],ksx[ikx]),2));
*/
