...
int main(int argc, char* argv[])
{
    ...
    if(verb) { 
        ...
        sf_warning("*******************************************");
        if (grad) sf_warning("Lowrank for RITE including gradient term for P!");
...
/* component-wise 3d elastic wave extrapolation operator */
{
    ...
    for(int a=0; a<nr; a++) 
    {
        ...
        for(int b=0; b<nc; b++)
        {
            ...
            switch (component) {
                case 0: // xx
                    //res(a,b) = u3x*conj(u3x);
                    res(a,b) = zpx(real(u3x)*real(u3x),0.);
                    break;
                case 1: // xy
                    //res(a,b) = u3x*conj(u3y);
                    res(a,b) = zpx(real(u3x)*real(u3y),0.);
                    break;
                case 2: // xz
                    //res(a,b) = u3x*conj(u3z);
                    res(a,b) = zpx(real(u3x)*real(u3z),0.);
                    break;
                case 3: // yx
                    //res(a,b) = u3y*conj(u3x);
                    res(a,b) = zpx(real(u3y)*real(u3x),0.);
                    break;
                case 4: // yy
                    //res(a,b) = u3y*conj(u3y);
                    res(a,b) = zpx(real(u3y)*real(u3y),0.);
                    break;
                case 5: // yz
                    //res(a,b) = u3y*conj(u3z);
                    res(a,b) = zpx(real(u3y)*real(u3z),0.);
                    break;
                case 6: // zx
                    //res(a,b) = u3z*conj(u3x);
                    res(a,b) = zpx(real(u3z)*real(u3x),0.);
                    break;
                case 7: // zy
                    //res(a,b) = u3z*conj(u3y);
                    res(a,b) = zpx(real(u3z)*real(u3y),0.);
                    break;
                case 8: // zz
                    //res(a,b) = u3z*conj(u3z);
                    res(a,b) = zpx(real(u3z)*real(u3z),0.);
                    break;
            }

            //if (abs(real(res(a,b)))>1. || abs(imag(res(a,b)))>1.) {
            //    sf_warning("Eigenvalue=(%f,%f)",real(sqrt(ww2[2])),imag(sqrt(ww2[2])));
            //    sf_warning("exp(ivkdt)=(%f,%f)",real(exp(v3t*zpx(0.,1.))),imag(exp(v3t*zpx(0.,1.))));
            //    sf_warning("Eigenvector  u3: (%f,%f) (%f,%f) (%f,%f)",real(u3x),imag(u3x),real(u3y),imag(u3y),real(u3z),imag(u3z));
            //    sf_warning("Eigenvector iu3: (%f,%f) (%f,%f) (%f,%f)",real(iu3x),imag(iu3x),real(iu3y),imag(iu3y),real(iu3z),imag(iu3z));
            //    sf_warning("u3'*iu1,u3'*iu2,u3'*iu3: (%f,%f) (%f,%f) (%f,%f)",real(u3x*iu1x+u3y*iu1y+u3z*iu1z),imag(u3x*iu1x+u3y*iu1y+u3z*iu1z),real(u3x*iu2x+u3y*iu2y+u3z*iu2z),imag(u3x*iu2x+u3y*iu2y+u3z*iu2z),real(u3x*iu3x+u3y*iu3y+u3z*iu3z),imag(u3x*iu3x+u3y*iu3y+u3z*iu3z));
            //    sf_warning("res(%d,%d) = (%f,%f)",a,b,real(res(a,b)),imag(res(a,b)));
            //}
