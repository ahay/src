...
/* static variables */
static float dt;
static bool tric,tstp,pseu,visc;
...
static std::valarray<float>  Qp,Qs;
static float w0;
...
int main(int argc, char* argv[])
{
    ...
    par.get("visc",visc,false);      // visco-elastic
    float fref;
    par.get("fref",fref,1000);       // reference frequency
    w0 = fref*2*SF_PI;
    ...
    if (nb==0) {
        ...
        if (visc) {
            iRSF qp("qp"), qs("qs");
            Qp.resize(nxyz); qp>>Qp;
            Qs.resize(nxyz); qs>>Qs;
        }
    } else {
        ...
        if (visc) {
            iRSF qp("qp"), qs("qs");
            Qp.resize(nxyz); qp>>tt; expand3d(tt,Qp,nb,nz,nx,ny);
            Qs.resize(nxyz); qs>>tt; expand3d(tt,Qs,nb,nz,nx,ny);
        }
    }
    ...
    if(verb) { 
        sf_warning("*******************************************");
        if (visc) sf_warning("*** Visco-elastic lowrank decomposition ***");
        else      sf_warning("****** Elastic lowrank decomposition ******");
        sf_warning("*******************************************");
...
/* component-wise 3d elastic wave extrapolation operator */
{
    ...
    for(int a=0; a<nr; a++) 
    {
        ...
        double gammap, gammas;
        if (visc) {
            gammap = atan(1./Qp[i])/SF_PI;
            gammas = atan(1./Qs[i])/SF_PI;
        }
        ...
        for(int b=0; b<nc; b++)
        {
            ...
            zpx w1t,w2t,w3t;
            if (visc) {
                double kk = sqrt(kx2+ky2+kz2);
                double v1=0, v2=0, v3=0;
                if (kk>0) { v1 = v1t/kk/dt; v2 = v2t/kk/dt; v3 = v3t/kk/dt; }
                double c1 = v1*cos(SF_PI*gammas/2.);
                double c2 = v2*cos(SF_PI*gammas/2.);
                double c3 = v3*cos(SF_PI*gammap/2.);
                double eta1 = -pow(v1,2.*gammas)*pow(w0,-2.*gammas)*cos(SF_PI*gammas);
                double eta2 = -pow(v2,2.*gammas)*pow(w0,-2.*gammas)*cos(SF_PI*gammas);
                double eta3 = -pow(v3,2.*gammap)*pow(w0,-2.*gammap)*cos(SF_PI*gammap);
                double tau1 = -pow(v1,2.*gammas-1.)*pow(w0,-2.*gammas)*sin(SF_PI*gammas);
                double tau2 = -pow(v2,2.*gammas-1.)*pow(w0,-2.*gammas)*sin(SF_PI*gammas);
                double tau3 = -pow(v3,2.*gammap-1.)*pow(w0,-2.*gammap)*sin(SF_PI*gammap);
                double tmp1 = tau1*pow(c1,2.)*pow(kk,2.*gammas+1.);
                double tmp2 = tau2*pow(c2,2.)*pow(kk,2.*gammas+1.);
                double tmp3 = tau3*pow(c3,2.)*pow(kk,2.*gammap+1.);
                w1t = zpx(sqrt(-pow(tmp1,2) - 4.*eta1*pow(c1,2.)*pow(kk,2.*gammas+2.)),-tmp1)*double(dt/2.); //JUNZHE!!!
                w2t = zpx(sqrt(-pow(tmp2,2) - 4.*eta2*pow(c2,2.)*pow(kk,2.*gammas+2.)),-tmp2)*double(dt/2.);
                w3t = zpx(sqrt(-pow(tmp3,2) - 4.*eta3*pow(c3,2.)*pow(kk,2.*gammap+2.)),-tmp3)*double(dt/2.);
            }
            ...
            switch (component) {
                case 0: // xx
                    if (tstp) {
                        if (pseu) {
                            res(a,b) = zpx(-v3t*v3t*u3x*u3x,0) + zpx(-v2t*v2t*u2x*u2x,0) + zpx(-v1t*v1t*u1x*u1x,0) ;
                        } else {
                            res(a,b) = zpx(2*(cos(v3t)-1.)*u3x*u3x,0) + zpx(2*(cos(v2t)-1.)*u2x*u2x,0) + zpx(2*(cos(v1t)-1.)*u1x*u1x,0) ;
                        }
                    } else {
                        if (visc) res(a,b) = exp(w3t*zpx(0.,1.))*u3x*u3x + exp(w2t*zpx(0.,1.))*u2x*u2x + exp(w1t*zpx(0.,1.))*u1x*u1x; //JUNZHE!!!
                ...
                case 1: // yy
                    if (tstp) {
                        if (pseu) {
                            res(a,b) = zpx(-v3t*v3t*u3y*u3y,0) + zpx(-v2t*v2t*u2y*u2y,0) + zpx(-v1t*v1t*u1y*u1y,0) ;
                        } else {
                            res(a,b) = zpx(2*(cos(v3t)-1.)*u3y*u3y,0) + zpx(2*(cos(v2t)-1.)*u2y*u2y,0) + zpx(2*(cos(v1t)-1.)*u1y*u1y,0) ;
                        }
                    } else {
                        if (visc) res(a,b) = exp(w3t*zpx(0.,1.))*u3y*u3y + exp(w2t*zpx(0.,1.))*u2y*u2y + exp(w1t*zpx(0.,1.))*u1y*u1y;
                ...
                case 2: // zz
                    if (tstp) {
                        if (pseu) {
                            res(a,b) = zpx(-v3t*v3t*u3z*u3z,0) + zpx(-v2t*v2t*u2z*u2z,0) + zpx(-v1t*v1t*u1z*u1z,0) ;
                        } else {
                            res(a,b) = zpx(2*(cos(v3t)-1.)*u3z*u3z,0) + zpx(2*(cos(v2t)-1.)*u2z*u2z,0) + zpx(2*(cos(v1t)-1.)*u1z*u1z,0) ;
                        }
                    } else {
                        if (visc) res(a,b) = exp(w3t*zpx(0.,1.))*u3z*u3z + exp(w2t*zpx(0.,1.))*u2z*u2z + exp(w1t*zpx(0.,1.))*u1z*u1z;
                ...
                case 3: // xy
                    if (tstp) {
                        if (pseu) {
                            res(a,b) = zpx(-v3t*v3t*u3x*u3y,0) + zpx(-v2t*v2t*u2x*u2y,0) + zpx(-v1t*v1t*u1x*u1y,0) ;
                        } else {
                            res(a,b) = zpx(2*(cos(v3t)-1.)*u3x*u3y,0) + zpx(2*(cos(v2t)-1.)*u2x*u2y,0) + zpx(2*(cos(v1t)-1.)*u1x*u1y,0) ;
                        }
                    } else {
                        if (visc) res(a,b) = exp(w3t*zpx(0.,1.))*u3x*u3y + exp(w2t*zpx(0.,1.))*u2x*u2y + exp(w1t*zpx(0.,1.))*u1x*u1y;
                ...
                case 4: // xz
                    if (tstp) {
                        if (pseu) {
                            res(a,b) = zpx(-v3t*v3t*u3x*u3z,0) + zpx(-v2t*v2t*u2x*u2z,0) + zpx(-v1t*v1t*u1x*u1z,0) ;
                        } else {
                            res(a,b) = zpx(2*(cos(v3t)-1.)*u3x*u3z,0) + zpx(2*(cos(v2t)-1.)*u2x*u2z,0) + zpx(2*(cos(v1t)-1.)*u1x*u1z,0) ;
                        }
                    } else {
                        if (visc) res(a,b) = exp(w3t*zpx(0.,1.))*u3x*u3z + exp(w2t*zpx(0.,1.))*u2x*u2z + exp(w1t*zpx(0.,1.))*u1x*u1z;
                ...
                case 5: // yz
                    if (tstp) {
                        if (pseu) {
                            res(a,b) = zpx(-v3t*v3t*u3y*u3z,0) + zpx(-v2t*v2t*u2y*u2z,0) + zpx(-v1t*v1t*u1y*u1z,0) ;
                        } else {
                            res(a,b) = zpx(2*(cos(v3t)-1.)*u3y*u3z,0) + zpx(2*(cos(v2t)-1.)*u2y*u2z,0) + zpx(2*(cos(v1t)-1.)*u1y*u1z,0) ;
                        }
                    } else {
                        if (visc) res(a,b) = exp(w3t*zpx(0.,1.))*u3y*u3z + exp(w2t*zpx(0.,1.))*u2y*u2z + exp(w1t*zpx(0.,1.))*u1y*u1z;
