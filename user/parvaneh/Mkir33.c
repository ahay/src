...
int main(int argc, char* argv[]) 
{
    ...
    int ns,nh, two, is, ih;
    float ***rfl, ***rgd, ***crv, ***dipx, ***dipy, *trace,*trace2;
    ...
    velocity3 vel;
    ktable ts, tg;
    sf_file refl, curv, modl, head, data;
    ...
    /* if linear operator */
	 ...
	 if (!sf_histint(modl,"n4",&nsx)) nsx=nx;
	 if (!sf_histfloat(modl,"o4",&s0x)) s0x=x0;
	 if (!sf_histfloat(modl,"d4",&dsx)) dsx=dx;
    
	 
	 if (!sf_histint(modl,"n5",&nsy)) nsy=ny;
	 if (!sf_histfloat(modl,"o5",&s0y)) s0y=y0;
	 if (!sf_histfloat(modl,"d5",&dsy)) dsy=dy;

	
	 /*** Initialize offsets ***/
	 
	 if (!sf_histint  (modl,"n2",&nhx)) nhx=nx;
	 if (!sf_histfloat(modl,"o2",&h0x)) h0x=0.;
	 if (!sf_histfloat(modl,"d2",&dhx)) dhx=dx;


	

	 if (!sf_histint  (modl,"n3",&nhy)) nhy=ny;
	 if (!sf_histfloat(modl,"o3",&h0y)) h0y=0.;
	 if (!sf_histfloat(modl,"d3",&dhy)) dhy=dy;
	 ...
	 if (adj) {
	     ...
	     sf_floatread(rfl[0][0],nxyc,data);
	 }
     }
	 ...
	 for (ih=0; ih < nh; ih++) { 
		 ...
		 sf_aastretch_lop (true,false,nxyc,nt,ampl[0][0],trace); 
	     ...
	     for (iy=0; iy < ny; iy++) { for (ix=0; ix < nx; ix++) {
			 ...
			 if (lin) {
			     if (adj) {
				 rfl[ic][iy][ix] += ampl[ic][iy][ix]*obl*dx/amp;
			     } else {
				 ampl[ic][iy][ix] = rfl[ic][iy][ix]*obl*dx/amp;
			     }
			 } else {
     ...
     if (lin && adj) sf_floatwrite(rfl[0][0],nxyc,data);
