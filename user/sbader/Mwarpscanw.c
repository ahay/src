...
{ 
    ...
    if (!sf_getbool("ren",&ren)) ren=false;
    /* use reference energy */
    if (!sf_getbool("den",&den)) den=false;
    /* use data energy */
    ...
    if (ren) {
      renergy = sf_input("renergy");

      if(!sf_histint(renergy,"n1",&nr)) sf_error ("No n1= in renergy");
      if(!sf_histfloat(renergy,"d1",&dr)) sf_error ("No d1= in renergy");
      if(!sf_histfloat(renergy,"o1",&or)) or = 0.;

      if (nr != n2) sf_error ("Length of renergy is not same as reference trace");

      rergy = sf_floatalloc2 (n2,ntr);
      sf_floatread(rergy[0],n2*ntr,renergy);
    }

    if (den) {
      denergy = sf_input("denergy");

      if(!sf_histint(denergy,"n1",&nd)) sf_error ("No n1= in denergy");
      if(!sf_histfloat(denergy,"d1",&dd)) sf_error ("No d1= in denergy");
      if(!sf_histfloat(denergy,"o1",&od)) or = 0.;

      if (nd != n1) sf_error ("Length of denergy is not same as data trace");

      dergy = sf_floatalloc2 (n1,ntr);
      sf_floatread(dergy[0],n1*ntr,denergy);
    }
    ...
    warpscanw(cheb,sign,ren,den,inp,oth,rergy,dergy,rat1);
