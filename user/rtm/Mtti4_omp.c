...
int main(int argc, char* argv[]) 
{
    ...
    float vk, vk2, tmpvk, k2, err, dt2, kx1, kz1, tmpk, tmp;
...
//    if (SF_FLOAT != sf_gettype(inp)) sf_error("Need float input");
  ...
  //  if (!sf_histfloat(inp,"d2",&dt)) sf_error("No d2= in input");
    ...
    if (!sf_getfloat("err",&err)) err = 0.000001;
...
//    sf_putfloat(out,"o1",x0);
    ...
    sf_putint(out,"n3",1+(nt-1)/jm);
    sf_putfloat(out,"d3",dt*jm);
   ...
   if(!sf_getfloat("vx0",&vx0)) { 
    ...
    sf_warning("Vp0: %f",vz0);
    sf_warning("Vx0: %f",vx0);
    sf_warning("Seta0: %f",se0);
    sf_warning("Eta0: %f",yi0);
...
*/
    ...
    for (it=0; it < nt; it++) {
        ...
        if(it<1500) source_smooth(cur,isz+nbt,isx+nbl,wav[it]);
        if(!(it%jm)) {
          for(iz=0;iz < nz;iz++) sf_floatwrite(cur[nbt+iz]+nbl,nx,out); 
