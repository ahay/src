...
/* now we have 6 sides of the cube!*/
      /****************************/
       ...
       #pragma omp parallel private(ix,iy,iz)
       {
...
/*******************************************************************************/
/****************************/
#pragma omp parallel private(ix,iy,iz,sigmax,sigmay,sigmaz)
{
 
// UP-LEFT-FRONT + UP-RIGHT-FRONT + DOWN-RIGHT-FRONT
for 		(iy=NOP; iy<nb; iy++){ sigmay=sigma[iy];
	for		(ix=NOP; ix<nb; ix++){ sigmax=sigma[ix];
		for (iz=NOP; iz<nb; iz++){ sigmaz=sigma[iz];
...
for			(iy=nb; iy<ny-nb; iy++){
	for		(ix=NOP; ix<nb; ix++){ sigmax=sigma[ix];
		for (iz=nz-nb; iz<nz-NOP; iz++){ sigmaz=sigma[nz - iz];
...
#pragma omp for schedule(dynamic,fdm->ompchunk)
/****************************/    
for			(iy=ny-nb; iy<ny-NOP; iy++){ sigmay=sigma[ny - iy];
	for		(ix=NOP; ix<nb; ix++){ sigmax=sigma[ix];
		for (iz=nb; iz<nz-nb; iz++){
