...
int main(int argc, char *argv[])
{
    ...
    float r0,a0,dr; // spiral parameters
    ...
    if (!sf_getfloat("dr",&dr)) sf_error("Need spiral parameter dr.");
    ...
    for (i3=0; i3<n3; i3++) {
        ...
        for (i2=0; i2<n2; i2++) {
            ...
            if (origin_a[i2] < 0) origin_a[i2] += 360.0;
            // Change angle from -180~180 to 0~360
            sorted[i2].value =
                floor( ((origin_d[i2]-r0)/dr + (origin_a[i2]-a0))/360.0 )
                * 360.0 + origin_a[i2] - a0;
            // Sorting values are set as theta+2*n*pi
            sorted[i2].ikey = i2;
        }
