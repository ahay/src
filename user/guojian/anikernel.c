...
struct shot_ker_ani_type shot_ker_ani_init(float *phi,int nx,float dx,float dy,float dz){
  ...
  pphi=fabs(*phi);
  stat=file_open_input("Extable",&anitable);
  oangle=anitable.o[4]; dangle=anitable.d[4]; nangle=anitable.n[4];
  iangle=(pphi/3.1415926*180.0-oangle)/dangle; if (iangle >=nangle) iangle=nangle-1;
  //if (iangle <18) iangle=18;
...
*/ 
  ...
  pphi=pphi/180.0*3.1415926; 
  if ( *phi<0 ) *phi=-pphi; else *phi=pphi; 

  ani_par.o_dl=anitable.o[1];     ani_par.d_dl=anitable.d[1];   ani_par.n_dl=anitable.n[1];
  ani_par.o_ep=anitable.o[2];     ani_par.d_ep=anitable.d[2];   ani_par.n_ep=anitable.n[2];
  ani_par.o_w_vp=anitable.o[3];   ani_par.d_w_vp=anitable.d[3]; ani_par.n_w_vp=anitable.n[3];
  
  ani_par.n=anitable.n[0]/2; ani_par.m=100; ani_par.f=1.0-10.0*10.0/(27.0*27.0);  
  ani_par.phi=pphi; 
  //printf("1111111\n");
  //printf("size of contable: nn=%d\n",ani_par.n*ani_par.n_dl*ani_par.n_ep*ani_par.n_w_vp);
  ani_par.contablepp=allocatec(ani_par.n*ani_par.n_dl*ani_par.n_ep*ani_par.n_w_vp);
  ani_par.contablepm=allocatec(ani_par.n*ani_par.n_dl*ani_par.n_ep*ani_par.n_w_vp);
  ani_par.conapp=allocatec(ani_par.n);  ani_par.conapm=allocatec(ani_par.n);
  //printf("22222222222\n");
  //printf("aniparnnnnnn=%d\n",ani_par.n);
  blocksize=8*(ani_par.n*2)*ani_par.n_dl*ani_par.n_ep*ani_par.n_w_vp;

  //printf("iangle=%d\n",iangle);
  if ( sseek_block("Extable",iangle,blocksize,0)!=iangle) seperr("Table seek wrong");
  if ( sreed("Extable",ani_par.contablepp,blocksize/2)!=blocksize/2 ) seperr("Table read wrong");
  if ( sreed("Extable",ani_par.contablepm,blocksize/2)!=blocksize/2 ) seperr("Table read wrong");
  return ani_par;
}
...
struct shot_ker_ani_type shot_ker_ani_init_impulse(float phi,int nx,float dx,float dy,float dz){
  ...
  n=5;  m=nx/2; m=150; 
  dkx=2.0*3.1415926/( (float)(nx)*dx );
  dkx=2.0*3.1415926/( (float)(2*m)*dx);
...
//********************************
  ...
  ani_par.contablepp=allocatec(n*n_dl*n_ep*n_w_vp);
  ani_par.contablepm=allocatec(n*n_dl*n_ep*n_w_vp);
  ani_par.conapp=allocatec(n);  ani_par.conapm=allocatec(n);
  printf("begin table\n");
  printf("begin mmmm=%d\n",m);
...
void shot_ani_convlv(__complex__ float *wld_z,float *v_z,float *ep_z,float *dl_z,float w,struct shot_ker_par_type ker_par,struct shot_ker_ani_type ani_par,int signn)
{
  float w_vp,ep,dl;
  int nx,ny;
  __complex__ float  *tmpwld_z; //(:,:)
  __complex__ float *conapp,*conapm;
  int  iy,ix,ic,i;
...
void inadd(__complex__ float *app,__complex__ float *p,float scale,int n){
  int i;
  for(i=0;i<n;i++)
     app[i]+=scale*p[i];
}
...
__complex__ float *conapp,*conapm,*contablepp,*contablepm;
__complex__ float *p000,*p001,*p010,*p011,*p100,*p101,*p110,*p111;
...
vector_value_c(conapp,cmplx(0.0,0.0),ani_par->n);
...
vector_value_c(conapm,cmplx(0.0,0.0),ani_par->n);
...
void shot_ani_convlv3dtest(__complex__ float *wld_z,__complex__ float *conapp,__complex__ float *conapm,int tnx,int tny,struct shot_ker_par_type ker_par, int signn)
{
  int nx,ny,tn;
  __complex__ float  *tmpwld_z; //(:,:)
  __complex__ float *conap,*conam;
  int  iy,ix,ic,i,icx,icy;
  tn=tnx*tny;
  nx=ker_par.nx; ny=ker_par.ny;
  tmpwld_z=ker_par.tmp_wld;   //allocate(tmpwld_z(nx,ny)) ker_par.tmp_wld is allocated in ker_init in kernal.c
  conap=allocatec(tn); conam=allocatec(tn);
  vector_cp_c(tmpwld_z,wld_z,nx*ny); //tmpwld_z=wld_z;
...
void shot_phase_shift_ani(__complex__ float *wld_z,float w_v0_z,float ep,float dl,float phi,float f,struct shot_ker_par_type ker_par,int signn)
{// wld_z(ker_par.ny,ker_par.nx)
  ...
  __complex__ float kz1;
  __complex__ float *kza;
  pio2=3.1415926*2.0;
  nx=ker_par.nx; ny=ker_par.ny; 
  ...
  for(iy=0;iy<ny; iy++){
    ...
    for(ix=0;ix<nx;ix++){
      ...
      if ( fabs(aimag( kza[i2(iy,ix,nx)]  ))<0.00000000000001){
        if ( fabs(aimag( kza[i2(iy,ix,nx)]  )) <0.00000000001*fabs(real( kza[i2(iy,ix,nx)]  )) )
         kza[i2(iy,ix,nx)]=cmplx(fabs(real(kza[i2(iy,ix,nx)])),0.0);    
        else
         kza[i2(iy,ix,nx)]=cmplx(0.0,-fabs(aimag(kza[i2(iy,ix,nx)])));
      }
      else
        kza[i2(iy,ix,nx)]=cmplx(0.0,-fabs(aimag(kza[i2(iy,ix,nx)])));
    }
  ...
  for (iy=0;iy<ny;iy++){
    for(ix=0;ix<nx;ix++){
     if (fabs(__imag__ kza[i2(iy,ix,nx)])<0.000001){
        if ( fabs(__imag__ kza[i2(iy,ix+1,nx)])>0.000001 && fabs(__imag__ kza[i2(iy,ix-1,nx)])>0.000001   )
           kza[i2(iy,ix,nx)]=cmplx( 0.0,1.0);
        if ( fabs(__imag__ kza[i2(iy+1,ix,nx)])>0.000001 && fabs(__imag__ kza[i2(iy-1,ix,nx)])>0.000001   )
           kza[i2(iy,ix,nx)]=cmplx( 0.0,1.0);
 ...
 for(ix=nx/2;ix<nx;ix++)
   if (fabs(__imag__ kza[i2(iy,ix,nx)])>0.001 && fabs(__real__ kza[i2(iy,ix,nx)])<0.00000001) break;
 weitmx2=ix;
 for(ix=0;ix<nx/2;ix++)
   if (fabs(__imag__ kza[i2(iy,ix,nx)])>0.001 && fabs(__real__ kza[i2(iy,ix,nx)])<0.00000001) break;
 weitmx1=ix; 
  ...
  for(iy=0;iy<ny;iy++){ //do iy=1,ny
    ...
    for(ix=0;ix<nx;ix++){  //do ix=1,nx
      ...
      if ( fabs(aimag(kz1)) <0.0000001*fabs(real(kz1)) ){
        phsft=-signn*dz*fabs(real(kz1));
        wld_z[i2(iy,ix,nx)]=wld_z[i2(iy,ix,nx)]*cmplx(cos(phsft),sin(phsft));
      }
      else{
        wld_z[i2(iy,ix,nx)]=cmplx(0.0,0.0);
        //phsft=dz*fabs(aimag(kz1));  
...
void shot_phase_shift_vti(__complex__ float *wld_z,float w_v0_z,float ep,float dl,struct shot_ker_par_type ker_par,int signn)
{// wld_z(ker_par.ny,ker_par.nx)
  int nx,ny,weitmx1,weitmx2,weitmy1,weitmy2;
  float fkx,fky,dkx,dky,dz;
