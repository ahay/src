/* 2-D analytic solution for acoustic wave equation */
/*
...
float kernel(float fi, float vel,  float tau, float t, float x, float x0)
{
    float dist, delt;
    dist = x - x0;
    delt = t - tau;
    return fi/(sqrt(vel*vel*delt*delt-dist*dist)+SF_EPS);//
}
