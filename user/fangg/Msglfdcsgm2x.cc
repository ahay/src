...
{   
	...
	case 3 : // exponential filter
	    ...
	    for (int k=0; k<LEN; k++) {
		//sigm._data[k] = exp(-1.0*alpha*pow((sx._data[k]+0.5)/len,2.0))*exp(-1.0*alpha*pow(sz._data[k]/len,2.0));
		sigm._data[k] = exp(-1.0*alpha*pow((fabs(sx._data[k])+0.5)/len,2.0))*exp(-1.0*alpha*pow(fabs(sz._data[k]+1)/len,4.0));
	    }
	default :
	   for (int k=0; k<LEN; k++) {
		sigm._data[k] = 1.0; 
    ...
    for (int ix=0; ix<nxz; ix+=1000){
	for (int ik=0; ik<nxz; ik++) {
	    tmpsum=0.0;
	    for (int il=0; il<LEN; il++) tmpsum+=G(ix, il)*B(il, ik);
	    tmpsum = tmpsum*ks[ik]*vs[ix]*dt/(kx[ik]*2);
