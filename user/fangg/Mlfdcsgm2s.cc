//  using Sergey's suggestion: add sigma filter to B, then apply weighted least square
...
int main(int argc, char** argv)
{
    ...
    sf_warning("=======================");
    sf_warning(" Sergey: 2D Lowrank FD coefficients with weighted least square and sigma filter");
    ...
    int LEN = sz._m;
    DblNumMat Bx(LEN, nxz), B(LEN, nxz), Bs(LEN, nxz);
    DblNumMat Bxc(LEN, COUNT), Bc(LEN, COUNT), Bsc(LEN, COUNT);
	...
	for(int ix=0; ix<nxz; ix++) {
	    B(k, ix) = B(k,ix)*sigm._data[k];
	}
    }
    for(int k=0; k<LEN; k++) {
	for(int ix=0; ix<COUNT; ix++) {
	    Bc(k, ix) = Bc(k,ix)*sigm._data[k];
	}
    }
