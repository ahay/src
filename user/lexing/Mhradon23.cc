// 2-D to 3-D hyperbolic Radon transform.
...
int main(int argc, char** argv)
{
  ...
  CpxNumMat u(nt,np1*np2);  setvalue(u,cpx(0,0));
  FltNumVec t(nt);   
  ...
  ck1 = clock();
  float time_chck = float(ck1-ck0)/CLOCKS_PER_SEC*float(nt)*float(np1*np2)/float(NC);
  //
  ...
  std::valarray<float> udata(nt*np1*np2);
  //udata.resize(nt*np1*np2);
    
  for (int i=0; i<nt; i++)
    for (int j=0; j<np1; j++)
      for (int k=0; k<np2; k++)
        udata[nt*np1*k+nt*j+i]=real(u(i,np1*k+j));
