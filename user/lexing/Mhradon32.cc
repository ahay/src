// 3-D to 2-D hyperbolic Radon transform.
...
int main(int argc, char** argv)
{
  ...
  CpxNumMat f(nw,nx*ny);  setvalue(f,cpx(0,0));
  for (int i=0; i<nw; i++)
    for (int j=0; j<nx; j++)
      for (int k=0; k<ny; k++)
	f(i,nx*k+j) = cpx(crealf(fdata[nw*nx*k+nw*j+i]),cimagf(fdata[nw*nx*k+nw*j+i]));
  FltNumVec w(nw);
