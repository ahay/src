/* Program for iteratively determining the optimal path through a cost function input array.  Input file's second dimension is a parameter that can vary along the path, and the first dimension is "time", or a coordinate that the output path, R(t), will be a function of. Dimensions greater than 2 in the input array represent additional panels to calculate the path through. To use this program for picking a path maximizing an objective function, first transform the objective function \alpha by exp(-1*\alpha) and feed the resulting output to this program. Path integral minimization is in the manner of "Path Optimization with Application to Tunneling", Dorothea M. Einarsdottir, Andri Arnaldsson, Finnbogi Oskarsson, and Hannes Jonsson, 2010 */
#include <rsf.h>
#include <stdlib.h>
#include <time.h>
#include "path.h"
int main (int argc, char* argv[]){
	...
	/* allocate path tangent array and its precursors.  Tangent determined in manner of: */
	/* Improved tangent estimate in the nudged elastic band method for finding minimum energy paths and saddle points, Graeme Henkelman and Hannes Jonsson, 2000 */
	...
	srand(time(0)*time(0));	
	float randum ;
	/* loop through midpoints (different picking panels)*/
	int imid, iter;
	for ( imid = 0 ; imid < nmid ; imid ++ ){
		...
		for ( iter = 0 ; iter < niter ; iter++ ){
			...
			/* apply anisotropy to make a preferred direction.  Decreasing the anisotropy factor reduces the cost of traveling along the picked coordinate (v) axis, and thus results in more serpentine paths */
			...
			/* get the attractive force pulling knots together trying to minimize path cost by "pulling" the path into lows of S along gradient */
			path_attractive_force( Attract, G, Tau_p, Tau_m, knots, ndim);
			/* get the "repulsive force" between knots.   This force resists increasing the path length by pulling a knot toward its neighbors.  */
