		...
		taper = 1.;
               ...
               if ( R1*R1 + R2*R2 > 1) {
                  taper = 0.;
                     ...
                     if ( L1*L1 + L2*L2 > 1) {
/*                            if(curDip1 > 0){
                            one = (curDip1 - dipI - secondLeft1)/(secondRight1 - secondLeft1);
                            }else{
                            one = (curDip1 - dipI + secondLeft1)/(secondRight1 - secondLeft1);
                                  }
                            if(curDip2 > 0){
                            two = (curDip2 - dipX - secondLeft2)/(secondRight2 - secondLeft2);
                            }else{
                            two = (curDip2 - dipX + secondLeft2)/(secondRight2 - secondLeft2);
                                  }
                            taper =1.-sqrt( one*one + two*two );
//                            taper = cos (place * M_PI * 0.5);

                            //pseudoradius for inner elipse
                            L_rad=secondLeft1*secondLeft1*secondLeft2*secondLeft2;
                            //pseudoradius for outer elipse
                            R_rad=secondRight1*secondRight1*secondRight2*secondRight2;

                            diff=R_rad-L_rad;
                            //calculate how far out from inner pseudoradius you are
                            distI = secondLeft1*secondLeft1*(curDip1 - dipI)*(curDip1 - dipI);
                            distX = secondLeft2*secondLeft2*(curDip2 - dipX)*(curDip2 - dipX);
                            delta = distI + distX - L_rad;
                            
                            diff_I = secondRight1*secondRight1-secondLeft1*secondLeft1;
                            diff_X = secondRight1*secondRight1-secondLeft2*secondLeft2;

                            sumd = (curDip1 - dipI)*(curDip1 - dipI)*diffI + (curDip2 - dipX)*(curDip2 - dipX)*diffX;
                            
                            fixd = sumd/(R_rad-L_rad);
                           
                            taper=fixd;

                            distI = secondLeft2*secondLeft2*(curDip1 - dipI)*(curDip1 - dipI);
                            distX = secondLeft1*secondLeft1*(curDip2 - dipX)*(curDip2 - dipX);
                            dist = distI + distX;
