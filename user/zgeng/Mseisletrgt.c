...
{
    int n1, n2, n3, n12, nt, nw; // n1: trace time length, n2: trace number
    float o1, d1;
    ...
    rgt = sf_input("rgt"); // input rgt file
    ...
    if (!sf_histint(rgt,"n1",&nt)) sf_error("No n1= in rgt");
    if (nt != n1) sf_error("n1=%d in rgt must equal n1=%d in input",nt,n1);
    if (!sf_histint(rgt,"n2",&nw)) sf_error("No n2= in rgt");
    if (nw != n2) sf_error("Need %d traces in rgt, got %d", n2, nw);
