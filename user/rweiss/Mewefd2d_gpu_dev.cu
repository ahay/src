...
/*
  ...
  of the ewefd2d_gpu that is able to take as input an intial acceleration field.
...
#include "fdutil.c"
#include "ewefd2d_kernels_dev.cu"
...
int main(int argc, char* argv[]) {
    ...
    bool verb,fsrf,snap,ssou,dabc,wavSrc;
	...
	if (!wavSrc){
		Fum  = sf_input ("um"); /* if wavSrc=n, looks for um file containing the previous displacement timestep */
		Fuo  = sf_input ("uo"); /* if wavSrc=n, looks for uo file containing the current displacement timestep */
	}
    /*------------------------------------------------------------*/
	...
	if (wavSrc){
		...
		float *h_uTemp = (float*)malloc(fdm->nxpad * fdm->nzpad * sizeof(float));
		
		sf_floatreadtransp(h_uTemp, Fum, fdm->nxpad, fdm->nzpad);
		cudaMemcpy(d_umz, h_uTemp, fdm->nzpad * fdm->nxpad * sizeof(float), cudaMemcpyHostToDevice);
		sf_floatreadtransp(h_uTemp, Fuo, fdm->nxpad, fdm->nzpad);
		cudaMemcpy(d_uoz, h_uTemp, fdm->nzpad * fdm->nxpad * sizeof(float), cudaMemcpyHostToDevice);
		
		sf_floatreadtransp(h_uTemp, Fum, fdm->nxpad, fdm->nzpad);
		cudaMemcpy(d_umx, h_uTemp, fdm->nzpad * fdm->nxpad * sizeof(float), cudaMemcpyHostToDevice);
		sf_floatreadtransp(h_uTemp, Fuo, fdm->nxpad, fdm->nzpad);
		cudaMemcpy(d_uox, h_uTemp, fdm->nzpad * fdm->nxpad * sizeof(float), cudaMemcpyHostToDevice);
