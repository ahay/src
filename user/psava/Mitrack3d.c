...
int main(int argc, char* argv[])
{
    ...
    float *gau,gxc,gxs,gyc,gys; /* Gaussian,center and sigma */
	...
	for(jf=0; jf<sf_n(af); jf++) {
		...
		amp[jf][jn] = dist==0?1.0:1.0/(4*SF_PI*dist);  
		tim[jf][jn] = dist*slow;                   
	    }
	}
    }
    ...
    if(!sf_getfloat("gyc",&gyc)) gyc=0.;   /* Gaussian center y */
    if(!sf_getfloat("gys",&gys)) gys=0.;   /* Gaussian stdev  y */
	...
	gxs = (gxs==0)?0:1./ (2.*pow(gxs,2));
	gys = (gys==0)?0:1./ (2.*pow(gys,2));
	for(jf=0; jf<sf_n(af); jf++)
	    gau[jf] = exp( - pow(ff[jf].x-gxc,2) * gxs  - pow(ff[jf].y-gyc,2) * gys);
    }
