/* 2D isotropic time-domain FD modeling */
/*
...
int main(int argc, char* argv[])
{
    ...
    /* precompute dt^2 and vp^2 */
    for    (ix=0; ix<fdm->nxpad; ix++) {
	for(iz=0; iz<fdm->nzpad; iz++) {
	    vt[ix][iz] = dt*dt;
	    
	    vp2[ix][iz] = vp[ix][iz] * vp[ix][iz];
	    vh2[ix][iz] = vp[ix][iz] * vp[ix][iz];
	    vv2[ix][iz] = vp[ix][iz] * vp[ix][iz];
...
#pragma omp parallel for				\
    ...
    private(ix,iz)					\
    shared(fdm,ua,uo,cox,coz,cax,caz,cbx,cbz,idx,idz,vp2)
#endif
	for    (ix=NOP; ix<fdm->nxpad-NOP; ix++) {
		...
		ua[ix][iz] =
		    (cox* uo[ix  ][iz  ] + 
		     ...
		     cbx*(uo[ix-2][iz  ] + uo[ix+2][iz  ])) * vp2[ix][iz] +
		    (coz* uo[ix  ][iz  ] + 
		     caz*(uo[ix  ][iz-1] + uo[ix  ][iz+1])  +
		     cbz*(uo[ix  ][iz-2] + uo[ix  ][iz+2])) * vp2[ix][iz] ;
