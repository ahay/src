/* 2D Born synthetic based on Helmholtz forward solver. */
/*
...
int main(int argc, char* argv[])
{
    ...
    float d1, d2, **v, **refl, ds, os, ow, dw;
    double omega;
    sf_complex ***f, ***fborn;
    sf_file in, out, source, reflfile;
    int uts, mts, is, i, j;
    char *order;
...
//#endif
    ...
    /* read reflectivity */
    if (NULL == sf_getstring("refl")) sf_error("Need refl=");
    reflfile = sf_input("refl");
    refl=sf_floatalloc2(n1,n2);
    sf_floatread(refl[0],n1*n2,reflfile);
    ...
    for (iw=0; iw<nw; iw++ ) { 
        ...
        for (j=0; j<n2; j++) {
        for (i=0; i<n1; i++) {
            fborn[is][j][i]=omega*omega*f[is][j][i]*refl[j][i]/(v[j][i]*v[j][i]);
        }
        }
        }
       
        /* calculate Born synthetic */
        sparse_solve(npml, pad1, pad2, fborn, hermite, ns, uts);

        /* write out wavefield */
        sf_complexwrite(fborn[0][0],n1*n2*ns,out);
