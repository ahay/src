...
int main(int argc, char* argv[])
{
    ...
    if (!sf_getfloat("epsilon",&epsilon)) epsilon=0.001;
...
*/
   ...
   if (NULL != snaps) {
	...
	for (iz=2; iz < nzb-2; iz++) {
	    for (ix=0; ix < nx; ix++) {
		for (ih=0; ih < nh; ih++) {
                    tmpq[iz][ix][ih] = a*tmp[iz][ix][ih] + b3*(tmp[iz+1][ix][ih]+tmp[iz-1][ix][ih]) + d3*(tmp[iz+2][ix][ih]+tmp[iz-2][ix][ih]);
                }
            }
        }
	for (ix=0; ix < nx; ix++) {
            for (ih=0; ih < nh; ih++) {
                ...
                */ 
                tmpq[0][ix][ih] = a*tmp[0][ix][ih] + 2.0*b3*(tmp[1][ix][ih]) + 2.0*d3*(tmp[2][ix][ih]);
                tmpq[1][ix][ih] = a*tmp[1][ix][ih] + b3*(tmp[0][ix][ih]+tmp[2][ix][ih]) + 2.0*d3*(tmp[3][ix][ih]);
                tmpq[nzb-2][ix][ih] = a*tmp[nzb-2][ix][ih] + b3*(tmp[nzb-1][ix][ih]+tmp[nzb-3][ix][ih]) + 2.0*d3*(tmp[nzb-4][ix][ih]);
                tmpq[nzb-1][ix][ih] = a*tmp[nzb-1][ix][ih] + 2.0*b3*(tmp[nzb-2][ix][ih]) + 2.0*d3*(tmp[nzb-3][ix][ih]);
	...
	sf_warning("    tmp=          %f           ;",curr[50][0][100]);
    }
    sf_warning(".");
