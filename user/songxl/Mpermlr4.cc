...
static float dt, kh, kx, kz, kh1, kx1;
static int nh,nx,nz;
...
// assuming dx=dh
{
	...
	for(int b=0; b<nc; b++) { /* wavenumber index */
	    ...
	    if (0==equation || 1==equation || 4==equation) {
		...
		case 2: // tariq's
		    ...
		    break;
		case 4: // tariq's
                       phi = (kh*kh+8*kh*kz+8*kz*kz)*(kh*kh+8*kh*kz+8*kz*kz)*(kx+kz)*vp2/(4.0*(4*kz*(kh+2*kz)+kh*kh1*kx1)*(4*kz*(kh+2*kz)+kh*kh1*kx1)+1.0e-7);	
/*
                    if (kx < kh) {
                       phi = (kh*kh+8*kh*kz+8*kz*kz)*(kh*kh+8*kh*kz+8*kz*kz)*(kx+kz)*vp2/(4.0*(4*kz*(kh+2*kz)+kh*kh1*kx1)*(4*kz*(kh+2*kz)+kh*kh1*kx1));	
                    }
                    else
                    phi = (kh*kh+8*kh*kz+8*kz*kz)*(kh*kh+8*kh*kz+8*kz*kz)*(kx+kz)*vp2/(4.0*(4.*kz*(kh+2.0*kz)+kh*kh)*(4.*kz*(kh+2.0*kz)));
*/
		    break;
...
{   
    ...
    iC( lowrank(m,n,sample,(float) tol,npk,lidx,ridx,mid) );
