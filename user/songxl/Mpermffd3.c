...
*/
   ...
   if (NULL != snaps) {
	...
	//for (iz=0; iz < nzb; iz++) {
		...
		for (ih=0; ih < nhb; ih++) {
		    kh = ih*dkh*2.0*pi;
                    //if(kz < epsilon*0.001) { tmp[iz][ix][ih] = 0. ; continue;}
                   // if(kz < sqrtf(kx*kh)) { tmp[iz][ix][ih] = 0. ; continue;}
                    if(kz < sqrtf(kx*kh) || kz < epsilon*0.001) { ih=nhb;continue;}
        //            if(kz < sqrtf(kx*kh)) { ih = nh; continue;}
		    ...
		    /*curr[iz][ix][ih] = 2*cosf(v*sqrtf(x*h))*c - prev[iz][ix][ih];*/
                 //  if(kz < sqrtf(kx*kh) || kz < epsilon*0.0001) { tmp[iz][ix][ih]=0.0; continue;}
	              //if(kz < sqrtf(kx*kh))
                       // { ih = nh; continue;}
