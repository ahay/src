...
*/
    ...
    for (iz=0; iz < nzb; iz++) {
        vz = v[iz];
        b3[iz][0][0] = vz*vz*(dt*dt*(vz*vz-v0*v0)-16.0*dz*dz)/(12.0*v0*v0*dz*dz*dz*dz);
        d3[iz][0][0] = vz*vz*(dt*dt*(v0*v0-vz*vz)+4.0*dz*dz)/(48.0*v0*v0*dz*dz*dz*dz);
        a[iz][0][0] = -2.0*b3[iz][0][0]-2.0*d3[iz][0][0];
/*
        b3[iz][0][0] = 0.; 
        d3[iz][0][0] = 0.;
        a[iz][0][0] = -2.0*b3[iz][0][0]-2.0*d3[iz][0][0];
*/
        for (ih=1; ih < nh; ih++) {
            kh = ih*dkh*2.0*pi;
            b3[iz][0][ih] = -sinf(kh*dt*vz/4.0)*(3.0*dt*vz*cosf(kh*dt*vz/4.0)+(4.0*kh*dz*dz-3.0*dt*v0*cosf(kh*dt*v0/4.0)/sinf(kh*dt*v0/4.0))*sinf(kh*dt*vz/4.0))/(3.0*kh*dz*dz*dz*dz*sinf(kh*dt*v0/4.0)*sinf(kh*dt*v0/4.0));
            d3[iz][0][ih] = sinf(kh*dt*vz/4.0)*(3.0*dt*vz*cosf(kh*dt*vz/4.0)+(kh*dz*dz-3.0*dt*v0*cosf(kh*dt*v0/4.0)/sinf(kh*dt*v0/4.0))*sinf(kh*dt*vz/4.0))/(12.0*kh*dz*dz*dz*dz*sinf(kh*dt*v0/4.0)*sinf(kh*dt*v0/4.0));
/*
            b3[iz][0][ih] = 0.;
            d3[iz][0][ih] = 0.;
*/
            a[iz][0][ih] = -2.0*(b3[iz][0][ih]+d3[iz][0][ih]);
        }
        for (ix=1; ix < nx; ix++) {
	    kx = ix*dkx*2.0*pi;
            /*b3[iz][ix][0] = -1.0/(sinf(kx*dt*v0/4.0)*sinf(kx*dt*v0/4.0)*6.0*kx*dz*dz*dz*dz)*((8.0*kx*dz*dz-6.0*dt*v0*1.0/tanf(kx*dt*v0/4.0))*sinf(kx*dt*vz/4.0)*sinf(kx*dt*vz/4.0)+3.0*dt*vz*sinf(kx*dt*vz/2.0));
            d3[iz][ix][0] = 1.0/(sinf(kx*dt*v0/4.0)*sinf(kx*dt*v0/4.0))*sinf(kx*dt*vz/4.0)*(3.0*dt*vz*cosf(kx*dt*vz/4.0)+(kx*dz*dz-3.0*dt*v0*1.0/tanf(kx*dt*v0/4.0))*sinf(kx*dt*vz/4.0))/(12.0*kx*dz*dz*dz*dz);*/
            b3[iz][ix][0] = -sinf(kx*dt*vz/4.0)*(3.0*dt*vz*cosf(kx*dt*vz/4.0)+(4.0*kx*dz*dz-3.0*dt*v0*cosf(kx*dt*v0/4.0)/sinf(kx*dt*v0/4.0))*sinf(kx*dt*vz/4.0))/(3.0*kx*dz*dz*dz*dz*sinf(kx*dt*v0/4.0)*sinf(kx*dt*v0/4.0));
            d3[iz][ix][0] = sinf(kx*dt*vz/4.0)*(3.0*dt*vz*cosf(kx*dt*vz/4.0)+(kx*dz*dz-3.0*dt*v0*cosf(kx*dt*v0/4.0)/sinf(kx*dt*v0/4.0))*sinf(kx*dt*vz/4.0))/(12.0*kx*dz*dz*dz*dz*sinf(kx*dt*v0/4.0)*sinf(kx*dt*v0/4.0));
/*
            b3[iz][ix][0] = 0.;
            d3[iz][ix][0] = 0.;
*/
            a[iz][ix][0] = -2.0*(b3[iz][ix][0]+d3[iz][ix][0]);
            for (ih=1; ih < nh; ih++) {
		kh = ih*dkh*2.0*pi;
                b3[iz][ix][ih] = sinf((kx+kh)*dt*vz/4.0)/(2.0*dz*dz*sinf((kx+kh)*dt*v0/4.0)*sinf((kx+kh)*dt*v0/4.0)*sinf(sqrtf(kx*kh)*dz))*(-2.0*sqrtf(kx*kh)*dz*sinf((kx+kh)*dt*vz/4.0)-cosf(sqrtf(kx*kh)*dz)/(sinf(sqrtf(kx*kh)*dz)*sinf(sqrtf(kx*kh)*dz)*(kx+kh))*(-2.0*sqrtf(kx*kh)*(kx+kh)*dz*cosf(sqrtf(kx*kh)*dz)*sinf((kx+kh)*dt*vz/4.0)+((kx+kh)*cosf((kx+kh)*(v0-vz)*dt/4.0)-(kx+kh)*cosf((kx+kh)*(v0+vz)*dt/4.0)+kx*kh*dt*((v0+vz)*sinf((kx+kh)*(v0-vz)*dt/4.0)+(vz-v0)*sinf((kx+kh)*(v0+vz)*dt/4.0)))*sinf(sqrtf(kx*kh)*dz)/sinf((kx+kh)*dt*v0/4.0)));
                d3[iz][ix][ih] = sqrtf(kx*kh)*sinf((kx+kh)*dt*vz/4.0)/(8.0*(kx+kh)*dz*dz*sinf((kx+kh)*dt*v0/4.0)*sinf((kx+kh)*dt*v0/4.0)*sinf(sqrtf(kx*kh)*dz)*sinf(sqrtf(kx*kh)*dz)*sinf(sqrtf(kx*kh)*dz))*(-2.0*(kx+kh)*dz*cosf(sqrtf(kx*kh)*dz)*sinf((kx+kh)*dt*vz/4.0)+((kx+kh)*cosf((kx+kh)*dt*(v0-vz)/4.0)-(kx+kh)*cosf((kx+kh)*dt*(v0+vz)/4.0) 
+kx*kh*dt*((v0+vz)*sinf((kx+kh)*dt*(v0-vz)/4.0)+(-v0+vz)*sinf((kx+kh)*dt*(v0+vz)/4.0)))*sinf(sqrtf(kx*kh)*dz)/(sqrtf(kx*kh)*sinf((kx+kh)*dt*v0/4.0)));
                a[iz][ix][ih] = kh*kx*sinf(((kh+kx)*dt*vz)/4.0)*sinf(((kh+kx)*dt*vz)/4.0)/(sinf(((kh+kx)*dt*v0)/4.0)*sinf(((kh+kx)*dt*v0)/4.0))-2.0*b3[iz][ix][ih]*cosf(sqrtf(kx*kh)*dz)-2.0*d3[iz][ix][ih]*cosf(2.0*sqrtf(kx*kh)*dz); 
            } 
        } 
    }
