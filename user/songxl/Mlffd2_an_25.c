...
int main(int argc, char* argv[]) 
{
    ...
    float dt, dx, dkx, kx, dz, dkz, kz, knx, knz, tmpdt, pi=SF_PI, o1, o2, kx0, kz0;
    ...
    velx = sf_input("velx");   /* horizontal velocity */
    ...
    if (SF_FLOAT != sf_gettype(yita)) sf_error("Need float input");
...
/*
    ...
    yi = sf_floatalloc2(nz,nx);
    se = sf_floatalloc2(nz,nx);
    sf_floatread(vz[0],nz*nx,velz);
    sf_floatread(vx[0],nz*nx,velx);
    sf_floatread(yi[0],nz*nx,yita);
    sf_floatread(se[0],nz*nx,seta);
    vx0 =0.0;
    vz0 =0.0;
    yi0 =0.0;
    se0 =0.0;
...
/*
            if(k2==0 || tmpk < err) 
              tmpdt = - (vz0*dt)*(vz0*dt);
              //tmpdt = 2.0*(cosf(vk*dt)-1.0)/(k2+err);
            else if(tmpk<0.1) 
              tmpdt = -(tmpk*tmpk-tmpk*tmpk*tmpk*tmpk/12.0)/k2;
            else 
              tmpdt = 2.0*(cosf(vk*dt)-1.0)/k2;
*/
              tmpdt = 2.0*(cosf(vk*dt));

              tmpdt = tmpdt * dehf(fabsf(kx1),knx,ax,factor)*dehf(fabsf(kz1),knz,az,factor);
            fcos[ikx][ikz] = tmpdt;
         }
    }
