...
//    if (SF_FLOAT != sf_gettype(inp)) sf_error("Need float input");
    ...
    d3    =  sf_floatalloc2(nz,nx);
    d4    =  sf_floatalloc2(nz,nx);
    d5    =  sf_floatalloc2(nz,nx);
    d6    =  sf_floatalloc2(nz,nx);
    d7    =  sf_floatalloc2(nz,nx);
    d8    =  sf_floatalloc2(nz,nx);
    ...
    sf_floatread(d3[0],nz*nx,G);
    sf_floatread(d4[0],nz*nx,G);
    sf_floatread(d5[0],nz*nx,G);
    sf_floatread(d6[0],nz*nx,G);
    sf_floatread(d7[0],nz*nx,G);
    sf_floatread(d8[0],nz*nx,G);
    ...
    for (ix=0; ix < nx; ix++) {
        for (iz=0; iz < nz; iz++) {
            ...
            old[ix][iz] = 0.0; 
/*
            a[ix][iz] = -(b1[ix][iz]+b2[ix][iz]+c1[ix][iz]+c2[ix][iz]
                          +d1[ix][iz]+d2[ix][iz]+d3[ix][iz]+d4[ix][iz]
                          +d5[ix][iz]+d6[ix][iz]+d7[ix][iz]+d8[ix][iz]);
*/
        }
    }
    cur[isx][isz] = wav[0];
	 ...
	 for (ix=2; ix < nx-2; ix++) {  
	     for (iz=2; iz < nz-2; iz++) {  
                 nxt[ix][iz]  = cur[ix][iz]*(a[ix][iz])
                              ...
                              +0.5*(cur[ix+1][iz+2]+cur[ix-1][iz-2])*d3[ix][iz]
                              +0.5*(cur[ix+1][iz-2]+cur[ix-1][iz+2])*d4[ix][iz]
                              +0.5*(cur[ix+2][iz+1]+cur[ix-2][iz-1])*d5[ix][iz]
                              +0.5*(cur[ix+2][iz-1]+cur[ix-2][iz+1])*d6[ix][iz]
                              +0.5*(cur[ix+2][iz+2]+cur[ix-2][iz-2])*d7[ix][iz]
                              +0.5*(cur[ix+2][iz-2]+cur[ix-2][iz+2])*d8[ix][iz]
                              - old[ix][iz];
/*
                 nxt[ix][iz] = 0.5*(cur[ix][iz-1]+cur[ix][iz+1]-2.0*cur[ix][iz])*b1[ix][iz]
                              ...
                              +0.5*(cur[ix+1][iz+2]+cur[ix-1][iz-2]-2.0*cur[ix][iz])*d3[ix][iz]
                              +0.5*(cur[ix+1][iz-2]+cur[ix-1][iz+2]-2.0*cur[ix][iz])*d4[ix][iz]
                              +0.5*(cur[ix+2][iz+1]+cur[ix-2][iz-1]-2.0*cur[ix][iz])*d5[ix][iz]
                              +0.5*(cur[ix+2][iz-1]+cur[ix-2][iz+1]-2.0*cur[ix][iz])*d6[ix][iz]
                              +0.5*(cur[ix+2][iz+2]+cur[ix-2][iz-2]-2.0*cur[ix][iz])*d7[ix][iz]
                              +0.5*(cur[ix+2][iz-2]+cur[ix-2][iz+2]-2.0*cur[ix][iz])*d8[ix][iz]
