...
//    for(int k=0; k<Bc._m*Bc._n; k++) Bc._data[k]=cos(2.0*pi*(Bc._data[k]+Bxc._data[k]));
    ...
    FltNumMat IBc(nxz,LEN);    iC( pinv(Bc, 1e-16, IBc) );
    FltNumMat coef(ridx.size(),LEN);
//    FltNumMat M2c;
//    iC( sample(ridx,ksc,M2c) );
    
    iC(dgemm(1.0,M2,IBc,0.0,coef));
