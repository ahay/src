...
int main(int argc, char* argv[]) 
{
    ...
    int i;
    int nr, jr, r0, tl, jm ;
    float ax, az, factorx,factorz, kxm, kzm;
    ...
    if (!sf_getfloat("ax",&ax)) ax= 1.0; /*suppress HF parameter*/
    if (!sf_getfloat("az",&az)) az= 1.0; /*suppress HF parameter*/
    if (!sf_getfloat("factorz",&factorz)) factorz= 0.8; /*suppress HF parameter*/
    if (!sf_getfloat("factorx",&factorx)) factorx= 0.8; /*suppress HF parameter*/
...
#pragma omp parallel
    ...
    for (ikz=0; ikz < nkz; ikz++) {
        ...
        for (ikx=0; ikx < nkx; ikx++) {
            ...
            tmpdt = 2.0*(cosf(tmpvk)-1.0)*dehf(kx,kxm,ax,factorx)*dehf(kz,kzm,az,factorz);
            fcos[ikz][ikx] = tmpdt;
        }
    }
