/* 2-D 4th-order Fourier finite-difference wave extrapolation */
/*
...
int main(int argc, char* argv[]) 
{
    ...
    float vk, vk2, tmpvk, k2, err, dt2, kx1, kz1, kmax1, kmax2, p, dt4;
...
//    if (SF_FLOAT != sf_gettype(inp)) sf_error("Need float input");
  ...
  //  if (!sf_histfloat(inp,"d2",&dt)) sf_error("No d2= in input");
    ...
    if (!sf_getfloat("err",&err)) err= dx*dx; /*decaying parameter*/
...
//    sf_putfloat(out,"o1",x0);
    ...
    dkz = 1./(nkz*dz);
    kz0 = -0.5/dz;
    kmax1 = dkx*2.0*pi*nkx/3.0;
    kmax2 = dkz*2.0*pi*nkz/3.0;
    ...
    h30 = sqrtf(-8.0*yi0*vx02*vz02*cosf(2.0*se0)*cosf(2.0*se0)/(1.0+2.0*yi0)+w30*w30);
    dt2 = dt*dt;
    dt4 = dt2*dt2;
    dx2 = dx*dx;
    dx4 = dx2*dx2;
...
//    err = dx*dx+dz*dz;
    ...
    for (iz=0; iz < nzb; iz++){
         ...
         h3 = sqrtf(-8.0*yi[iz][ix]*vx2*vz2*cosf(2.0*se[iz][ix])*cosf(2.0*se[iz][ix])/(1.0+2.0*yi[iz][ix])+w3*w3);
         aa[iz][ix][4] = 1.0/(12.0*dx2)+dt2/(4.0*dx4)*(-w1-h1+w10+h10)
         +p*dt4/(96.0*dx4)*(h1*h1-6*h1*h10+5*h10*h10+2*h1*w1-6*h10*w1+w1*w1-6*h1*w10+10*h10*w10-6*w1*w10+5*w10*w10)
         +p*dt2/(48.0*dx2)*(-w1-h1+w10+h10);
         aa[iz][ix][5] = 1.0/(12.0*dz2)+dt2/(4.0*dz4)*(-w2-h2+w20+h20)
         +p*dt4/(96.0*dz4)*(h2*h2-6*h2*h20+5*h20*h20+2*h2*w2-6*h20*w2+w2*w2-6*h2*w20+10*h20*w20-6*w2*w20+5*w20*w20)
         +p*dt2/(48.0*dz2)*(-w2-h2+w20+h20);
         aa[iz][ix][3] = 1.0/(12.0*dx2)+1.0/(12.0*dz2)-aa[iz][ix][4]*dx2/dz2-aa[iz][ix][5]*dz2/dx2-(dt2*(w3+h3-w30-h30))/(2.0*dx2*dz2)
         +p*dt4/(96.0*dx2*dz2)*((h3+w3)*(h3+w3)-6.0*(h3+w3)*(h30+w30)+5.0*(h30+w30)*(h30+w30))-p*dt2/(48.0*dz2)*(h1-h10+w1-w10)-p*dt2/(48.0*dx2)*(h2-h20+w2-w20);
         aa[iz][ix][1] = -1.0/dx2-2.0*aa[iz][ix][3]-4.0*aa[iz][ix][4]+p*dt2/(4.0*dx2)*(h1-h10+w1-w10);
         aa[iz][ix][2] = -1.0/dz2-2.0*aa[iz][ix][3]-4.0*aa[iz][ix][5]+p*dt2/(4.0*dz2)*(h2-h20+w2-w20);
         aa[iz][ix][0] = -2.0*aa[iz][ix][1]-2.0*aa[iz][ix][2]-4.0*aa[iz][ix][3]-2.0*aa[iz][ix][4]-2.0*aa[iz][ix][5]+p;
        }
      }
...
*/
         ...
         for (ikz=0; ikz < nkz; ikz++) {
             ...
             for (ikx=0; ikx < nkx; ikx++) {
                 ...
                 vk = sqrtf(vk2);
                 tmpdt = 2.0*(cosf(vk*dt));
              //   if(abs(k2) < 0.00000001) 
                   tmpdt /=(k2+err);
              //  else 
              //     tmpdt /= (k2);
                 if((kx1/kmax1)*(kx1/kmax1)+(kz1/kmax2)*(kz1/kmax2) > 1.)
                   tmpdt = 0.0;
                   //tmpdt = 2.0/(k2+err);
