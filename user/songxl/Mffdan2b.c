...
int main(int argc, char* argv[]) 
{
    ...
    float ***aa, w, w0, g11, g22, dx2, dz2, dxz, dxz1, dxz2, ct, cb, cl, cr; //top, bottom, left, right 
    float vk, vk2, tmpvk, k2, err;
...
//    sf_putfloat(out,"o1",x0);
    ...
    dxz1 = dx-dz;
    dxz2 = dx+dz;
    dxz  = 2.0*(dx2*dx2+dz2*dz2)-dxz1*dxz1*dxz1*dxz1-dxz2*dxz2*dxz2*dxz2;
    for (iz=0; iz < nzb; iz++){
         for (ix=0; ix < nxb; ix++) {
         ...
         g11 = vx2/vx02;
         g22 = vz2/vz02;
         aa[iz][ix][3] = (-dt*dt*(vx2+vz2+w)+dt*dt*(vx2+vz2+w)*(vx2+vz2+w)/(vx02+vz02+w0)-g11*dx2-g22*dz2)/dxz;
         aa[iz][ix][1] = -g11/dx2-2.0*aa[iz][ix][3];
         aa[iz][ix][2] = -g22/dz2-2.0*aa[iz][ix][3];
         aa[iz][ix][0] = -2.0*aa[iz][ix][1]-2.0*aa[iz][ix][2]-4.0*aa[iz][ix][3];
        }
      }
