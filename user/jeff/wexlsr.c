...
// Calculate a wavefield perturbation from a slowness perturbation
{
    int ix,iy,isc;
    float wo;
    sf_complex iwdz;
    ...
    // higher order terms (should be 5 for the different weights?)
    if(lsr->nsc>0) {
        ...
        for( isc=1; isc<=lsr->nsc; isc++) {
            KOOP( lsr->wk[ompith][iy][ix] = sf_cmplx(0.,0.); );  // Initialize
            LOOP( lsr->wk[ompith][iy][ix] = lsr->wt[ompith][iy][ix]; ); // Set weighting function

			// FFT
            wexfft(false,(kiss_fft_cpx**) lsr->wk[ompith],lsr->f2d[ompith]);


			// Apply Fourier weighting
#ifdef SF_HAS_COMPLEX_H
            KOOP( lsr->wk[ompith][iy][ix] *= lsr->kw[iy][ix] * pow(lsr->kk[iy][ix],isc); );
#else
            KOOP( lsr->wk[ompith][iy][ix] = sf_crmul(lsr->wk[ompith][iy][ix],
                                        lsr->kw[iy][ix] * pow(lsr->kk[iy][ix],isc)); );
#endif

            // IFT  - Bring results back to space
            wexfft(true,(kiss_fft_cpx**) lsr->wk[ompith],lsr->f2d[ompith]);

			// Get Scattering operators (ws)^-2
#ifdef SF_HAS_COMPLEX_H
            LOOP( lsr->wk[ompith][iy][ix] *= pow(wo*slo->s[iz][ lsr->ly[iy] ][ lsr->lx[ix] ],-2*isc); );
            
            //Wavefield perturbation for higher order
            LOOP( pw[iy][ix] += lsr->wk[ompith][iy][ix] * lsr->csc[isc]; );
#else
...
/* Calculate a slowness perturbation from a wavefield perturbation*/
{
    int ix,iy,isc;
    float wo;
    sf_complex iwdz;
    ...
    if(lsr->nsc>0) {
        ...
        for( isc=1; isc<=lsr->nsc; isc++) {
           ...
           wexfft(true,(kiss_fft_cpx**) lsr->wk[ompith],lsr->f2d[ompith]);
            ...
            LOOP( ps[iy][ix] += lsr->wk[ompith][iy][ix] * lsr->csc[isc]; );
#else
