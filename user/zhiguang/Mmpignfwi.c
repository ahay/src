...
int main(int argc, char* argv[])
{
	...
	sf_file Fv, Fw, Fdat, Finv=NULL, Ferr=NULL, Fmod=NULL, Fgrad;
	...
	/* get prepared */
	preparation_gn(Fv, Fw, acpar, soupar, array);
	...
	if(function == 1){ // forward modeling
		...
		forward_modeling_gn(Fdat, &mpipar, soupar, acpar, array, verb);
		...
		if(!sf_getint("lniter", &fwipar->lniter)) fwipar->lniter=10; /* CG iteration number */
		...
		sf_putint(Fgrad, "n3", 2); /* gradient & GN perturbation */
		sf_putfloat(Fgrad, "d3", 1);
		sf_putfloat(Fgrad, "o3", 0);
		...
		if(Finv != NULL){
			...
			sf_putstring(Ferr, "label1", "Iteration");
		...
		gnfwi(Fdat, Finv, Ferr, Fmod, Fgrad, &mpipar, soupar, acpar, array, fwipar, optpar, verb);
