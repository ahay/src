...
int main(int argc, char* argv[])
{
    bool verb;
    int ix, iz;
    int nx, nz, nt, n0, nxz, nxt, padx, padz, n2, padnx, padnz, niter, rect1, rect2, order;
    float dx, dz, dt, dt2, lambda;
    
    int *head;
    float *mm, *dd;
    float **vv, **padvv, **dp;
    
    sf_file in, out, vel, mask, dip;
    sf_init(argc, argv);
    ...
    ptshape_init(nx, nz, nt, verb, n0, padx, padz, padnx,
                    padnz, dx, dz, head, padvv, rect1, rect2, order, dp, lambda);                    
    ptshape(niter, nxz, nxt, mm, dd);
    ptshape_close();
