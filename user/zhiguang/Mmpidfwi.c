...
int main(int argc, char* argv[])
{
	...
	sf_file Fv, Fd, Fw, Fdat, Fimg, Fgrad, Finv=NULL, Ferr=NULL;
	...
	if(!sf_getfloat("coef", &acpar->coef)) acpar->coef=3.; /* maximum velocity of the medium */
	...
	if(function == 1){ // forward modeling
		...
		forward_modeling_d(Fdat, &mpipar, soupar, acpar, array, verb);
...
//		if(!sf_getint("para_type",&fwipar->para_type)) fwipar->para_type=1;
//		/* if 1, velocity and impedance; if 2, velocity and density */
