...
int main(int argc, char* argv[]) 
{
 ...
 vs0 =0.0;
    ...
    for (ix=0; ix < nxb; ix++) {
         for (iz=0; iz < nzb; iz++) {
         w = vp[ix][iz]*vp[ix][iz];
         g1 = dt*dt*(vp[ix][iz]*vp[ix][iz]-vp0*vp0)/(12.0*dz*dz);
         g2 = dt*dt*(vp[ix][iz]*vp[ix][iz]-vp0*vp0)/(12.0*dx*dx);
         aa[1][ix][iz] = w*g1;
         aa[2][ix][iz] = w*g2;
    ...
    /* propagation in time */
    psstep2e_init(nzb,nxb,dz,dx,opt);
    ...
    for (it=0; it < nt; it++) {
        ...
        psstep2e(upold, upcur, wpold, wpcur, usold, uscur, wsold, wscur, uu, ww, nzb, nxb, dz, dx, vp0, vs0, vp, vs, dt); 
