...
//	p=alloc3dfloat(nz2,nx2,ny2);
	p= sf_floatalloc3(nz2,nx2,ny2);
	...
	for (iy=0; iy < ny; iy++) {
		for (ix=0; ix < nx; ix++) {
			for (iz=0; iz < nz; iz++) {
				p[iy+np][ix+np][iz+np] = init_model[iy][ix][iz];
			}
		}
	}
	...
	for (iy=0; iy < ny2; iy++) {
		for (ix=0; ix < nx2; ix++) {
			for (iz=0; iz < np; iz++) {
				p[iy][ix][iz] = p[iy][ix][np];
				p[iy][ix][nz2-iz-1] = p[iy][ix][nz2-np-1];
			}
		}
	}
	...
	for (iy=0; iy < ny2; iy++) {
		for (ix=0; ix < np; ix++) {
			for (iz=0; iz < nz2; iz++) {
				p[iy][ix][iz] = p[iy][np][iz];
				p[iy][nx2-ix-1][iz] = p[iy][nx2-np-1][iz];
			}
		}
	}
	...
	for (iy=0; iy < np; iy++) {
		for (ix=0; ix < nx2; ix++) {
			for (iz=0; iz < nz2; iz++) {
				p[iy][ix][iz] = p[np][ix][iz];
				p[ny2-iy-1][ix][iz] = p[ny2-np-1][ix][iz];
			}
		}
	}
