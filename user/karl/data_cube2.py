...
class PlotFrame(DemoFrame):
    ...
    def _update_images(self):
        ...
        # row-major order
        slicexy=transpose(cube[:, :, self.slice_z], (1,0,2))
        print "slicexy.shape=",slicexy.shape
        pd.set_data("xy", slicexy)
        slicexz=transpose(cube[:, self.slice_y, :], (1,0,2))
        print "slicexz.shape=",slicexz.shape
        pd.set_data("xz", slicexz)

        sliceyz=cube[self.slice_x, :, :]
        print "sliceyz.shape=",sliceyz.shape
        pd.set_data("yz", sliceyz)
