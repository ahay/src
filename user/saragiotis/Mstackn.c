...
int main (int argc, char* argv[])
{
    ...
    for (i3=0; i3<n3; i3++) {
		...
		if (i3==0 || nm2>1) {	
			if (minflag) {	// read mask values and transform them to indices
				sf_floatread(min,n1,minin);
				for (i1=0; i1<n1; i1++) {
					min[i1] = SF_MIN(SF_MAX(0,min[i1]), o2+(n2-1)*d2);
					im1[i1] = roundf(min[i1]/d2);
				}
			} else {
				for (i1=0; i1<n1; i1++) {
					im1[i1] = 0;
			...
			if (maxflag) {	// read mask values and transform them to indices
				sf_floatread(max,n1,maxin);
				for (i1=0; i1<n1; i1++) {
					max[i1] = SF_MIN(SF_MAX(0,max[i1]), o2+(n2-1)*d2);
					im2[i1] = roundf(max[i1]/d2);
				}
			} else {
				for (i1=0; i1<n1; i1++) {
					im2[i1] = n2-1;
