...
int main(int argc, char* argv[])
{
    ...
    float *trace, **image, **v, rho, **stack, *pp, *off, s, ssq;// s is a heterogeneity parameter in shifted hyperbola approximation
    ...
    if (!sf_getfloat("s",&s)) s = 1.; //create a case, which tests it is within reasonable range
    /* Heterogeneity coefficient in shifted hyperbola travel-time approximation */
	...
	for (iy=0; iy < nx; iy++) { 
		...
		for (it=0; it < nt; it++) {
		    t = t0 + it*dt;
                    //t = t*2; 
		    vi = v[ix][it]/2; //zero-offset exploding reflector
			...
			t1 = t*(1 - (1/s));//
			ssq = sqrt(s);
			t2 = hypotf(t,ssq*(x-h)/vi)/s; // 1/s*sqrt(t^2 + s*(x0-x)^2/v^2)
			ti = t1+t2; // since this is two-way travel-time

		    /* tx = |dt/dx| */
		    //tx = fabsf(x-h)/(vi*vi*(t1+dt))+
		         //fabsf(x+h)/(vi*vi*(t2+dt));

			tx = fabsf(x-h)/(vi*vi*(t2*s+dt));
