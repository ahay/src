...
static void pick(bool adj, float ti, float deltat, 
		 float *trace, float *out, double *dstack, double *dstack2, int *folds, int i,
...
int main(int argc, char* argv[])
{
    ...
    int nt, nx, nh, nh2, ix, ih, iy, i, nn, it, **fold, apt, **folds;
    float *trace, **image, **v, rho, **stack, *pp, *off, *semb;
    double *dstack, *dstack2, num, den;
    float h, x, t, h0, dh, dx, ti, tx, t0, t1, t2, dt, vi, aal, angle;
    sf_file inp, out, vel, gather, offset, semblance;

    sf_init (argc,argv);
    inp = sf_input("in");
    vel = sf_input("vel");  
    out = sf_output("out");
    semblance = sf_output("semblance");
    ...
    dstack = (double*) sf_alloc(nt*nx,sizeof(double)); 
    dstack2 = (double*) sf_alloc(nt*nx,sizeof(double));
    semb = sf_floatalloc(nx*nt);

    folds = sf_intalloc2(nt,nx); 
	...
	for (iy=0; iy < nx; iy++) { 
		...
		for (it=0; it < nt; it++) {
		    ...
		    pick(adj,ti,fabsf(tx*dx*aal),trace,image[ix],&dstack[nt*ix],&dstack2[nt*ix],folds[ix],it,nt,dt,t0);

                    if(folds[ix][it] > 0){

                    	num = dstack[nt*ix + it]/folds[ix][it];
                    	num *= num;
                    	den = dstack2[nt*ix + it]/folds[ix][it];
                    	semb[nt*ix + it] = (den > 0.0f)? num/den : 0.0f;
    ...
    if (adj) sf_floatwrite(semb,nt*nx,semblance); 
