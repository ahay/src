...
//---------------------------------------
int BFIO::eval34(int N, const CpxNumTns& f, const FltNumVec& w, const FltNumVec& x, const FltNumVec& y, CpxNumTns& u, const FltNumVec& tau, const FltNumVec& p, const FltNumVec& q, const float xx)
{
  //---------------
  clock_t time0 = clock();  
  //--------------
	    ...
	    for(int k2=int(round(k2stt/kB2)); k2<int(round(k2end/kB2)); k2++) 
	        ...
	        for(int x1=0; x1<nx1; x1++)
			  ...
			  src[g] = ks(gud[g](0),gud[g](1),gud[g](2));
		        CpxNumMat scl;    iC( kernel34(N, trg, src, scl, xx) );
		        for(int g=0; g<int(gud.size()); g++)
		          ext(g) = ext(g) * scl(0,g);
		        //transform
	  ...
	  for(int k1=int(round(k1stt/kB1)); k1<int(round(k1end/kB1)); k1++)
		...
		Point3 kc( (k1+0.5)*kB1, (k2+0.5)*kB2, (k3+0.5)*kB3 );
			...
			trg[g] = ewmul(trg[g] + Point3(x1,x2,x3), Point3(xB1,xB2,xB3));
		      CpxNumMat evl(NGx1*NGx2*NGx3,NGk1*NGk2*NGk3);    iC( kernel34(N, trg, src, evl, xx) );
