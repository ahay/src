...
{   
    ...
    // first apply half prop* to input get output1
    sf_complex *copt1;
    copt1 = sf_complexalloc(nzx);
    if (flag==1) {
	prop1P( cipt, copt1, clt, crt, nz, nx, nkzx, m2);  
    } else if (flag==2) {
	prop2N( cipt, copt1, clt, crt, nz, nx, nkzx, m2);  
    } else if (flag==3) {
	prop1Pa( cipt, copt1, clt, crt, nz, nx, nkzx, m2);  
    } else if (flag==4) {
	prop2Na( cipt, copt1, clt, crt, nz, nx, nkzx, m2);  
    } else {
    ...
    // finally apply half prop to output1 to get output3, output2 to get output4
    sf_complex *copt3, *copt4;
    copt3 = sf_complexalloc(nzx); 
    copt4 = sf_complexalloc(nzx);       
    if (flag==1) {
	prop1Pa( copt1, copt3, clt, crt, nz, nx, nkzx, m2);  
        prop1Pa( copt2, copt4, clt, crt, nz, nx, nkzx, m2);  
    } else if (flag==2) {
	prop2Na( copt1, copt3, clt, crt, nz, nx, nkzx, m2);  
        prop2Na( copt2, copt4, clt, crt, nz, nx, nkzx, m2);  
    } else if (flag==3) {
	prop1P( copt1, copt3, clt, crt, nz, nx, nkzx, m2);  
        prop1P( copt2, copt4, clt, crt, nz, nx, nkzx, m2);  
    } else if (flag==4) {
	prop2N( copt1, copt3, clt, crt, nz, nx, nkzx, m2);  
        prop2N( copt2, copt4, clt, crt, nz, nx, nkzx, m2);  
    } else {
	cerr<<"Need to provide flag#"<<endl;
    }


    // Write output1, ouput2, output3, output4
    std::valarray<sf_complex> opt1(nzx), opt2(nzx), opt3(nzx), opt4(nzx);
    for (int i=0; i<nzx; i++) {
        opt1[i] = copt1[i];
        opt2[i] = copt2[i];
	opt3[i] = copt3[i];
    ...
    oRSF output1, output2("output2"), output3("output3"), output4("output4");
    ...
    output3.type(SF_COMPLEX);
    output3.put("n1",nz);
    output3.put("n2",nx);
    output3 << opt3;

    output4.type(SF_COMPLEX);
    output4.put("n1",nz);
    output4.put("n2",nx);
    output4 << opt4;
