//   check <f1,f2>?=<g1,g2>
//   input f1(t,x), f2(t,x) complex
//   input g1(t,x), g2(t,x) complex
//
...
int main(int argc, char** argv)
{
  ...
  std::valarray<sf_complex> f1data(nt1*nx1);
  std::valarray<sf_complex> f2data(nt2*nx2);
  ...
  for (int i=0; i<nt1; i++)
    for (int j=0; j<nx1; j++) {
      double a=crealf(f1data[nt1*j+i]);
      double b=cimagf(f1data[nt1*j+i]);
      double c=crealf(f2data[nt1*j+i]);
      double d=cimagf(f2data[nt1*j+i]);
  ...
  std::valarray<sf_complex> g1data(nt3*nx3);
  std::valarray<sf_complex> g2data(nt4*nx4);
  ...
  for (int i=0; i<nt3; i++)
    for (int j=0; j<nx3; j++) {
      double a=crealf(g1data[nt3*j+i]);
      double b=cimagf(g1data[nt3*j+i]);
      double c=crealf(g2data[nt3*j+i]);
      double d=cimagf(g2data[nt3*j+i]);
