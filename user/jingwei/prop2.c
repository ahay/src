...
int prop2(sf_complex *input, sf_complex *output, sf_complex *lt, sf_complex *rt, int nz, int nx, int nkzx, int m2, float reg)
...
#else
		    c = sf_cadd(c,sf_cmul(conjf(lt[im*nzx+i]), wave2[im][j]));
#endif
		}
		curr[j] = c;
	    }
...
#else
		c = sf_cadd(c,sf_cmul(wave[im][ik],rt[ik*m2+im]));
#endif
	    }
	    cwavem[ik] = c;
	}
