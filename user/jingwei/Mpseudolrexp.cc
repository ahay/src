//   Generate rank-1 approximation for lowrank wave propagation: lrexp
...
static int nt, nz, nx, ntx, nzx, nkz, nkx, nkzx, m2, flag, gpz;
static float dt, dz, dx, t0, z0, x0, dkz, dkx, kz0, kx0;
...
int sample(vector<int>& rs, vector<int>& cs, CpxNumMat& res)
{
	...
	for(int b=0; b<nc; b++) {
	    ...
	    sf_complex *cin, *cout, *ctmp;
	    cin = sf_complexalloc(nzx);
	    ctmp = sf_complexalloc(ntx);
