...
//---------------------------------------
int BFIO::apeval2(int N, const CpxNumMat& f, const FltNumVec& w, const FltNumVec& x, CpxNumMat& u, const FltNumVec& tau, const FltNumVec& p, const float xx)
{
  int N1 = f.m();
  int N2 = f.n();
  int M1 = u.m();
  ...
  for(int z1=0; z1<nz1; z1++)
	...
	//
	  ...
	  for(int k2=int(round(k2stt/kB2)); k2<int(round(k2end/kB2)); k2++) {
		...
		CpxNumMat all(NGk1,NGk2);    setvalue(all, cpx(0,0));
		  ...
		  CpxNumMat scl;    iC( apkernel2(N, trg, src, scl, xx) );
		  for(int g=0; g<int(gud.size()); g++)
		    ext(g) = ext(g) * scl(0,g);
		  //transform
	...
	for(int k1=int(round(k1stt/kB1)); k1<int(round(k1end/kB1)); k1++)
	  for(int k2=int(round(k2stt/kB2)); k2<int(round(k2end/kB2)); k2++) {
		...
		CpxNumMat evl(NGx1*NGx2,NGk1*NGk2);    iC( apkernel2(N, trg, src, evl, xx) );
