/* Multi-dimensional missing data interpolation using shaping. */
/*
...
{
    int dim, na,ia, niter, n1, n2, maxlag, padin, padout, p1, p2, j, i, rect;
    float a0, *mm, *kk, *xx, eps, lam;
    bool exact, *known;
    ...
    /* shaping radius */
    if (!sf_getbool("exact",&exact)) exact=false;
    /* if keep the true values */
    ...
    sf_mask_init(known);
    hshape_init(n2,rect,aa,eps);
    sf_conjgrad_init(n2, n2, n2, n2, lam, 10*FLT_EPSILON, true, true); 

    sf_conjgrad(NULL,sf_mask_lop,hshape_lop,xx,mm,mm,niter);
