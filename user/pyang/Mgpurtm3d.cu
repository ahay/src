/* 3D GPU-based reverse time migration (RTM)
*/
/*
...
__global__ void cuda_apply_abc(float *p0, float *p1, float *vv, float dtz, float dtx, float dty, int n1, int n2, int n3)
/*< apply Clayton-Enquist absorbing boundary condition (ABC, A2)  >*/
{
	int id=threadIdx.x+blockIdx.x*blockDim.x;
	int nn1=n1+2;
	int nn2=n2+2;
	int nn3=n3+2;
	int iz, ix, iy, ioff;
	float v1, v2, v3, diff1, diff2, diff3;
	...
	if(id<4*(n1+n2+n3))
	{
		ioff=iz+nn1*ix+nn1*nn2*iy;
		v1=vv[ioff]*dtz;
		v2=vv[ioff]*dtx;
		v3=vv[ioff]*dty;

		if(iz==0){
			diff1=(p1[ioff+1]-p1[ioff])-(p0[ioff+1]-p0[ioff]);
			diff1*=v1;
		}else if(iz==nn1-1){
			diff1=-(p1[ioff]-p1[ioff-1])+(p0[ioff]-p0[ioff-1]);
			diff1*=v1;
		}else{// iz changes with thread
			diff1=p1[ioff-1]-2.0*p1[ioff]+p1[ioff+1];
			diff1*=0.5*v1*v1;
		}

		if(ix==0){
			diff2=(p1[ioff+nn1]-p1[ioff])-(p0[ioff+nn1]-p0[ioff]);
			diff2*=v2;
		}else if(ix==nn2-1){
			diff2=-(p1[ioff]-p1[ioff-nn1])+(p0[ioff]-p0[ioff-nn1]);
			diff2*=v2;
		}else {// ix changes with thread
			diff2=p1[ioff-nn1]-2.0*p1[ioff]+p1[ioff+nn1];
			diff2*=0.5*v2*v2;	
		}

		if(iy==0){
			diff2=(p1[ioff+nn1*nn2]-p1[ioff])-(p0[ioff+nn1*nn2]-p0[ioff]);
			diff2*=v2;
		}else if(iy==nn3-1){
			diff2=-(p1[ioff]-p1[ioff-nn1*nn2])+(p0[ioff]-p0[ioff-nn1*nn2]);
			diff2*=v2;
		}else{// iy changes with thread
			diff3=p1[ioff-nn1*nn2]-2.0*p1[ioff]+p1[ioff+nn1*nn2];
			diff3*=0.5*v3*v3;	
		}

		p0[ioff]=2.0*p1[ioff]-p0[ioff]+diff1+diff2+diff3;
	}
}
