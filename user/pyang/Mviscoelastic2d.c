/* 2D 8-th order visco-elastic wave propagation using sponge ABC
*/
/*
...
void forward_txx_tzz_txz(float **txx, float **tzz, float **txz, 
	float **rxx, float **rzz, float **rxz, float **uvx, float **uvz, 
	float **taup, float **taus, float **tau0, float **vp, float **vs)
/*< forward step: update txx, tzz, txz, rxx, rzz, rxz >*/
{
	int i1, i2;
	float diff1, diff2, diff3, diff4, tmp;

#ifdef _OPENMP
#pragma omp parallel for default(none) 			\
	private(i1,i2,diff1,diff2,diff3,diff4, tmp)	\
	shared(uvx,uvz,txx,tzz,txz,rxx, rzz, rxz, taup,taus,tau0, vp,vs,nxpad,nzpad,dt,_dx,_dz)
#endif
	...
	for(i1=3; i1<nzpad-4; i1++)
	{
		...
		txx[i2][i1]+=dt*(vp[i2][i1]*_dx*diff4*(1.+taup[i2][i1])
			+(vp[i2][i1]*(1.+taup[i2][i1])-2*vs[i2][i1]*(1.+taus[i2][i1]))*_dz*diff1);
		tzz[i2][i1]+=dt*(vp[i2][i1]*_dz*diff1*(1.+taup[i2][i1])
			+(vp[i2][i1]*(1.+taup[i2][i1])-2*vs[i2][i1]*(1.+taus[i2][i1]))*_dx*diff4);
		txz[i2][i1]+=dt*vs[i2][i1]*(1.+taus[i2][i1])*(_dz*diff2+_dx*diff3);
		tmp=dt/tau0[i2][i1];
		rxx[i2][i1]=( (1.-0.5*tmp)*rxx[i2][i1]-tmp*vs[i2][i1]*(_dx*diff4+_dz*diff1))/(1.+0.5*tmp);
		rzz[i2][i1]=( (1.-0.5*tmp)*rzz[i2][i1]-tmp*vs[i2][i1]*(_dz*diff1+_dx*diff4))/(1.+0.5*tmp);
		rxz[i2][i1]=( (1.-0.5*tmp)*rxz[i2][i1]-tmp*vs[i2][i1]*(_dz*diff2+_dx*diff3))/(1.+0.5*tmp);
	}
}
...
void variable_trans(float **vp, float **vs, float **rho)
...
int main(int argc, char* argv[])
{
	...
	float a, *wlt, *bndr;
	float **tmp, **vp, **vs, **rho, **uvx, **uvz, **txx, **tzz, **txz, **rxx, **rzz, **rxz, **taup, **taus, **tau0;
	sf_file Fvp, Fvs, Frho, Ftaup, Ftaus, Ftau0, Fwavx, Fwavz;
	...
	for(it=0; it<nt; it++)
	{
		...
		forward_txx_tzz_txz(txx, tzz, txz, rxx, rzz, rxz, uvx, uvz, taup, taus, tau0, vp, vs);
