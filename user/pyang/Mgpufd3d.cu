...
#pragma unroll 4
        ...
        float value=(_dz2+_dx2+_dy2)*stencil[0]*current;
#pragma unroll 4
        for (int i=1; i <= radius ; i++)
        {
	  value +=stencil[i]*(_dy2*(infront[i-1]  + behind[i-1]  )+ 
			      _dx2*(tile[t2-i][t1]+ tile[t2+i][t1])+ 
			      _dz2*(tile[t2][t1-i]+ tile[t2][t1+i]));
        }

        if (validw) p0[outIndex]=2.0*p1[outIndex]-p0[outIndex]+vv[outIndex]*value;
    }
}
