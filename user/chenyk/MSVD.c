...
Compute [U,O,V]=SVD(A), A=UOV, a little bit different from A=UoV';
 (without need of python api, numpy, scipy)*/
/*
...
{
    int i, n1, n2, n3, ka; /*n1 is trace length, n2 is the number of traces, n3 is the number of 3th axis*/
    sf_file in, out, outu, outv;
    ...
    sf_putint(out, "n1", n1);   /* out is o */  
    sf_putint(out, "n2", n2);
    sf_putint(out, "n3", n3);

    if(NULL!=sf_getstring("outu")) ifoutu=1; else ifoutu=0;
    if(NULL!=sf_getstring("outv")) ifoutv=1; else ifoutv=0;
	...
	sf_floatread(a, n1*n2, in);
        svduov(a, u, o, v, eps, ka); /* implement svd */
	sf_floatwrite(o, n1*n2, out);   /* output o */ 
