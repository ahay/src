/* 2D multisource coustic time-domain FD modeling for testing
...
int main(int argc, char* argv[])
{
   ...
   // if(! sf_getbool("expl",&expl)) expl=false; /* Multiple sources, one wvlt */
    ...
    as = sf_iaxa(Fsou1,2); sf_setlabel(as,"s"); if(verb) sf_raxa(as); /* sources */
...
//	ww = sf_floatalloc( 1);
  //  } else {
    ...
    ss1 = (pt2d*) sf_alloc(ns,sizeof(*ss1)); 
    ss2 = (pt2d*) sf_alloc(ns,sizeof(*ss2)); 
    rr = (pt2d*) sf_alloc(nr,sizeof(*rr)); 

    pt2dread1(Fsou1,ss1,ns,2); /* read (x,z) coordinates */
