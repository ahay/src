...
/*
  ...
  Modified by Yangkang Chen on Sep 13, 2021
...
int main (int argc,char* argv[]) 
{
	...
	if(!sf_histint(shots,"n2",&ndim)|| ndim !=3 ) 
	    sf_error("Need n2=3 in shotfile");
	if(!sf_histint(shots,"n1",&nshot)) 
	    sf_error("No n1= in shotfile");
  
  	ss = sf_floatalloc2 (nshot,ndim);
	s = sf_floatalloc2 (ndim,nshot);
	sf_floatread(ss[0],nshot*ndim,shots);
	...
	for(i1=0;i1<1;i1++)
		for(i2=0;i2<n2;i2++)
			for(i3=0;i3<n3;i3++)
				tt[i2+i3*n2]=t[i1+i2*n1+i3*n1*n2];
	sf_floatwrite (tt,n23,time);
    }
    sf_warning(".");
