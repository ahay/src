...
sf_cram_data2 sf_cram_data2_init (sf_file data, sf_file ddaemon, bool mtrace)
/*< Initialize object >*/
{
    size_t n, j, i0, i1;
    ...
    if (false == cram_data->mtrace) {
        ...
        if (NULL == ddaemon || 0 == cram_data->nc)
...
static size_t sf_cram_data2_access_trace (sf_cram_data2 cram_data, size_t i) {
    ...
    if (i < cram_data->i || i >= (cram_data->i + cram_data->ntr)) {
        ...
        if (local) {
            if (cram_data->nc)
                sf_warning ("Failed to receive trace %lu from a remote server, using local access",
                            i);
