...
namespace TSOpt {
  ...
  void GridDC::open_p() const {
	...
	*/
	string inkey="in";
	if (!(parse<string>(*par,inkey,datafile))) {
	  RVLException e;
	  e<<"Error: GridDC::open_p\n";
  ...
  GridDCF::GridDCF(string _protohdr, bool _incore, ostream & _outfile)
    ...
    // extract other metadata - defaults OK
    inkey="data_format";
    parse<string>(*par,inkey,data_format);
    inkey="scale";
    int scale = 0;
    parse<int>(*par,inkey,scale);
    inkey="data_type";
    parse<string>(*par,inkey,data_type);
