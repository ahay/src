...
void fputtr(FILE *fp, segy *tp)
/*< put a segy trace to a file by file pointer >*/
{
...
void fputtr_internal(FILE *fp, segy *tp, cwp_Bool fixed_length)
{
	...
	*oldinfoptr = (struct outsegyinfo *)
	...
	switch (infoptr->ftype = filestat(fileno(fp))) {
	    case DIRECTORY:
		sf_error("%s: segy output can't be a directory", __FILE__);
	    case TTY:
		sf_error("%s: segy output can't be tty", __FILE__);
	    default:
		/* the rest are ok */
		break;
	}
...
/*--------------------------------------------------------------------*\
  ...
  \*--------------------------------------------------------------------*/
	...
	sf_getint( "lheader" ,&out_line_hdr );

	if( out_line_hdr ){

	    if( in_line_hdr ){
		(void) fwrite(&(su_text_hdr[0]), 1 ,3200 
			      ,infoptr->outfp);
	...
	sf_error("%s: on trace #%ld, number of samples in header (%d) "
		 "differs from number for first trace (%d)", 
		 __FILE__, (infoptr->itr)+1, tp->ns, infoptr->nsfirst);
	

    (void) fwrite(tp, 1,HDRBYTES, infoptr->outfp);
