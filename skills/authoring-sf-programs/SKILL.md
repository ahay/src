...
| `.jl` | Julia |
| `.java` | Java |
| `.m` | Matlab / Octave (disambiguated at configure time by which API is enabled) |
| `.chpl` | Chapel |
...
For C, the scraper uses this regex (from `doc.py` line 842):
...
- **C++**: `// comment` lines at the top of the file; parameters from `par.get(...)` with `// desc`.
...
**Python**: `sys.stderr.write("msg\n"); sys.exit(1)` or raise an exception; the RSF Python API also provides `rsf.api.error()`.
...
**Chapel**: `halt("msg")` terminates with an error; the RSF Chapel module also wraps `sf_error`.
...
**Program-level tests** — in each user directory's `SConstruct`, test targets are SCons `Flow()` or `Command()` nodes. Running `scons test` in a directory (or globally) executes these flows. The test typically pipes synthetic data through the new program and checks the output.
...
- **Fortran 77** — legacy only; avoid for new programs.
- **Julia** — when your ecosystem is Julia or your collaborators prefer it; Julia programs are installed as scripts.
- **Matlab / Octave** — when the algorithm is already in `.m` form and conversion is not worth the effort; the build system uses configure flags to distinguish Matlab vs. Octave.
- **Java** — rarely used; follow the existing `user/*/M*.java` patterns.
