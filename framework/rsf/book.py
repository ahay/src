...
report = os.path.basename(os.getcwd()).upper()
...
def get_year(default):
    ...
    if not year:
        ...
        year = str(year)
        year = '-'.join([str(int(year)-1),year[-2:]])
    return year
...
def report_tpg(target=None,source=None,env=None):
    ...
    tpg.write('\\maintitle{%s}\n' % \
              env.get('group',full.get(group)).upper())
...
class RSFReport(Environment):
    ...
    def Papers(self,papers,**kw):
        ...
        if type(papers[0]) is tuple:
            ...
            papers = Split(' '.join([x[1] for x in papers]))
