...
/* Igor, 09/28/09. This was moved from iwave.h */
#ifdef IWAVE_USE_MPI

static const char *MODEL_ACTION_NAMES[MODEL_MAX_NACTION] = {
  "COMPUTE",
  "EXCHANGE"
};
  
#endif
/* Igor, 09/28/09. END (This was moved from iwave.h) */
/*----------------------------------------------------------------------------*/
/* 
...
void dump_rs(PARARRAY * pars, IMODEL * model, FILE * stream, int sends) {
  ...
  if ( dump ) {
    ...
    for ( i = 0; i < nnei; ++i ) {
      ...
      for ( j = 0; j < narr; ++j ) {
	fprintf(stream, "%d ", (*model).tsinfo.arrs[j]);
	rd_dump(rd + i, (*model).tsinfo.arrs[j], stream);
      }
      fprintf(stream, "-------------------\n");
    }
  }
...
/*----------------------------------------------------------------------------*/
/*
  Prepare data exchanges
*/
#ifdef IWAVE_USE_MPI
void prepexch(PARALLELINFO * pinfo, IMODEL * model,
	      int stats, 
	      double * wt0) {
  
  int na = (*model).tsinfo.narr;
  int nnei = (*model).nnei;
  int i, iv, ia;
  
  for ( i = 0; i < nnei; ++i ) {
    for ( iv = 0; iv < na; ++iv ) {
      ia = (*model).tsinfo.arrs[iv];
