...
extern "C" {
  ...
  */
   ...
   * base/include/utils.h.
   */
  int iwave_construct(IWAVE * state, PARARRAY * pars, FILE * stream,  
		      int (*minit)(PARARRAY *, FILE *, IMODEL *) );
  ...
  /** 
   * Reads nondynamical fields (density, sound speed etc) from files.
   *
   * @param [out] state - (IWAVE *) IWAVE struct pointer
  ...
  /**
   * Applies one action in action list every time calling it. 
   */
  ...
  /**
   * Sets up paralle information for communication - sets the MPI_DATATYPE for sending and receiving.
   * 
