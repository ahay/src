/* 
   ...
   purpose: standalone test of acd sim except for trivial step 
*/
/*============================================================================
 *                            BEGIN INCLUDES 
 * ============================================================================*/
...
int main(int argc, char ** argv) {
  ...
  system("echo wavelet; suspike ntr=1 nt=501  offset=0 nspk=1 ix1=1 it1=201 dt=0.002 | sushw key=delrt  a=-400 |sufilter f=0.,2.5,15.0,20.0 amps=0.,1.,1.,0. | sugain scale=1.e6 > wavelet.su");
  ...
  if (retrieveGroupID() == MPI_UNDEFINED) {
	...
	abortexit(err,pars,&stream);
      ...
      // set up source - array source only for this app
...
#ifdef VERBOSE_STEP
	fprintf(stream,"step %d substep %d - update dynamical fields\n",
		state.model.tsind.it,
		state.model.tsind.iv);
	fflush(stream);
#endif
	...
	*/
	...
	// note - only zero dynamical field panel recorded
	// even for extended model
