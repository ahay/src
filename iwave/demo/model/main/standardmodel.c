...
#define bm_bottom		20700

#define vp_wedge		2.6
#define density_wedge	2200
#define bm_wedge		14872

#define vp_wedge2		2.1
#define density_wedge2	2050.0
#define bm_wedge2		9040.5 

#define vp_h2o			1.5
#define density_h2o		1000.0
#define bm_h2o			2250

float Vdefault[4] = {2100.0, 2.3, 11109.0, 1.0/2100};
float Vbgdefault[4] ={4000.0, 2.5, 25000.0, 1.0/4000}; 
...
int main(int argc, char **argv) {
  ...
  err=ps_ffcstring(par,"par",&parfile);
  if (err==0) {
    ps_destroy(&par);
    if (ps_createfile(&par,parfile)) {
      fprintf(stderr,
              "ERROR. could not create param array from file %s\n",parfile);
      exit(1);
    }
