...
int asg_modelinit(PARARRAY *pars, 
    ...
    /* main branch */
    if (ps_flint(*pars,"order",&(sgnm->k))) {
	/* old-style iwave specs - deprecated */
	if ((err=ps_flcstring(*pars,"scheme_phys",&jnk))) {
	    fprintf(stream,"ERROR: asg_modelinit from ps_flstring\n");
	...
	fprintf(stream,"ERROR: asg_modelinit\n");
	fprintf(stream,"scheme not implemente\n");
	return E_BADINPUT;
    }
