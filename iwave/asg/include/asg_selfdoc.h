...
" ",
"     Spatial interpolation order for sampling (traces) and adjoint ",
"     sampling (source), either 0 (greatest-integer sampling) or 1 ",
...
"       homogeneous fluid ",
"       ",
"       refbou    = [float] [Default = 1.0 g/cm^3] buoyancy in hypothetical",
"       homogeneous fluid",
"       ",
...
"       ",
"       cfl = [float] [Default=0.4] use this fraction of max permitted",
"       time step",
"       ",
...
" refkappa = [float] [default = 2.25 GPa],",
" refbuoy  = [float] [default = 1.0 cm^3/g],",
" refvel   = [float] [default = 1.5 m/ms],",
" refden   = [float] [default = 1.0 g/cm^3]",
" ",
...
"  ",
"  ",
" 1. Parameter Parsing: Parameters are specified as key=value pairs, ",
" as in SU and similar applications. String parameter values representing",
" file names can be either names of files in the working directory, or ",
" relative or fully qualified pathnames. Any line not containing the ",
" separator character (by default, the equals sign =) is treated as a",
...
" parameter parsing in IWAVE, see IWAVE base package documentation.   ",
" ",
" ",
...
" more on installation of these options, see the IWAVE install and build",
" notes.",
" ",
...
" each coordinate axis. The grids are divided as evenly as possible into ",
" subgrids - the code currently makes no attempt to account for load imbalance,",
...
" rocks are  velocities = O(1e+0) m/ms, bulk moduli = O(1-10) GPa, "
" densities = O(1) g/cm^3.",
" ",
...
" "
" This scaling device is a placeholder for an intended unit specification",
" and handling system, to be implemented in some future release."
" ",
" ",
...
" ",
" 1. Stability: The code checks that the input velocity does not exceed ",
" the bounds specified by the optional parameters cmax and cmin. HOWEVER",
" it is by now well-known that for this type of scheme, the CFL condition",
" is not sufficient to guarantee stability.  In fact stabiliy depends also",
" on the density field, markedly so when the density has",
...
" if the user insists on overriding the internally computed step (see ",
" Alternate Use Cases) - in that case, as we said, caveat emptor!   ",
" ",
" ",
