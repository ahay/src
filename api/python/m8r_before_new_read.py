...
class Input(_File):
    ...
    def gettrace(self):
        ...
        if _swig_:
            #kls is self.datatype already set?  can read be changed
            # so I can just read(datacount,self.datatype)  ?
            if self.type == 'float':
                data=np.zeros(datacount,dtype=np.float32)
            elif self.type == 'complex':
                data=np.zeros(datacount,dtype=np.complex64)
            elif self.type == 'int':
                data=np.zeros(datacount,dtype=np.int32)
            else:
                raise TypeError, 'Unsupported file type %s' % self.type
            read(self,data)            
        else:
