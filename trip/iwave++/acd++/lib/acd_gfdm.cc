...
int acd_udfm(int ia, int iv, const IMODEL * m) {
  return ((FD_MODEL *)(m->specs))->update(ia,iv);
}

int acd_udam(int ia, int iv, const IMODEL * m) {
  return ((FD_MODEL *)(m->specs))->update(ia,iv);
}

void acd_adjrefsubstep(int* it,int* iv, const IMODEL* m) {
  *it = (m->tsind).it-1;
  *iv = 0;
}

void acd_linrefsubstep(int* it,int* iv, const IMODEL* m) {
  *it = (m->tsind).it;
  *iv = 0;
}

int acd_gfdm(GFD_MODEL * gfdm) {

  gfdm->gfd_model_init = acd_gfdm;
  gfdm->fd_model_init  = acd_modelinit;
  gfdm->tsf            = acd_tsf;
  gfdm->tsfm           = acd_tsfm;
  gfdm->tsam           = acd_tsam;
  gfdm->udfm           = acd_udfm;
  gfdm->udam           = acd_udam;
  gfdm->linrefsubstep  = acd_linrefsubstep;
  gfdm->adjrefsubstep  = acd_adjrefsubstep;
