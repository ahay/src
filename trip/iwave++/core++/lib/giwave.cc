...
/* altered 12.02.12 to reflect new form of derivative time
   step functions, IWaveLinState */
/* altered 20.03.12 to allow different update flags
   for different modes, iwave2 semantics */
...
int giwave_dmod(IWAVE * pstate, 
  ...
  int rit;      /* ref time step */   
  int riv;      /* ref internal step index */
  int ia;       /* array index */
  int printact; /* verbosity */
  
  FD_MODEL * fdm = (FD_MODEL *)((pstate->model).specs);
  ...
  rit = (rstate->model).tsind.it;
  riv = (rstate->model).tsind.iv;
  printact = rstate->printact;
  //  printact=6;

  if ( printact > 1 ) {
    fprintf(stream,"\n------ giwave_dmod: COMPUTE it = %d iv = %d rit = %d riv = %d ------\n",it,iv,rit,riv);
    fflush(stream); 
  }
  if ( printact > 5 ) {
    fprintf(stream,"\n------ giwave_dmod: before update\n");

    fprintf(stream,"\n------ REFERENCE:\n");
    for (ia=0;ia<RDOM_MAX_NARR;ia++) {
      //      if (ud(ia,iv,&(rstate->model))) {
      if (isdyn(fdm,ia)) {
	fprintf(stream,"------ iarr = %d\n",ia);
	rd_print(&((rstate->model).ld_a), ia, stream);
      }
    }
    fprintf(stream,"\n------ PERTURBATION:\n");
    for (ia=0;ia<RDOM_MAX_NARR;ia++) {
    ...
    fprintf(stream,"giwave_dmod -> ts, iv=%d\n",iv);

    fflush(stream); 
  }

  err = ts(&((pstate->model).ld_c), 
	   &((rstate->model).ld_c), 
	   iv, 
	   ((FD_MODEL *)((rstate->model).specs))->fdpars);
  
  if ( err > 0 ) {
    fprintf(stream, 
	    "ERROR: giwave_dmod\n");
    fprintf(stream,
	    "Internal: timestep error #%d at it=%d, iv=%d. ABORT.\n", 
	    err, it, iv);
    return err;
  }
  ...
  if ( printact > 1 ) {
    fprintf(stream,"\n------ giwave_dmod: finished it = %d iv = %d -------------\n",it,iv);
    fflush(stream); 
  }
...
int giwave_dmod2(IWAVE * ddstate, 
  ...
  //  FD_MODEL * fdm = (FD_MODEL *)((ddstate->model).specs);

  /* we use the time step internal index for the perturbed field because
     this is the field being updated - on call should be same as index for
     unperturbed field, but don't use this */
  it = (ddstate->model).tsind.it;
  iv = (ddstate->model).tsind.iv;
  rit = (rstate->model).tsind.it;
  riv = (rstate->model).tsind.iv;
  printact = rstate->printact;
  //  printact=6;
  ...
  if ( printact > 1 ) {
    fprintf(stream,"\n------ giwave_dmod2: finished it = %d iv = %d -------------\n",it,iv);
    fflush(stream); 
  }
