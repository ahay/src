...
namespace {
  ...
  public:
    ...
    ACDCauchyOpAdjTest(): ic() {
      ...
      val = 0.0;
      create_hfile("movie100.rsf",g,val);
    }
  ...
  TEST_F(ACDCauchyOpAdjTest, wetrun_op_conly_onestep_ord1) {
    try {
      ...
      /*
      AssignFilename csqbfn("csqb.rsf");
      cbm[0].eval(csqbfn);
      AssignFilename ucbfn("ucb.rsf");
      cbm[1].eval(ucbfn);
      AssignFilename upbfn("upb.rsf");
      cbm[2].eval(upbfn);
      */
  ...
  TEST_F(ACDCauchyOpAdjTest, wetrun_op_movie_100step_fdord4_ord1) {
    try {
      rpt<<"\n======================================================\n";
      rpt<<"ACDCauchyOpAdjTest, wetrun_op_movie_100step_fdord4_ord1\n\n";

      // fake command line environment
      std::vector<std::string> args(0);
      args.push_back("deriv=0");
      args.push_back("cmin=1");
      args.push_back("cmax=2");
      args.push_back("cfl=0.5");
      args.push_back("uc_in=u_in.rsf");
      args.push_back("up_in=u_in.rsf");
      args.push_back("uc_out=movie100.rsf");
      args.push_back("up_out=movie100.rsf");
      args.push_back("csq=csq.rsf");
      args.push_back("order=2");
      args.push_back("dt=2");
