    ...
    try {
        ...
        if (retrieveGroupID() == MPI_UNDEFINED) {
            ...
            ChebPolicyData<float> pd(valparse<float>(*pars,"gamma",0.04f),
                                     valparse<float>(*pars,"epsilon",0.01f),
                                     valparse<float>(*pars,"alpha",1.01f),
                                     valparse<float>(*pars,"lbd_est",0.0f),
                                     valparse<int>(*pars,"MaxIter",10),
                                     true);
            ...
            LinFitLSCheb<float> f(top,preop,hop,td,dm0,pd,false,res);
            ...
            LinFitLSCheb<float> const & f3 =
            dynamic_cast<LinFitLSCheb<float> const & >
