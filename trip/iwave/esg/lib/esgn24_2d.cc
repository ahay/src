...
int esgn_gts2d_24ns (RDOM *, RDOM *, RDOM *, void *);

// Shear stress updater of 2D stagger-grid 2-4 scheme for EWE  
int esgn_gts2d_24ss0(RDOM *, RDOM *, RDOM *, void *);

//Velocity component vz updater of 2D stagger-grid 2-4 scheme for EWE  
int esgn_gts2d_24v0 (RDOM *, RDOM *, RDOM *, void *);

//Velocity component vx updater of 2D stagger-grid 2-4 scheme for EWE  
int esgn_gts2d_24v1 (RDOM *, RDOM *, RDOM *, void *);

/*----------------------------------------------------------------------------*/
int esgn_gts2d_24(RDOM *dom, RDOM *rdom, RDOM *cdom, int iarr, void *pars) {
/*----------------------------------------------------------------------------*/
#ifdef MARIO_VERBOSE
	fprintf(stderr,">> Inside esgn_gts2d_24\n");
#endif
 	ESGN_TS_PARS * esgnpars = (ESGN_TS_PARS *) pars;  
  	...
  	if ( iarr == D_P0 ) return esgn_gts2d_24ns (dom, rdom, cdom, esgnpars);
  	if ( iarr == D_S0 ) return esgn_gts2d_24ss0(dom, rdom ,cdom, esgnpars);
  	if ( iarr == D_V0 ) return esgn_gts2d_24v0 (dom, rdom, cdom, esgnpars);
  	if ( iarr == D_V1 ) return esgn_gts2d_24v1( dom, rdom, cdom, esgnpars);
	return E_NOTIMESTEP;
}

#ifdef VEC
int esgn_gts2d_24ns (RDOM *dom, RDOM *rdom, RDOM *cdom, void *pars) {
  	fprintf(stderr, "Warning: VEC version 2D normal stress updater not implemented\n");
  	return 0;
}
int esgn_gts2d_24ss0(RDOM *dom, RDOM *rdom, RDOM *cdom, void *pars) {
  	fprintf(stderr, "Warning: VEC version 2D shear stress updater not implemented\n");
  	return 0;
}
int esgn_gts2d_24v0 (RDOM *dom, RDOM *rdom, RDOM *cdom, void *pars) {
	fprintf(stderr, "Warning: VEC version 2D v0 updater not implemented\n");
  	return 0;
}
int esgn_gts2d_24v1 (RDOM *dom, RDOM *rdom, RDOM *cdom, void *pars) {
...
/*----------------------------------------------------------------------------*/
#ifdef MARIO_VERBOSE
	fprintf(stderr,"> Inside esgn_gts2d_24ns\n");
	int index_x, index_y;
#endif
	int nx, ny, gxs, gys, gxe, gye, px_a, py_a, mp00_a, mp01_a, vx_a, vy_a, ix, iy, tsz, tid;
...
#pragma omp single
	{            
		...
		vy_a         = tsz * s[D_V1]._dims[0].n0 - nx;
			...
			fprintf(stderr,"(gxs,gxe) = (%d,%d)\n(gys,gye) = (%d,%d)\n",gxs,gxe,gys,gye);
			fprintf(stderr,"starting and ending y-axis pml_I indexes: (%d,%d)\n",gys_pml_I,gye_pml_I);
			fprintf(stderr,"px_a = %d\npy_a = %d\n",px_a,py_a);
			fprintf(stderr,"px_pml_I_a   = %d\npy_pml_I_a   = %d\n",px_pml_I_a,py_pml_I_a);
			fprintf(stderr,"px_pml_II_a  = %d\npy_pml_II_a  = %d\n",px_pml_II_a,py_pml_II_a);
			fprintf(stderr,"px_pml_III_a = %d\npy_pml_III_a = %d\n",px_pml_III_a,py_pml_III_a);
			fprintf(stderr,"px_pml_IV_a  = %d\npy_pml_IV_a  = %d\n",px_pml_IV_a,py_pml_IV_a);
			fprintf(stderr,"mp00_a = %d\nmp01_a = %d\n",mp00_a,mp01_a);
			fprintf(stderr,"vx_a = %d\nvy_a = %d\n",vx_a,vy_a);
#endif
	}
#pragma omp barrier
	/** gys == gys_pml_I */
	...
	for ( iy = gys_pml_I+tid; iy < gye_pml_I+1; iy += tsz ){
		...
		for (_pxend = _px + nx; _px < _pxend; ){
			...
			dfdx = (vx0 - vx3 + (vx2 - vx1) * 27.0) * lax; 
			dfdy = ((*_vy0++) - (*_vy3++) + ((*_vy2++) - (*_vy1++)) * 27.0) * lay;
...
#ifdef MARIO_VERBOSE
			index_x = (_px - s[D_P0]._s) % s[D_P0]._dims[0].n0 + s[D_P0]._dims[0].gs;
			index_y = (_px - s[D_P0]._s) / s[D_P0]._dims[1].n0 + s[D_P0]._dims[1].gs;
			if (iy <= gys_pml_I+tid+2){
				fprintf(stderr,"(ix,iy) = (%d,%d)\n",ix,iy);
				fprintf(stderr,"printing px indexes: (%d,%d)\n",index_x,index_y);
			}
// 			if( index_x <= s[D_P0]._dims[0].gs || index_y <= s[D_P0]._dims[1].gs ){
// 				fprintf(stderr,"_px(%d,%d) = %g\n",index_x,index_y,*_px);
// 			}
#endif
		...
		//accounting fot offset in x direction from stencil halo
		_px     += px_a; 
		_py     += py_a; 
		_mp00   += mp00_a; 
		_mp01   += mp01_a; 
	...
	index_x = (_px - s[D_P0]._s) % s[D_P0]._dims[0].n0 + s[D_P0]._dims[0].gs;
	index_y = (_px - s[D_P0]._s) / s[D_P0]._dims[1].n0 + s[D_P0]._dims[1].gs;
	fprintf(stderr,"indexes of px after pml I loop: (%d,%d)\n",index_x,index_y);
#endif		
	/*************************************************************************************/
	...
	index_x = (_px - s[D_P0]._s) % s[D_P0]._dims[0].n0 + s[D_P0]._dims[0].gs;
	index_y = (_px - s[D_P0]._s) / s[D_P0]._dims[1].n0 + s[D_P0]._dims[1].gs;
	fprintf(stderr,"starting indexes for pml 3,4 and physical of px = (%d,%d)\n",index_x,index_y);

	index_x = (_px_x_III - s_pml[D_P0]._s) % s_pml[D_P0]._dims[0].n0 + s_pml[D_P0]._dims[0].gs;
	index_y = (_px_x_III - s_pml[D_P0]._s) / s_pml[D_P0]._dims[1].n0 + s_pml[D_P0]._dims[1].gs;
	fprintf(stderr,"starting indexes for pml 3,4 and physical of px_x_III = (%d,%d)\n",index_x,index_y);
#endif
...
/*----------------------------------------------------------------------------*/
#ifdef MARIO_VERBOSE
	fprintf(stderr,"> Inside esgn_gts2d_24ss0\n");
#endif
...
#pragma omp barrier
    ...
    for ( iy = gys_pml_I+tid; iy < gye_pml_I+1; iy += tsz )
    {
      ...
      for ( _sxyend = _sxy + nx; _sxy < _sxyend; )
      {
        ...
        dfdx = (vy0 - vy3 + (vy2 - vy1) * 27.0) * lax; 
        dfdy = ((*_vx0++) - (*_vx3++) + ((*_vx2++) - (*_vx1++)) * 27.0) * lay;
...
/*----------------------------------------------------------------------------*/
#ifdef MARIO_VERBOSE
	fprintf(stderr,"> Inside esgn_gts2d_24v0\n");
#endif
...
#pragma omp barrier
    ...
    for ( iy = gys_pml_I+tid; iy < gye_pml_I+1; iy += tsz )
    {
      ...
      for ( _vxend = _vx + nx; _vx < _vxend; )
      {
        ...
        dfdx = (px0 - px3 + (px2 - px1) * 27.0) * lax; 
        dfdy = ((*_sxy0++) - (*_sxy3++) + ((*_sxy2++) - (*_sxy1++)) * 27.0) * lay;
...
/*----------------------------------------------------------------------------*/
#ifdef MARIO_VERBOSE
	fprintf(stderr,"> Inside esgn_gts2d_24v1\n");
#endif
    ...
    for ( iy = gys_pml_I+tid; iy < gye_pml_I+1; iy += tsz )
    {
      ...
      */
      for ( _vyend = _vy + nx; _vy < _vyend; )
      {
        ...
        dfdx = (sxy0 - sxy3 + (sxy2 - sxy1) * 27.0) * lax; 
        dfdy = ((*_py0++) - (*_py3++) + ((*_py2++) - (*_py1++)) * 27.0) * lay;
