...
int init_elastic_geom_par(grid * g, PARARRAY par, FILE * stream) {
#ifdef IWAVE_VERBOSE
	fprintf(stream,">> Inside init_elastic_geom_par\n");
	fflush(stream);
#endif
  ...
  ps_flcstring(par, "pvelocity", &pvelkey);
  ps_flcstring(par, "svelocity", &svelkey);
  ps_flcstring(par, "lambda", &lambdakey);
  ps_flcstring(par, "mu", &mukey);

  if (pvelkey) {
    err = read_grid(g, pvelkey, stream);
    //MB replaced: free(pvelkey); return err;
    if (pvelkey) userfree_(pvelkey);
    if (err) return err;
  }
  else if (svelkey) {
    err = read_grid(g, svelkey, stream);
    //MB replaced: free(svelkey); return err;
    if (svelkey) userfree_(svelkey);
    if (err) return err;
  }
  else if (lambdakey) {
    err = read_grid(g, lambdakey, stream);
    //MB replaced: free(lambdakey); return err;
    if (lambdakey) userfree_(lambdakey);
    if (err) return err;
  }
  else if (mukey) {
    err = read_grid(g, mukey, stream);
    //MB replaced: free(mukey); return err;
    if (mukey) userfree_(mukey);
    if (err) return err;
  }
  else {
        fprintf(stream, "Warning: init_elastic_geom\n",
		        "Could not initialize correctly!\n",
			"Searching for n's, d's and o's in parameter list!\n");
    ...
    if (RARR_MAX_NDIM > 0) {
      ...
      if (tmp < 0 || tmp > RARR_MAX_NDIM-1) {
        fprintf(stream, "Error: init_elastic_geom\n");
...
#ifdef IWAVE_VERBOSE
	fprintf(stream,">> Inside esg_readtimegrid\n");
	fflush(stream);
#endif
...
int esg_readmedia(PARARRAY * par, FILE * stream, IMODEL * model, int panelindex) {
#ifdef IWAVE_VERBOSE
	fprintf(stream,">> Inside esg_readmedia\n");
	fflush(stream);
#endif
  ...
  esg_set_grid_type(stream, ndim, gtype);
  ...
  if ( !ps_ffcstring(*par, "lambda", &lambdakey) ) {
    if ( ps_ffcstring(*par, "mu", &mukey) ) {
      userfree_(lambdakey); //MB replaced: free(lambdakey);
      userfree_(mukey); //MB replaced: free(mukey);
      fprintf(stream, 
    ...
    if ( ps_ffcstring(*par, "svelocity", &svelkey) ) {
      userfree_(pvelkey);//RN
      userfree_(svelkey);//RN
      fprintf(stream,
...
static int esgn_setetas(PARARRAY *pars, FILE *stream, IMODEL *model) {
#ifdef IWAVE_VERBOSE
	fprintf(stream,">> Inside esgn_setetas\n");
	fflush(stream);
#endif
  ...
  for ( idim = 0; idim < ndim; ++idim )
  {
    ...
    if ( L0 > 0.0 ) eta0 = pmlampl * cmax / L0; //MB eta0 = log(1/R) * 3/2 * Vp/delta
    else            eta0 = 0.0;
...
//MB	comment: computing d(x) = d0*(s/delta)^2, and storing them in the different grids related to the pressure and vel fields
//MB	comment: may have to modify
    iarr = D_EP[idim];
...
/*fprintf(stderr,"D_EP[%d][%d] = %g\n",idim,ind[0],s);*/
    }
//MB	comment: storing eta0 and eta1 in
    iarr = D_EV[idim];
    rd_gse(dom, iarr, gsa, gea);
    for ( i = 1; i < RARR_MAX_NDIM; ++i ) ind[i] = gsa[i] = gea[i] = 0;
...
#ifdef IWAVE_VERBOSE
	fprintf(stream,">> Inside esgn_readmedia\n");
	fflush(stream);
#endif
...
#ifdef IWAVE_VERBOSE
	fprintf(stream,">> Inside esg_readpmlgrid\n");
	fflush(stream);
#endif
...
int esg_readgrid(PARARRAY * pars, 
		 ...
		 IMODEL * model) {
#ifdef IWAVE_VERBOSE
	fprintf(stream,">> Inside esg_readgrid\n");
	fflush(stream);
#endif
