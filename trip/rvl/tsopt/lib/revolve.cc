...
ACTION::action Online_r2::revolve()
{
  ...
  //fprintf(stdout, " 1 *check %d ch[*check] %d *capo %d endl",check,checkpoint->ch[check],capo); 
	commands++;

	//cout <<  "f->get_check() = " << get_check() << endl;
	//check = get_check();
	//	int blah;
	//cin >> blah;
...
Revolve::Revolve(int st,int sn,int inf)
{
